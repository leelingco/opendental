...
*/
public class PageHeader  extends ReportLink 
{
    RSize _Height;
    ...
    public PageHeader(ReportDefn r, ReportLink p, XmlNode xNode) throws Exception {
        super(r, p);
        _Height = null;
    ...
    public void run(IPresent ip, Row row) throws Exception {
        ...
        // don't process page headers for sub-reports
    ...
    public void runPage(Pages pgs) throws Exception {
        ...
        for (Object __dummyForeachVar1 : pgs)
        {
            ...
            p.setYOffset(OwnerReport.getTopMargin().getPoints());
            p.setXOffset(0);
            ...
            // Don't put header on the first page
            if (p.getPageNumber() == pgs.getCount() && !_PrintOnLastPage)
                continue;
             
            // Don't put header on the last page
