...
import fyiReporting.RDL.CssCacheEntry;
...
*/
public class RenderHtml   implements IPresent
{
    ...
    public RenderHtml(fyiReporting.RDL.Report rep, IStreamGen sg) throws Exception {
        r = rep;
        _sg = sg;
    ...
    private void cssGenerate(TextWriter ftw) throws Exception {
        ...
        {
            CssCacheEntry cce = (CssCacheEntry)__dummyForeachVar1;
    ...
    private String cssPosition(ReportLink rl, Row row, boolean bForceRelative, float h, float w) throws Exception {
        ...
        if (ri instanceof Matrix || ri instanceof Image)
        {
        }
        else
        {
        ...
        else if (ri instanceof Matrix || ri instanceof Table || ri instanceof Image)
        {
        }
    ...
    public void image(Image i, Row r, String mimeType, Stream ioin) throws Exception {
        ...
        int h = i.getHeight() == null ? -1 : i.getHeight().getPixelsY();
        int w = i.getWidth() == null ? -1 : i.getWidth().getPixelsX();
        switch(i.getSizing())
        {
            case AutoSize: 
                break;
        ...
        // would have to create an image to handle this
        sw.Write("/>");
        if (bookmark != null)
            sw.Write("</div>");
