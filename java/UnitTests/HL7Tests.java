...
{
    /**
    * Runs all the tests for a single interface.  Database gets wiped between each interface test.
    */
    public static String hL7TestAll(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
        String retval = "";
    ...
    */
    ...
    /**
    * Test 2: Locate the patient inserted in TEST 1 with PID.2 and the guarantor from TEST 1 with GT1.2.  EcwOldTight,EcwOldFull,EcwTight,HL7DefEcwFull: PID.2 and GT1.2 are PatNum so locate by PatNum.  Update patient.ChartNumber to A11.  All other demographic information should be updated.  EcwOldStandalone,HL7DefEcwStandalone: PID.2 and GT1.2 are ChartNumber so locate by ChartNumber.  patient.ChartNumber is not changed.  All other demographic information should be updated.
    */
    public static String test2(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
    ...
    /**
    * Test 3: If either the FName or LName fields are blank, no information should be updated.  Compare test patient and guarantor before message to test patient and guarantor after message.
    */
    public static String test3(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
    ...
    /**
    * Test 4: -Insert two new patients, patient from PID and guarantor from GT1.  If birthdate is less than 8 digits, set to MinValue of 0001-01-01.  If birthdate is 8 digits or more, set to the correct precision with yyyyMMddHHmmss format and HHmmss... all optional.  patient.Birthdate=0001-01-01.  guarantor.Birthdate=1977-07-03 7:11 AM but inserted as date 1977-07-03.
    */
    public static String test4(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
    ...
    /**
    * Test 5: EcwOldTight,EcwOldFull,EcwTight,HL7DefEcwFull: Fail to locate patient so insert new patient.  No PV1 or AIG segment.  New appointment so insert appointment into database with appointment.AptNum=100.  appointment.PatNum=30; appointment.AptStatus=ApptStatus.Scheduled; appointment.Note="Checkup"; appointment.AptDateTime="2012-09-01 10:00 AM"; appointment.Pattern="XXXX"; appointment.ProvNum=pat.PriProv=preference.PracticeDefaultProv.  EcwOldStandalone,HL7DefEcwStandalone: SIU messages are not processed in Standalone mode.
    */
    public static String test5(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
        ...
        if (!StringSupport.equals(apt.Note, "Checkup"))
        {
        ...
        if (!StringSupport.equals(apt.Pattern, "XXXX"))
        {
    ...
    /**
    * Test 6: EcwOldTight,EcwOldFull,EcwTight,HL7DefEcwFull: Locate the patient with PatNum in PID.2 and appointment with AptNum in SCH.1 (inserted in Test 5).  Update the appointment and patient information.  appointment.Note="Checkup and Fillings"; appointment.AptDateTime="2012-09-02 10:00 AM"; appointment.Pattern="XXXXXXXX".  EcwOldStandalone,HL7DefEcwStandalone: SIU messages are not processed in Standalone mode.
    */
    public static String test6(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
        ...
        pat = Patients.getPat(correctPat.PatNum);
        guar = Patients.getPat(correctPat.PatNum);
        //Get copies of same pat to send to our comparison function CompareGuarAndPat
        ...
        if (!StringSupport.equals(apt.Note, "Checkup and Fillings"))
        {
        ...
        if (!StringSupport.equals(apt.Pattern, "XXXXXXXX"))
        {
    ...
    /**
    * Test 7: EcwOldTight,EcwOldFull,EcwTight,HL7DefEcwFull: Before processing this message, change appointment.PatNum from appointment updated in Test 6 to 40.  Locate the patient with PID.2 and appointment with SCH.1.  Compare appointment.PatNum (now 40) to patient.PatNum (30) and since they don't match anymore, do not update any information.  We will use appointment.AptDateTime to verify that nothing was changed in the appointment table.  Compare patient before processing message to patient after processing to verify no changes were made.  After test return appointment.PatNum to 30 for TEST 8.  EcwOldStandalone,HL7DefEcwStandalone: SIU messages are not processed in Standalone mode.
    */
    public static String test7(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
        ...
        catch (Exception ex)
        {
            ...
            if (!StringSupport.equals(ex.ToString().Substring(18, 35), "Appointment does not match patient:"))
            {
    ...
    /**
    * Test 8: EcwOldTight,EcwOldFull,EcwTight,HL7DefEcwFull: Locate the patient and appointment but since there is no LName, do not update any information.  We will use appointment.Note and patient information to verify that no information was changed.  EcwOldStandalone,HL7DefEcwStandalone: SIU messages are not processed in Standalone mode.
    */
    public static String test8(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
        ...
        {
            //Catch the exception and if it is the expected exception that would normally get logged in event log just move on.
            if (!StringSupport.equals(ex.ToString().Substring(18, 68), "Appointment not processed due to missing patient first or last name."))
            {
    ...
    /**
    * Test 9: EcwOldTight,EcwOldFull,EcwTight,HL7DefEcwFull: Optional AIG segment is included so use that segment to insert a new provider.  provider.Abbr=DOC1; provider.EcwID=DOC1; provider.LName=Albert; provider.FName=Brian; provider.MI=S; appointment.ProvNum=provider.ProvNum; patient.PriProv=provider.ProvNum.  EcwOldStandalone,HL7DefEcwStandalone: SIU messages are not processed in Standalone mode.
    */
    public static String test9(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
        ...
        switch(hl7TestInterfaceEnum)
        {
            case EcwOldFull: 
            case EcwOldTight: 
                if (!StringSupport.equals(prov.LName, "Albert, Brian S"))
                {
            ...
            case HL7DefEcwFull: 
            case HL7DefEcwTight: 
                if (!StringSupport.equals(prov.LName, "Albert"))
                {
                    retval += "Provider.LName is not 'Albert'.\r\n";
                }
                 
                if (!StringSupport.equals(prov.FName, "Brian"))
                {
                    retval += "Provider.FName is not 'Brian'.\r\n";
                }
                 
                if (!StringSupport.equals(prov.MI, "S"))
                {
        ...
        if (!StringSupport.equals(prov.Abbr, "DOC1"))
        {
    ...
    /**
    * Test 10: EcwOldTight,EcwOldFull,EcwTight,HL7DefEcwFull: Use the PV1 segment to insert new provider and set appropriate fields as in Test 9.  EcwOldStandalone,HL7DefEcwStandalone: SIU messages are not processed in Standalone mode.
    */
    public static String test10(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
        ...
        String retval = "";
        if (!StringSupport.equals(prov.Abbr, "DOC2"))
        {
            retval += "Provider.Abbr is not 'DOC2'.\r\n";
        }
         
        if (!StringSupport.equals(prov.LName, "Lexington"))
        {
            retval += "Provider.LName is not 'Lexington'.\r\n";
        }
         
        if (!StringSupport.equals(prov.FName, "Sarah"))
        {
            retval += "Provider.FName is not 'Sarah'.\r\n";
        }
         
        if (!StringSupport.equals(prov.MI, "J"))
        {
    ...
    /**
    * Test 11: EcwOldTight,EcwOldFull,EcwTight,HL7DefEcwFull: Add 2 D0230 procedures, a D0150 procedure, and a D2332 procedure all with complete status and ProcDate='2012-09-06' to patient with PatNum=10.  Set the D2332 procedurelog.ToothNum=26 and procedurelog.Surf=MID.  Make sure the procedurelog.ProcFee for D0230 is 20.00, the D0150 is 75.00, and the D2332 is 150.00.  Using DOC1 provider from TEST 9, schedule appointment for patient 10 with appointment.AptNum=500 and appointment.ProvNum=provider.ProvNum and attach the four procedures.  Create a DFT message for this patient, provider, appointment, and procedures.  EcwOldStandalone,HL7DefEcwStandalone: DFT messages are not created in Standalone mode.
    */
    public static String test11(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
    ...
    /**
    * Test 12: EcwOldTight,EcwOldFull,EcwTight,HL7DefEcwFull: Create message with multiple providers.  Change the provider for the D2332 procedure on the appointment with AptNum=500 to DOC2 from Test 10.  Create DFT message again for this patient, provider, appointment, and procedures and verify that the FT1 segment for that procedure lists the correct provider.  EcwOldStandalone,HL7DefEcwStandalone: DFT messages are not created in Standalone mode.
    */
    public static String test12(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
        if (hl7TestInterfaceEnum == HL7TestInterfaceEnum.EcwOldStandalone || hl7TestInterfaceEnum == HL7TestInterfaceEnum.HL7DefEcwStandalone)
        {
    ...
    /**
    * Test 13: EcwOldTight,EcwOldFull,EcwTight,HL7DefEcwFull: Truncate D codes to Dxxxx, if not a D code don't truncate: Insert a procedure D2332A, with treatment area surface and ProcFee=200.00.  Insert another procedure 2332AA with treatment area surface and ProcFee=250.00.  Add a D2332A on tooth 1 with surfaces MOD, and a 2332AA on tooth 2 with surfaces MOD to patient's chart.  Schedule an appointment for patient with AptNum=600, appointment.ProvNum=ProvNum for provider DOC1 and attach the two procedures.  Create a DFT message for these procedures, appointment, patient, and provider and verify that the D2332A gets truncated to D2332 and the 2332AA doesn't get changed.  EcwOldStandalone,HL7DefEcwStandalone: DFT messages are not created in Standalone mode.
    */
    public static String test13(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
        if (hl7TestInterfaceEnum == HL7TestInterfaceEnum.EcwOldStandalone || hl7TestInterfaceEnum == HL7TestInterfaceEnum.HL7DefEcwStandalone)
        {
    ...
    /**
    * Test 14: Create a treatment plan DFT message with an embedded PDF.  SendHL7 with appointment.AptNum=600, patient.PatNum=10, patient.PriProv=provider.ProvNum for DOC1, pdfDescription=treatment, and the PDF data converted to Base-64 string (for testing purposes this string will just be "Test Message").  No FT1 segments in a PDF DFT message.
    */
    public static String test14(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
        return "";
    }
    ...
    */
    public static String test15(HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
        ...
        if (prov.FeeSched != FeeSchedC.getListShort()[0].FeeSchedNum)
        {
            return "Test 15: Provider fee schedule is not set to " + FeeSchedC.getListShort()[0].Description + ".\r\n";
        }
    ...
    /**
    * Compares the pat and guar to correctPat and correctGuar to make sure every field matches.
    */
    public static String compareGuarAndPat(Patient pat, Patient correctPat, Patient guar, Patient correctGuar, HL7TestInterfaceEnum hl7TestInterfaceEnum) throws Exception {
        String retval = "";
        ...
        if (!StringSupport.equals(pat.ChartNumber, correctPat.ChartNumber) || !StringSupport.equals(guar.ChartNumber, correctGuar.ChartNumber))
        {
        ...
        if (hl7TestInterfaceEnum == HL7TestInterfaceEnum.EcwOldFull || hl7TestInterfaceEnum == HL7TestInterfaceEnum.EcwOldTight || hl7TestInterfaceEnum == HL7TestInterfaceEnum.HL7DefEcwFull || hl7TestInterfaceEnum == HL7TestInterfaceEnum.HL7DefEcwTight)
        {
        ...
        if (!StringSupport.equals(pat.LName, correctPat.LName) || !StringSupport.equals(guar.LName, correctPat.LName))
        {
        ...
        if (!StringSupport.equals(pat.FName, correctPat.FName) || !StringSupport.equals(guar.FName, correctGuar.FName))
        {
        ...
        if (!StringSupport.equals(pat.MiddleI, correctPat.MiddleI) || !StringSupport.equals(guar.MiddleI, correctGuar.MiddleI))
        {
        ...
        //}
        if (!StringSupport.equals(pat.Address, correctPat.Address) || !StringSupport.equals(pat.Address2, correctPat.Address2) || !StringSupport.equals(pat.City, correctPat.City) || !StringSupport.equals(pat.State, correctPat.State) || !StringSupport.equals(pat.Zip, correctPat.Zip))
        {
        ...
        if (!StringSupport.equals(guar.Address, correctGuar.Address) || !StringSupport.equals(guar.Address2, correctGuar.Address2) || !StringSupport.equals(guar.City, correctGuar.City) || !StringSupport.equals(guar.State, correctGuar.State) || !StringSupport.equals(guar.Zip, correctGuar.Zip))
        {
        ...
        if (!StringSupport.equals(pat.HmPhone, correctPat.HmPhone) || !StringSupport.equals(guar.HmPhone, correctGuar.HmPhone))
        {
        ...
        if (!StringSupport.equals(pat.WkPhone, correctPat.WkPhone) || !StringSupport.equals(guar.WkPhone, correctGuar.WkPhone))
        {
        ...
        if (!StringSupport.equals(pat.SSN, correctPat.SSN) || !StringSupport.equals(guar.SSN, correctGuar.SSN))
        {
    ...
    /**
    * Compare an HL7 message to a correct HL7 message and return any mismatched fields.
    */
    public static String compareMsgs(MessageHL7 msg, MessageHL7 correctMsg) throws Exception {
        String retval = "";
        for (int s = 0;s < msg.Segments.Count;s++)
        {
            for (int f = 0;f < msg.Segments[s].Fields.Count;f++)
            {
                ...
                {
                    retval += "Correct message segment " + correctMsg.Segments[s].Name.ToString() + " field " + f + ": " + correctMsg.Segments[s].Fields[f].ToString() + ".  " + "Message segment " + msg.Segments[s].Name.ToString() + " field " + f + ": " + msg.Segments[s].Fields[f].ToString() + ".\r\n";
                }
