...
* 
*/
public class FormFinanceCharges  extends System.Windows.Forms.Form 
{
    ...
    private OpenDental.ValidDate textDateLastRun;
    private Label label5 = new Label();
    private OpenDental.UI.Button butUndo;
    private GroupBox groupBox2 = new GroupBox();
    private OpenDental.ValidDate textDateUndo;
    private Label label6 = new Label();
    private ListBox listBillType = new ListBox();
    private Panel panel1 = new Panel();
    private Label label8 = new Label();
    private OpenDental.ValidDouble textBillingCharge;
    private RadioButton radioBillingCharge = new RadioButton();
    private RadioButton radioFinanceCharge = new RadioButton();
    private Label label7 = new Label();
    //private int adjType;
    /**
    * 
    */
    public FormFinanceCharges() throws Exception {
        initializeComponent();
        Lan.f(this);
    }
    ...
    */
    ...
    private void initializeComponent() throws Exception {
        System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(FormFinanceCharges.class);
        ...
        this.textAPR.setMaxVal(255);
        this.textAPR.setMinVal(0);
    ...
    private void formFinanceCharges_Load(Object sender, System.EventArgs e) throws Exception {
        ...
        Ledgers.runAging();
        if (DefC.getShort()[((Enum)DefCat.BillingTypes).ordinal()].Length == 0)
        {
        ...
        if (StringSupport.equals(defaultChargeMethod, "Finance"))
        {
        ...
        else if (StringSupport.equals(defaultChargeMethod, "Billing"))
        {
    ...
    private void radioFinanceCharge_CheckedChanged(Object sender, EventArgs e) throws Exception {
    ...
    private void radioBillingCharge_CheckedChanged(Object sender, EventArgs e) throws Exception {
    ...
    private void butUndo_Click(Object sender, EventArgs e) throws Exception {
        ...
        {
            if (MessageBox.Show(Lan.g(this,"Undo all finance charges for ") + textDateUndo.Text + "?", "", MessageBoxButtons.OKCancel) != DialogResult.OK)
            {
                return ;
            }
        ...
        {
            if (MessageBox.Show(Lan.g(this,"Undo all billing charges for ") + textDateUndo.Text + "?", "", MessageBoxButtons.OKCancel) != DialogResult.OK)
            {
                return ;
            }
    ...
    private void butOK_Click(Object sender, System.EventArgs e) throws Exception {
        if (!StringSupport.equals(textDate.errorProvider1.GetError(textDate), "") || !StringSupport.equals(textAPR.errorProvider1.GetError(textAPR), ""))
        {
        ...
        double OverallBalance = new double();
        int rowsAffected = 0;
        boolean billingMatch = new boolean();
        for (int i = 0;i < AgingList.Length;i++)
        {
            //loop through each guarantor that owes money.
            OverallBalance = 0;
            //this WILL NOT be the same as the patient's total balance
            if (radio30.Checked)
            {
            ...
            {
                if (DefC.getShort()[((Enum)DefCat.BillingTypes).ordinal()][listBillType.SelectedIndices[b]].DefNum == AgingList[i].BillingType)
                {
                    billingMatch = true;
                    break;
                }
        ...
        if (radioFinanceCharge.Checked)
        {
            if (Prefs.UpdateString(PrefName.FinanceChargeAPR, textAPR.Text) | Prefs.UpdateString(PrefName.FinanceChargeLastRun, POut.Date(date, false)))
            {
        ...
        {
            if (Prefs.UpdateString(PrefName.BillingChargeAmount, textBillingCharge.Text) | Prefs.UpdateString(PrefName.BillingChargeLastRun, POut.Date(date, false)))
            {
                DataValid.setInvalid(InvalidType.Prefs);
            }
    ...
    private void addFinanceCharge(long PatNum, DateTime date, String APR, double OverallBalance, long PriProv) throws Exception {
        InstallmentPlan installPlan = InstallmentPlans.getOneForFam(PatNum);
        if (installPlan != null)
        {
            //Patient has an installment plan so use that APR instead.
    ...
    private void addBillingCharge(long PatNum, DateTime date, String BillingChargeAmount, long PriProv) throws Exception {
        Adjustment AdjustmentCur = new Adjustment();
        AdjustmentCur.PatNum = PatNum;
        //AdjustmentCur.DateEntry=PIn.PDate(textDate.Text);//automatically handled
        AdjustmentCur.AdjDate = date;
