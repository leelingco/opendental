...
* 
*/
public class FormRpReceivablesBreakdown  extends System.Windows.Forms.Form 
{
    ...
    private Label labelProvider = new Label();
    private ListBox listClinic = new ListBox();
    private Label labClinic = new Label();
    private GroupBox groupInsBox = new GroupBox();
    ...
    * 
    */
    public FormRpReceivablesBreakdown() throws Exception {
        initializeComponent();
        Lan.f(this);
    }
    ...
    */
    private void initializeComponent() throws Exception {
        System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(FormRpReceivablesBreakdown.class);
        ...
        this.label2.Text = "This report has some limitations.  It does not take into account any payment plan" + "s.";
        //
    ...
    private void formRpReceivablesBreakdown_Load(Object sender, EventArgs e) throws Exception {
    ...
    private void butOK_Click(Object sender, System.EventArgs e) throws Exception {
        ...
        double rcvProd = 0;
        double rcvAdj = 0;
        double rcvWriteoff = 0;
        double rcvPayment = 0;
        double rcvInsPayment = 0;
        double runningRcv = 0;
        double rcvDaily = 0;
        double[] ColTotal = new double[8];
        String wMonth = new String();
        String wYear = new String();
        String wDay = "01";
        ...
        for (int j = 0;j <= 1;j++)
        {
            ...
            {
                bDate = wDate;
                eDate = POut.Date(date1.SelectionStart.AddDays(1)).Substring(1, 10);
            } 
            // Needed because all Queries are < end date to get correct Starting AR
            ...
            String whereProvx = new String();
            //Extended for more than 4 names
            whereProv = "";
            ...
            if (listProv.SelectedIndices[0] != 0)
            {
                for (int i = 0;i < listProv.SelectedIndices.Count;i++)
                {
                    ...
                    whereProv += "claimproc.ProvNum = " + POut.Long(ProviderC.getListShort()[listProv.SelectedIndices[i] - 1].ProvNum) + " ";
                }
                whereProv += ") ";
            }
            ...
            {
                //CapComplete
                report.Query = "SELECT DateCP, " + "SUM(WriteOff) FROM claimproc WHERE " + "DateCP >= '" + bDate + "' " + "AND DateCP < '" + eDate + "' " + "AND Status = '7' " + whereProv + " GROUP BY DateCP " + "ORDER BY DateCP";
            }
            else
            {
                //CapComplete
                report.Query = "SELECT ProcDate, " + "SUM(WriteOff) FROM claimproc WHERE " + "ProcDate >= '" + bDate + "' " + "AND ProcDate < '" + eDate + "' " + "AND Status = '7' " + whereProv + " GROUP BY ProcDate " + "ORDER BY ProcDate";
            } 
            ...
            if (listProv.SelectedIndices[0] != 0)
            {
                for (int i = 0;i < listProv.SelectedIndices.Count;i++)
                {
                    ...
                    whereProv += "ProvNum = " + POut.Long(ProviderC.getListShort()[listProv.SelectedIndices[i] - 1].ProvNum) + " ";
                }
                whereProv += ") ";
            }
            ...
            if (radioWriteoffPay.Checked)
            {
                //Recieved, supplemental, capclaim. Otherwise, it's only an estimate. 7-CapCompl handled above.
                report.Query = "SELECT DateCP, " + "SUM(WriteOff) FROM claimproc WHERE " + "DateCP >= '" + bDate + "' " + "AND DateCP < '" + eDate + "' " + "AND Status IN (1,4,5) " + whereProv + " GROUP BY DateCP " + "ORDER BY DateCP";
            }
            else
            {
                //Notreceived, received, supplemental, capclaim. 7-CapCompl handled above.
                report.Query = "SELECT ProcDate, " + "SUM(WriteOff) FROM claimproc WHERE " + "ProcDate >= '" + bDate + "' " + "AND ProcDate < '" + eDate + "' " + "AND Status IN (0,1,4,5) " + whereProv + " GROUP BY ProcDate " + "ORDER BY ProcDate";
            } 
            ...
            if (listProv.SelectedIndices[0] != 0)
            {
                for (int i = 0;i < listProv.SelectedIndices.Count;i++)
                {
                    ...
                    whereProv += "paysplit.ProvNum = " + POut.Long(ProviderC.getListShort()[listProv.SelectedIndices[i] - 1].ProvNum) + " ";
                }
                whereProv += ") ";
            }
             
            report.Query = "SELECT paysplit.DatePay,SUM(paysplit.splitamt) FROM paysplit " + "WHERE paysplit.PayPlanNum=0 " + "AND paysplit.DatePay >= '" + bDate + "' " + "AND paysplit.DatePay < '" + eDate + "' " + whereProv + " GROUP BY paysplit.DatePay ORDER BY DatePay";
            ...
            //Received, supplemental, capclaim, capcomplete.
            report.Query = "SELECT DateCP,SUM(InsPayamt) " + "FROM claimproc WHERE " + "Status IN (1,4,5,7) " + "AND DateCP >= '" + bDate + "' " + "AND DateCP < '" + eDate + "' " + whereProv + " GROUP BY DateCP ORDER BY DateCP";
            ...
            report.Query = "SELECT adjdate, SUM(adjamt) FROM adjustment WHERE " + "adjdate >= '" + bDate + "' " + "AND adjdate < '" + eDate + "' " + whereProv + " GROUP BY adjdate ORDER BY adjdate";
            ...
            else
            {
                ...
                eDate = POut.Date(date1.SelectionStart).Substring(1, 10);
                // Reset EndDate to Selected Date
                ...
                if (listProv.SelectedIndices[0] != 0)
                {
                    ...
                    for (int i = 0;i < listProv.SelectedIndices.Count;i++)
                    {
                        if (nameCount < 3)
                        {
                            whereProv += " " + ProviderC.getListShort()[listProv.SelectedIndices[i] - 1].GetFormalName() + " /";
                        }
                        else
                        {
                            whereProvx += " " + ProviderC.getListShort()[listProv.SelectedIndices[i] - 1].GetFormalName() + " /";
                        } 
                        nameCount += 1;
                    }
                ...
                report.Summary.Add(Lan.g(this,"Receivables Calculation: (Production + Adjustments - Writeoffs) - (Payments + Insurance Payments)"));
