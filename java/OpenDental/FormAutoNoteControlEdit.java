//
// Translated by CS2J (http://www.cs2j.com): 2/15/2016 8:01:41 PM
//

package OpenDental;

import CS2JNet.System.StringSupport;
import OpenDental.Lan;
import OpenDental.MsgBox;
import OpenDental.MsgBoxButtons;
import OpenDentBusiness.AutoNoteControl;
import OpenDentBusiness.AutoNoteControls;
import OpenDental.FormAutoNoteControlEdit;
import OpenDental.Properties.Resources;

public class FormAutoNoteControlEdit  extends Form 
{

    public boolean IsNew = new boolean();
    /**
    * The current AutoNoteControl that is being edited, whether new or not.
    */
    public AutoNoteControl ControlCur;
    public FormAutoNoteControlEdit() throws Exception {
        initializeComponent();
        Lan.F(this);
    }

    private void formAutoNoteControlEdit_Load(Object sender, EventArgs e) throws Exception {
        textBoxControlDescript.Text = ControlCur.Descript;
        textBoxControlLabel.Text = ControlCur.ControlLabel;
        comboType.Items.Clear();
        comboType.Items.Add("Text");
        comboType.Items.Add("OneResponse");
        comboType.Items.Add("MultiResponse");
        comboType.SelectedItem = ControlCur.ControlType;
        textOptions.Text = ControlCur.ControlOptions;
    }

    private void comboType_SelectedIndexChanged(Object sender, EventArgs e) throws Exception {
        SelectedItem.APPLY __dummyScrutVar0 = comboType.SelectedItem.ToString();
        if (__dummyScrutVar0.equals("Text"))
        {
            labelResponses.Text = Lan.g(this,"Default text");
        }
        else if (__dummyScrutVar0.equals("OneResponse"))
        {
            labelResponses.Text = Lan.g(this,"Possible responses (one line per item)");
        }
        else if (__dummyScrutVar0.equals("MultiResponse"))
        {
            labelResponses.Text = Lan.g(this,"Possible responses (one line per item)");
        }
           
    }

    private void butUp_Click(Object sender, EventArgs e) throws Exception {
        if (StringSupport.equals(textOptions.Text, ""))
        {
            return ;
        }
         
        int selStart = textOptions.SelectionStart;
        //calculate which row to highlight, based on selection start.
        int selectedRow = 0;
        int sumPreviousLines = 0;
        String[] linesOrig = new String[textOptions.Lines.Length];
        textOptions.Lines.CopyTo(linesOrig, 0);
        for (int l = 0;l < textOptions.Lines.Length;l++)
        {
            if (l > 0)
            {
                sumPreviousLines += textOptions.Lines[l - 1].Length + 2;
            }
             
            //the 2 is for \r\n
            if (selStart < sumPreviousLines + textOptions.Lines[l].Length)
            {
                selectedRow = l;
                break;
            }
             
        }
        //swap rows
        int newSelectedRow = new int();
        if (selectedRow == 0)
        {
            newSelectedRow = 0;
        }
        else
        {
            //and no swap
            //doesn't allow me to directly set lines, so:
            String newtext = "";
            for (int l = 0;l < textOptions.Lines.Length;l++)
            {
                if (l > 0)
                {
                    newtext += "\r\n";
                }
                 
                if (l == selectedRow)
                {
                    newtext += linesOrig[selectedRow - 1];
                }
                else if (l == selectedRow - 1)
                {
                    newtext += linesOrig[selectedRow];
                }
                else
                {
                    newtext += linesOrig[l];
                }  
            }
            textOptions.Text = newtext;
            newSelectedRow = selectedRow - 1;
        } 
        //highlight the newSelectedRow
        sumPreviousLines = 0;
        for (int l = 0;l < textOptions.Lines.Length;l++)
        {
            if (l > 0)
            {
                sumPreviousLines += textOptions.Lines[l - 1].Length + 2;
            }
             
            //the 2 is for \r\n
            if (newSelectedRow == l)
            {
                textOptions.Select(sumPreviousLines, textOptions.Lines[l].Length);
                break;
            }
             
        }
    }

    private void butDown_Click(Object sender, EventArgs e) throws Exception {
        if (StringSupport.equals(textOptions.Text, ""))
        {
            return ;
        }
         
        int selStart = textOptions.SelectionStart;
        //calculate which row to highlight, based on selection start.
        int selectedRow = 0;
        int sumPreviousLines = 0;
        String[] linesOrig = new String[textOptions.Lines.Length];
        textOptions.Lines.CopyTo(linesOrig, 0);
        for (int l = 0;l < textOptions.Lines.Length;l++)
        {
            if (l > 0)
            {
                sumPreviousLines += textOptions.Lines[l - 1].Length + 2;
            }
             
            //the 2 is for \r\n
            if (selStart < sumPreviousLines + textOptions.Lines[l].Length)
            {
                selectedRow = l;
                break;
            }
             
        }
        //swap rows
        int newSelectedRow = new int();
        if (selectedRow == textOptions.Lines.Length - 1)
        {
            newSelectedRow = textOptions.Lines.Length - 1;
        }
        else
        {
            //and no swap
            //doesn't allow me to directly set lines, so:
            String newtext = "";
            for (int l = 0;l < textOptions.Lines.Length;l++)
            {
                if (l > 0)
                {
                    newtext += "\r\n";
                }
                 
                if (l == selectedRow)
                {
                    newtext += linesOrig[selectedRow + 1];
                }
                else if (l == selectedRow + 1)
                {
                    newtext += linesOrig[selectedRow];
                }
                else
                {
                    newtext += linesOrig[l];
                }  
            }
            textOptions.Text = newtext;
            newSelectedRow = selectedRow + 1;
        } 
        //highlight the newSelectedRow
        sumPreviousLines = 0;
        for (int l = 0;l < textOptions.Lines.Length;l++)
        {
            if (l > 0)
            {
                sumPreviousLines += textOptions.Lines[l - 1].Length + 2;
            }
             
            //the 2 is for \r\n
            if (newSelectedRow == l)
            {
                textOptions.Select(sumPreviousLines, textOptions.Lines[l].Length);
                break;
            }
             
        }
    }

    private void butDelete_Click(Object sender, EventArgs e) throws Exception {
        if (IsNew)
        {
            DialogResult = DialogResult.Cancel;
            return ;
        }
         
        if (!MsgBox.show(this,MsgBoxButtons.OKCancel,"Completely delete this prompt?  It will not be available from any AutoNote."))
        {
            return ;
        }
         
        AutoNoteControls.delete(ControlCur.AutoNoteControlNum);
        DialogResult = DialogResult.OK;
    }

    private void butOK_Click(Object sender, EventArgs e) throws Exception {
        if (StringSupport.equals(textBoxControlDescript.Text.ToString(), "") || comboType.SelectedIndex == -1)
        {
            MsgBox.show(this,"Please make sure that the Description and Type are not blank");
            return ;
        }
         
        if (!Regex.IsMatch(textBoxControlDescript.Text, "^[a-zA-Z_0-9 ]*$"))
        {
            MsgBox.show(this,"The description can only contain letters, numbers, underscore, and space.");
            return ;
        }
         
        ControlCur.Descript = textBoxControlDescript.Text.ToString();
        ControlCur.ControlLabel = textBoxControlLabel.Text.ToString();
        ControlCur.ControlType = comboType.SelectedItem.ToString();
        ControlCur.ControlOptions = textOptions.Text;
        if (IsNew)
        {
            AutoNoteControls.insert(ControlCur);
        }
        else
        {
            AutoNoteControls.update(ControlCur);
        } 
        DialogResult = DialogResult.OK;
    }

    private void butCancel_Click(Object sender, EventArgs e) throws Exception {
        DialogResult = DialogResult.Cancel;
    }

    /**
    * Required designer variable.
    */
    private System.ComponentModel.IContainer components = null;
    /**
    * Clean up any resources being used.
    * 
    *  @param disposing true if managed resources should be disposed; otherwise, false.
    */
    protected void dispose(boolean disposing) throws Exception {
        if (disposing && (components != null))
        {
            components.Dispose();
        }
         
        super.Dispose(disposing);
    }

    /**
    * Required method for Designer support - do not modify
    * the contents of this method with the code editor.
    */
    private void initializeComponent() throws Exception {
        System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(FormAutoNoteControlEdit.class);
        this.labelOptions = new System.Windows.Forms.Label();
        this.textBoxOptionsControl = new System.Windows.Forms.TextBox();
        this.labelLabel = new System.Windows.Forms.Label();
        this.labelName = new System.Windows.Forms.Label();
        this.listBoxOptionsControls = new System.Windows.Forms.ListBox();
        this.textBoxLabelControl = new System.Windows.Forms.TextBox();
        this.textBoxDescriptControl = new System.Windows.Forms.TextBox();
        this.labelType = new System.Windows.Forms.Label();
        this.labeldouble = new System.Windows.Forms.Label();
        this.label = new System.Windows.Forms.Label();
        this.textBoxTextControl = new System.Windows.Forms.TextBox();
        this.label1 = new System.Windows.Forms.Label();
        this.comboType = new System.Windows.Forms.ComboBox();
        this.textBoxControlLabel = new System.Windows.Forms.TextBox();
        this.textBoxControlDescript = new System.Windows.Forms.TextBox();
        this.labelControlType = new System.Windows.Forms.Label();
        this.labelControlLabel = new System.Windows.Forms.Label();
        this.labelControlName = new System.Windows.Forms.Label();
        this.textOptions = new System.Windows.Forms.TextBox();
        this.labelResponses = new System.Windows.Forms.Label();
        this.butDelete = new OpenDental.UI.Button();
        this.butCancel = new OpenDental.UI.Button();
        this.butOK = new OpenDental.UI.Button();
        this.butAddOptionControl = new OpenDental.UI.Button();
        this.butCreate = new OpenDental.UI.Button();
        this.butDown = new OpenDental.UI.Button();
        this.butUp = new OpenDental.UI.Button();
        this.SuspendLayout();
        //
        // labelOptions
        //
        this.labelOptions.AutoSize = true;
        this.labelOptions.Location = new System.Drawing.Point(76, 208);
        this.labelOptions.Name = "labelOptions";
        this.labelOptions.Size = new System.Drawing.Size(207, 13);
        this.labelOptions.TabIndex = 99;
        this.labelOptions.Text = "You can add options for the control below ";
        this.labelOptions.Visible = false;
        //
        // textBoxOptionsControl
        //
        this.textBoxOptionsControl.Location = new System.Drawing.Point(103, 250);
        this.textBoxOptionsControl.Name = "textBoxOptionsControl";
        this.textBoxOptionsControl.Size = new System.Drawing.Size(177, 20);
        this.textBoxOptionsControl.TabIndex = 97;
        this.textBoxOptionsControl.Visible = false;
        //
        // labelLabel
        //
        this.labelLabel.AutoSize = true;
        this.labelLabel.Location = new System.Drawing.Point(64, 107);
        this.labelLabel.Name = "labelLabel";
        this.labelLabel.Size = new System.Drawing.Size(33, 13);
        this.labelLabel.TabIndex = 96;
        this.labelLabel.Text = "Label";
        //
        // labelName
        //
        this.labelName.AutoSize = true;
        this.labelName.Location = new System.Drawing.Point(28, 70);
        this.labelName.Name = "labelName";
        this.labelName.Size = new System.Drawing.Size(71, 13);
        this.labelName.TabIndex = 95;
        this.labelName.Text = "Control Name";
        //
        // listBoxOptionsControls
        //
        this.listBoxOptionsControls.FormattingEnabled = true;
        this.listBoxOptionsControls.Location = new System.Drawing.Point(121, 276);
        this.listBoxOptionsControls.Name = "listBoxOptionsControls";
        this.listBoxOptionsControls.Size = new System.Drawing.Size(120, 147);
        this.listBoxOptionsControls.TabIndex = 94;
        this.listBoxOptionsControls.Visible = false;
        //
        // textBoxLabelControl
        //
        this.textBoxLabelControl.Location = new System.Drawing.Point(103, 104);
        this.textBoxLabelControl.Name = "textBoxLabelControl";
        this.textBoxLabelControl.Size = new System.Drawing.Size(177, 20);
        this.textBoxLabelControl.TabIndex = 93;
        //
        // textBoxDescriptControl
        //
        this.textBoxDescriptControl.Location = new System.Drawing.Point(103, 67);
        this.textBoxDescriptControl.Name = "textBoxDescriptControl";
        this.textBoxDescriptControl.Size = new System.Drawing.Size(177, 20);
        this.textBoxDescriptControl.TabIndex = 92;
        //
        // labelType
        //
        this.labelType.AutoSize = true;
        this.labelType.Location = new System.Drawing.Point(64, 32);
        this.labelType.Name = "labelType";
        this.labelType.Size = new System.Drawing.Size(31, 13);
        this.labelType.TabIndex = 104;
        this.labelType.Text = "Type";
        //
        // labeldouble
        //
        this.labeldouble.AutoSize = true;
        this.labeldouble.Location = new System.Drawing.Point(99, 221);
        this.labeldouble.Name = "labeldouble";
        this.labeldouble.Size = new System.Drawing.Size(163, 13);
        this.labeldouble.TabIndex = 105;
        this.labeldouble.Text = "Double click on option to remove";
        this.labeldouble.Visible = false;
        //
        // label
        //
        this.label.AutoSize = true;
        this.label.Location = new System.Drawing.Point(109, 234);
        this.label.Name = "label";
        this.label.Size = new System.Drawing.Size(132, 13);
        this.label.TabIndex = 106;
        this.label.Text = "Click Create when finished";
        this.label.Visible = false;
        //
        // textBoxTextControl
        //
        this.textBoxTextControl.Location = new System.Drawing.Point(102, 141);
        this.textBoxTextControl.Multiline = true;
        this.textBoxTextControl.Name = "textBoxTextControl";
        this.textBoxTextControl.ScrollBars = System.Windows.Forms.ScrollBars.Vertical;
        this.textBoxTextControl.Size = new System.Drawing.Size(178, 63);
        this.textBoxTextControl.TabIndex = 107;
        this.textBoxTextControl.Visible = false;
        //
        // label1
        //
        this.label1.AutoSize = true;
        this.label1.Location = new System.Drawing.Point(28, 144);
        this.label1.Name = "label1";
        this.label1.Size = new System.Drawing.Size(68, 13);
        this.label1.TabIndex = 108;
        this.label1.Text = "Multi line text";
        this.label1.Visible = false;
        //
        // comboType
        //
        this.comboType.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList;
        this.comboType.FormattingEnabled = true;
        this.comboType.Location = new System.Drawing.Point(186, 68);
        this.comboType.Name = "comboType";
        this.comboType.Size = new System.Drawing.Size(177, 21);
        this.comboType.TabIndex = 0;
        this.comboType.SelectedIndexChanged += new System.EventHandler(this.comboType_SelectedIndexChanged);
        //
        // textBoxControlLabel
        //
        this.textBoxControlLabel.Location = new System.Drawing.Point(186, 42);
        this.textBoxControlLabel.Name = "textBoxControlLabel";
        this.textBoxControlLabel.Size = new System.Drawing.Size(177, 20);
        this.textBoxControlLabel.TabIndex = 2;
        //
        // textBoxControlDescript
        //
        this.textBoxControlDescript.Location = new System.Drawing.Point(186, 14);
        this.textBoxControlDescript.Name = "textBoxControlDescript";
        this.textBoxControlDescript.Size = new System.Drawing.Size(177, 20);
        this.textBoxControlDescript.TabIndex = 1;
        //
        // labelControlType
        //
        this.labelControlType.Location = new System.Drawing.Point(123, 68);
        this.labelControlType.Name = "labelControlType";
        this.labelControlType.Size = new System.Drawing.Size(61, 18);
        this.labelControlType.TabIndex = 115;
        this.labelControlType.Text = "Type";
        this.labelControlType.TextAlign = System.Drawing.ContentAlignment.MiddleRight;
        //
        // labelControlLabel
        //
        this.labelControlLabel.Location = new System.Drawing.Point(73, 42);
        this.labelControlLabel.Name = "labelControlLabel";
        this.labelControlLabel.Size = new System.Drawing.Size(111, 18);
        this.labelControlLabel.TabIndex = 114;
        this.labelControlLabel.Text = "Prompt text";
        this.labelControlLabel.TextAlign = System.Drawing.ContentAlignment.MiddleRight;
        //
        // labelControlName
        //
        this.labelControlName.Location = new System.Drawing.Point(70, 14);
        this.labelControlName.Name = "labelControlName";
        this.labelControlName.Size = new System.Drawing.Size(114, 18);
        this.labelControlName.TabIndex = 113;
        this.labelControlName.Text = "Description";
        this.labelControlName.TextAlign = System.Drawing.ContentAlignment.MiddleRight;
        //
        // textOptions
        //
        this.textOptions.AcceptsReturn = true;
        this.textOptions.HideSelection = false;
        this.textOptions.Location = new System.Drawing.Point(186, 95);
        this.textOptions.Multiline = true;
        this.textOptions.Name = "textOptions";
        this.textOptions.ScrollBars = System.Windows.Forms.ScrollBars.Vertical;
        this.textOptions.Size = new System.Drawing.Size(364, 352);
        this.textOptions.TabIndex = 4;
        //
        // labelResponses
        //
        this.labelResponses.Location = new System.Drawing.Point(3, 95);
        this.labelResponses.Name = "labelResponses";
        this.labelResponses.Size = new System.Drawing.Size(182, 105);
        this.labelResponses.TabIndex = 123;
        this.labelResponses.Text = "Possible responses";
        this.labelResponses.TextAlign = System.Drawing.ContentAlignment.TopRight;
        //
        // butDelete
        //
        this.butDelete.setAdjustImageLocation(new System.Drawing.Point(0, 0));
        this.butDelete.Anchor = ((System.Windows.Forms.AnchorStyles)((System.Windows.Forms.AnchorStyles.Bottom | System.Windows.Forms.AnchorStyles.Right)));
        this.butDelete.setAutosize(true);
        this.butDelete.setBtnShape(OpenDental.UI.enumType.BtnShape.Rectangle);
        this.butDelete.setBtnStyle(OpenDental.UI.enumType.XPStyle.Silver);
        this.butDelete.setCornerRadius(4F);
        this.butDelete.Image = Resources.getdeleteX();
        this.butDelete.ImageAlign = System.Drawing.ContentAlignment.MiddleLeft;
        this.butDelete.Location = new System.Drawing.Point(12, 422);
        this.butDelete.Name = "butDelete";
        this.butDelete.Size = new System.Drawing.Size(76, 24);
        this.butDelete.TabIndex = 124;
        this.butDelete.Text = "Delete";
        this.butDelete.Click += new System.EventHandler(this.butDelete_Click);
        //
        // butCancel
        //
        this.butCancel.setAdjustImageLocation(new System.Drawing.Point(0, 0));
        this.butCancel.Anchor = ((System.Windows.Forms.AnchorStyles)((System.Windows.Forms.AnchorStyles.Bottom | System.Windows.Forms.AnchorStyles.Right)));
        this.butCancel.setAutosize(true);
        this.butCancel.setBtnShape(OpenDental.UI.enumType.BtnShape.Rectangle);
        this.butCancel.setBtnStyle(OpenDental.UI.enumType.XPStyle.Silver);
        this.butCancel.setCornerRadius(4F);
        this.butCancel.Location = new System.Drawing.Point(586, 422);
        this.butCancel.Name = "butCancel";
        this.butCancel.Size = new System.Drawing.Size(76, 24);
        this.butCancel.TabIndex = 9;
        this.butCancel.Text = "Cancel";
        this.butCancel.Click += new System.EventHandler(this.butCancel_Click);
        //
        // butOK
        //
        this.butOK.setAdjustImageLocation(new System.Drawing.Point(0, 0));
        this.butOK.Anchor = ((System.Windows.Forms.AnchorStyles)((System.Windows.Forms.AnchorStyles.Bottom | System.Windows.Forms.AnchorStyles.Right)));
        this.butOK.setAutosize(true);
        this.butOK.setBtnShape(OpenDental.UI.enumType.BtnShape.Rectangle);
        this.butOK.setBtnStyle(OpenDental.UI.enumType.XPStyle.Silver);
        this.butOK.setCornerRadius(4F);
        this.butOK.Location = new System.Drawing.Point(586, 391);
        this.butOK.Name = "butOK";
        this.butOK.Size = new System.Drawing.Size(76, 24);
        this.butOK.TabIndex = 8;
        this.butOK.Text = "OK";
        this.butOK.Click += new System.EventHandler(this.butOK_Click);
        //
        // butAddOptionControl
        //
        this.butAddOptionControl.setAdjustImageLocation(new System.Drawing.Point(0, 0));
        this.butAddOptionControl.setAutosize(true);
        this.butAddOptionControl.setBtnShape(OpenDental.UI.enumType.BtnShape.Rectangle);
        this.butAddOptionControl.setBtnStyle(OpenDental.UI.enumType.XPStyle.Silver);
        this.butAddOptionControl.setCornerRadius(4F);
        this.butAddOptionControl.Location = new System.Drawing.Point(286, 247);
        this.butAddOptionControl.Name = "butAddOptionControl";
        this.butAddOptionControl.Size = new System.Drawing.Size(91, 25);
        this.butAddOptionControl.TabIndex = 98;
        this.butAddOptionControl.Text = "Add";
        this.butAddOptionControl.Visible = false;
        //
        // butCreate
        //
        this.butCreate.setAdjustImageLocation(new System.Drawing.Point(0, 0));
        this.butCreate.setAutosize(true);
        this.butCreate.setBtnShape(OpenDental.UI.enumType.BtnShape.Rectangle);
        this.butCreate.setBtnStyle(OpenDental.UI.enumType.XPStyle.Silver);
        this.butCreate.setCornerRadius(4F);
        this.butCreate.Location = new System.Drawing.Point(351, 344);
        this.butCreate.Name = "butCreate";
        this.butCreate.Size = new System.Drawing.Size(91, 25);
        this.butCreate.TabIndex = 101;
        this.butCreate.Text = "Create";
        //
        // butDown
        //
        this.butDown.setAdjustImageLocation(new System.Drawing.Point(1, 0));
        this.butDown.setAutosize(true);
        this.butDown.setBtnShape(OpenDental.UI.enumType.BtnShape.Rectangle);
        this.butDown.setBtnStyle(OpenDental.UI.enumType.XPStyle.Silver);
        this.butDown.setCornerRadius(4F);
        this.butDown.Image = Resources.getdown();
        this.butDown.Location = new System.Drawing.Point(582, 95);
        this.butDown.Name = "butDown";
        this.butDown.Size = new System.Drawing.Size(25, 24);
        this.butDown.TabIndex = 126;
        this.butDown.Click += new System.EventHandler(this.butDown_Click);
        //
        // butUp
        //
        this.butUp.setAdjustImageLocation(new System.Drawing.Point(1, 0));
        this.butUp.setAutosize(true);
        this.butUp.setBtnShape(OpenDental.UI.enumType.BtnShape.Rectangle);
        this.butUp.setBtnStyle(OpenDental.UI.enumType.XPStyle.Silver);
        this.butUp.setCornerRadius(4F);
        this.butUp.Image = Resources.getup();
        this.butUp.Location = new System.Drawing.Point(556, 95);
        this.butUp.Name = "butUp";
        this.butUp.Size = new System.Drawing.Size(25, 24);
        this.butUp.TabIndex = 125;
        this.butUp.Click += new System.EventHandler(this.butUp_Click);
        //
        // FormAutoNoteControlEdit
        //
        this.ClientSize = new System.Drawing.Size(674, 458);
        this.Controls.Add(this.butDown);
        this.Controls.Add(this.butUp);
        this.Controls.Add(this.butDelete);
        this.Controls.Add(this.labelResponses);
        this.Controls.Add(this.textOptions);
        this.Controls.Add(this.labelControlType);
        this.Controls.Add(this.labelControlLabel);
        this.Controls.Add(this.labelControlName);
        this.Controls.Add(this.comboType);
        this.Controls.Add(this.butCancel);
        this.Controls.Add(this.butOK);
        this.Controls.Add(this.textBoxControlLabel);
        this.Controls.Add(this.textBoxControlDescript);
        this.Icon = ((System.Drawing.Icon)(resources.GetObject("$this.Icon")));
        this.MaximizeBox = false;
        this.MinimizeBox = false;
        this.Name = "FormAutoNoteControlEdit";
        this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
        this.Text = "Auto Note Prompt Edit";
        this.Load += new System.EventHandler(this.FormAutoNoteControlEdit_Load);
        this.ResumeLayout(false);
        this.PerformLayout();
    }

    private System.Windows.Forms.Label labelOptions = new System.Windows.Forms.Label();
    private OpenDental.UI.Button butAddOptionControl;
    private System.Windows.Forms.TextBox textBoxOptionsControl = new System.Windows.Forms.TextBox();
    private System.Windows.Forms.Label labelLabel = new System.Windows.Forms.Label();
    private System.Windows.Forms.Label labelName = new System.Windows.Forms.Label();
    private System.Windows.Forms.ListBox listBoxOptionsControls = new System.Windows.Forms.ListBox();
    private System.Windows.Forms.TextBox textBoxLabelControl = new System.Windows.Forms.TextBox();
    private System.Windows.Forms.TextBox textBoxDescriptControl = new System.Windows.Forms.TextBox();
    private OpenDental.UI.Button butCreate;
    private System.Windows.Forms.Label labelType = new System.Windows.Forms.Label();
    private System.Windows.Forms.Label labeldouble = new System.Windows.Forms.Label();
    private System.Windows.Forms.Label label = new System.Windows.Forms.Label();
    private System.Windows.Forms.TextBox textBoxTextControl = new System.Windows.Forms.TextBox();
    private System.Windows.Forms.Label label1 = new System.Windows.Forms.Label();
    private System.Windows.Forms.ComboBox comboType = new System.Windows.Forms.ComboBox();
    private OpenDental.UI.Button butCancel;
    private OpenDental.UI.Button butOK;
    private System.Windows.Forms.TextBox textBoxControlLabel = new System.Windows.Forms.TextBox();
    private System.Windows.Forms.TextBox textBoxControlDescript = new System.Windows.Forms.TextBox();
    private System.Windows.Forms.Label labelControlType = new System.Windows.Forms.Label();
    private System.Windows.Forms.Label labelControlLabel = new System.Windows.Forms.Label();
    private System.Windows.Forms.Label labelControlName = new System.Windows.Forms.Label();
    private System.Windows.Forms.TextBox textOptions = new System.Windows.Forms.TextBox();
    private System.Windows.Forms.Label labelResponses = new System.Windows.Forms.Label();
    private OpenDental.UI.Button butDelete;
    private OpenDental.UI.Button butDown;
    private OpenDental.UI.Button butUp;
}


