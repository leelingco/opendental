...
public class FormSheetFieldImage  extends Form 
{
    ...
    */
    ...
    public FormSheetFieldImage() throws Exception {
        initializeComponent();
        Lan.F(this);
    }

    private void formSheetFieldImage_Load(Object sender, EventArgs e) throws Exception {
        if (IsReadOnly)
        {
            butOK.Enabled = false;
    ...
    private void fillCombo() throws Exception {
        if (PrefC.getAtoZfolderUsed())
        {
            ...
            for (int i = 0;i < files.Length;i++)
            {
                ...
                if (files[i].EndsWith("db") || files[i].EndsWith("doc") || files[i].EndsWith("pdf"))
                {
                    continue;
                }
    ...
    private void fillImage() throws Exception {
        ...
        else if (StringSupport.equals(comboFieldName.Text, "Patient Info.gif"))
        {
            pictureBox.Image = Resources.getPatient_Info();
            textWidth2.Text = pictureBox.Image.Width.ToString();
            textHeight2.Text = pictureBox.Image.Height.ToString();
    ...
    private void butOK_Click(Object sender, EventArgs e) throws Exception {
        ...
        try
        {
            //catch valid files that are not valid images.
        ...
        if (!File.Exists(textFullPath.Text) && !StringSupport.equals(comboFieldName.Text, "Patient Info.gif"))
        {
