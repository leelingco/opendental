...
* 
*/
public class FormRpAdjSheet  extends System.Windows.Forms.Form 
{
    ...
    * 
    */
    public FormRpAdjSheet() throws Exception {
        initializeComponent();
        Lan.f(this);
    }
    ...
    */
    ...
    private void initializeComponent() throws Exception {
        ...
        System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(FormRpAdjSheet.class);
        ...
        // butTypeAll
        //
        butTypeAll.setAdjustImageLocation(new System.Drawing.Point(0, 0));
        butTypeAll.setAutosize(true);
        butTypeAll.setBtnShape(OpenDental.UI.enumType.BtnShape.Rectangle);
        butTypeAll.setBtnStyle(OpenDental.UI.enumType.XPStyle.Silver);
        butTypeAll.setCornerRadius(4F);
    ...
    private void formDailyAdjustment_Load(Object sender, System.EventArgs e) throws Exception {
        ...
        {
            listType.Items.Add(DefC.getShort()[((Enum)DefCat.AdjTypes).ordinal()][i].ItemName);
    ...
    private void butTypeAll_Click(Object sender, EventArgs e) throws Exception {
    ...
    private void butOK_Click(Object sender, System.EventArgs e) throws Exception {
        ...
        for (int i = 0;i < listType.SelectedIndices.Count;i++)
        {
            ...
            whereType += "adjustment.AdjType = '" + POut.Long(DefC.getShort()[((Enum)DefCat.AdjTypes).ordinal()][listType.SelectedIndices[i]].DefNum) + "' ";
        }
        whereType += ")";
        ReportSimpleGrid report = new ReportSimpleGrid();
        report.Query = "SELECT adjustment.AdjDate," + DbHelper.concat("patient.LName","', '","patient.FName","', '","patient.MiddleI") + "," + "adjustment.ProvNum,adjustment.ClinicNum," + "definition.ItemName,adjustment.AdjNote,adjustment.AdjAmt FROM " + "adjustment,patient,definition WHERE adjustment.AdjType=definition.DefNum " + "AND patient.PatNum=adjustment.PatNum " + whereProv + whereClin + "AND " + whereType + " " + "AND adjustment.AdjDate >= " + POut.Date(date1.SelectionStart) + " " + "AND adjustment.AdjDate <= " + POut.Date(date2.SelectionStart);
        ...
        for (int i = 0;i < table.Rows.Count;i++)
        {
            ...
            //name
            row[2] = Providers.GetAbbr(PIn.Long(table.Rows[i][2].ToString()));
            //prov
            colI = 3;
            if (!PrefC.getBool(PrefName.EasyNoClinics))
            {
                row[colI] = Clinics.GetDesc(PIn.Long(table.Rows[i][3].ToString()));
                //clinic
                colI++;
            }
             
            row[colI] = table.Rows[i][4].ToString();
            //Type
            colI++;
            row[colI] = table.Rows[i][5].ToString();
            //Note
            colI++;
            dec = PIn.Decimal(table.Rows[i][6].ToString());
            //Amount
