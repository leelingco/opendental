...
* 
*/
public class FormRpPaySheet  extends System.Windows.Forms.Form 
{
    ...
    private RadioButton radioPatient = new RadioButton();
    private RadioButton radioCheck = new RadioButton();
    private CheckBox checkAllTypes = new CheckBox();
    private ListBox listTypes = new ListBox();
    private CheckBox checkIns = new CheckBox();
    ...
    * 
    */
    public FormRpPaySheet() throws Exception {
        initializeComponent();
        Lan.f(this);
    }
    ...
    */
    private void initializeComponent() throws Exception {
        System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(FormRpPaySheet.class);
    ...
    private void formPaymentSheet_Load(Object sender, System.EventArgs e) throws Exception {
        ...
        {
            listTypes.Items.Add(DefC.getShort()[((Enum)DefCat.PaymentTypes).ordinal()][i].ItemName);
        }
    ...
    private void checkAllTypes_Click(Object sender, EventArgs e) throws Exception {
    ...
    private void butOK_Click(Object sender, System.EventArgs e) throws Exception {
        ...
        String queryIns = "SELECT CONVERT(" + DbHelper.dateFormatColumn("claimproc.DateCP","%c/%d/%Y") + ",CHAR(25)) DateCP,MAX(" + DbHelper.concat("patient.LName","', '","patient.FName","' '","patient.MiddleI") + ") lfname,\r\n" + 
        ...
        "WHERE (claimproc.Status=1 OR claimproc.Status=4) " + whereProv + whereClin + "AND claimpayment.CheckDate >= " + POut.Date(date1.SelectionStart) + " " + "AND claimpayment.CheckDate <= " + POut.Date(date2.SelectionStart) + " " + "GROUP BY CONVERT(" + DbHelper.dateFormatColumn("claimproc.DateCP","%c/%d/%Y") + ",CHAR(25)),\r\n" + 
        ...
        if (!checkAllClin.Checked)
        {
            for (int i = 0;i < listClin.SelectedIndices.Count;i++)
            {
                ...
                {
                    whereClin += "payment.ClinicNum = " + POut.Long(Clinics.getList()[listClin.SelectedIndices[i] - 1].ClinicNum) + " ";
                } 
            }
            whereClin += ") ";
        }
         
        String queryPat = "SELECT CONVERT(" + DbHelper.dateFormatColumn("payment.PayDate","%c/%d/%Y") + ",CHAR(25)) AS DatePay,MAX(" + DbHelper.concat("patient.LName","', '","patient.FName","' '","patient.MiddleI") + ") AS lfname,\r\n" + 
        ...
        "WHERE 1 " + whereProv + whereClin + "AND paysplit.DatePay >= " + POut.Date(date1.SelectionStart) + " " + "AND paysplit.DatePay <= " + POut.Date(date2.SelectionStart) + " ";
        if (listTypes.SelectedIndices.Count > 0)
        {
            ...
            for (int i = 0;i < listTypes.SelectedIndices.Count;i++)
            {
                ...
                queryPat += "payment.PayType = " + POut.Long(DefC.getShort()[((Enum)DefCat.PaymentTypes).ordinal()][listTypes.SelectedIndices[i]].DefNum) + " ";
            }
            queryPat += ") ";
        }
         
        queryPat += "GROUP BY " + "payment.PayNum,payment.PayDate,provider.ProvNum,payment.ClinicNum" + ",provider.Abbr,clinic.Description,payment.CheckNum,definition.ItemName";
        if (radioPatient.Checked)
        {
        ...
        FormR.setSourceRdlString(Resources.getPaymentsRDL());
        FormR.getRdlReport().getDataSets().get___idx("Data").setData(tableIns);
        FormR.getRdlReport().getDataSets().get___idx("DataPatPay").setData(tablePat);
        FormR.getRdlReport().getDataSets().get___idx("DataPracticeTitle").setData(tablePref);
        FormR.ShowDialog();
