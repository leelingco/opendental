...
*/
public class FormTransactionEdit  extends System.Windows.Forms.Form 
{
    ...
    */
    ...
    private List<JournalEntry> JournalListOld = new List<JournalEntry>();
    private Label label3 = new Label();
    private TextBox textDebit = new TextBox();
    private TextBox textCredit = new TextBox();
    private CheckBox checkMemoSame = new CheckBox();
    private OpenDental.UI.Button butAdd;
    private CheckBox checkSimple = new CheckBox();
    private Label label5 = new Label();
    private Label label4 = new Label();
    private OpenDental.ValidDouble textAmount;
    private Label label6 = new Label();
    private Panel panelSimple = new Panel();
    private Panel panelCompound = new Panel();
    ...
    /**
    * The account where the edit originated from.  This affects how the simple version gets displayed, and the signs on debit and credit.
    */
    private Account AccountOfOrigin;
    /**
    * When in simple mode, this is the 'other' account, not the one of origin.  It can be null.
    */
    private Account AccountPicked;
    private GroupBox groupBox1 = new GroupBox();
    private TextBox textSourceDeposit = new TextBox();
    private Label label9 = new Label();
    private Label label8 = new Label();
    private TextBox textSourcePay = new TextBox();
    private OpenDental.UI.Button butAttachPay;
    private Label labelReconcile = new Label();
    private TextBox textReconcile = new TextBox();
    /**
    * Just used for security.
    */
    public boolean IsNew = new boolean();
    /**
    * 
    */
    public FormTransactionEdit(long transNum, long accountNum) throws Exception {
        //
    ...
    */
    private void initializeComponent() throws Exception {
        System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(FormTransactionEdit.class);
        ...
        this.gridMain.setTitle("Splits");
        this.gridMain.setTranslationName("TableTransSplits");
        ...
        // butAttachPay
        //
        this.butAttachPay.setAdjustImageLocation(new System.Drawing.Point(0, 0));
        this.butAttachPay.setAutosize(true);
        this.butAttachPay.setBtnShape(OpenDental.UI.enumType.BtnShape.Rectangle);
        this.butAttachPay.setBtnStyle(OpenDental.UI.enumType.XPStyle.Silver);
        this.butAttachPay.setCornerRadius(4F);
        ...
        // butAttachDep
        //
        this.butAttachDep.setAdjustImageLocation(new System.Drawing.Point(0, 0));
        this.butAttachDep.setAutosize(true);
        this.butAttachDep.setBtnShape(OpenDental.UI.enumType.BtnShape.Rectangle);
        this.butAttachDep.setBtnStyle(OpenDental.UI.enumType.XPStyle.Silver);
        this.butAttachDep.setCornerRadius(4F);
    ...
    private void formTransactionEdit_Load(Object sender, EventArgs e) throws Exception {
        JournalList = JournalEntries.getForTrans(TransCur.TransactionNum);
        //Count might be 0
        if (IsNew)
        {
        ...
        if (AccountOfOrigin == null)
        {
            ...
            //don't allow user to switch back to simple view
            fillCompound();
        }
        ...
        else if (JournalList.Count > 2)
        {
            //compound
            checkSimple.Checked = false;
            fillCompound();
        }
        else //so count must be 0 or 2
        if (JournalList.Count == 2 && !StringSupport.equals(((JournalEntry)JournalList[0]).Memo, ((JournalEntry)JournalList[1]).Memo))
        {
        ...
        else
        {
            ...
            textSourceDeposit.Text = dep.DateDeposit.ToShortDateString() + "  " + dep.Amount.ToString("c");
        ...
        else
        {
            ...
            textSourcePay.Text = Patients.getPat(pay.PatNum).getNameFL() + " " + pay.PayDate.ToShortDateString() + " " + pay.PayAmt.ToString("c");
            butAttachPay.Text = Lan.g(this,"Detach");
        } 
    }

    /**
    * Used when compound view (3 or more journal entries).  Account nums allowed to be 0.
    */
    private void fillCompound() throws Exception {
        panelSimple.Visible = false;
        panelCompound.Visible = true;
        boolean memoSame = true;
        String memo = "";
        double debits = 0;
        double credits = 0;
        ...
        for (int i = 0;i < JournalList.Count;i++)
        {
            ...
            row.getCells().add(Accounts.getDescript(((JournalEntry)JournalList[i]).AccountNum));
            ...
            {
                row.getCells().Add(((JournalEntry)JournalList[i]).DebitAmt.ToString("n"));
            } 
            ...
            {
                row.getCells().Add(((JournalEntry)JournalList[i]).CreditAmt.ToString("n"));
            } 
            row.getCells().add(((JournalEntry)JournalList[i]).Memo);
            ...
            {
                if (!StringSupport.equals(memo, ((JournalEntry)JournalList[i]).Memo))
                {
                    memoSame = false;
                }
    ...
    /**
    * Used when switching simple view (0, 1, or 2 journal entries with identical notes).  This function fills in the correct fields in the simple view, and then deletes any journal entries.  2 journal entries will be recreated upon leaving for compound view by CreateTwoEntries.  This is only called once upon going to simple view.  It's not called repeatedly as a way of refreshing the screen.
    */
    private void fillSimple() throws Exception {
        ...
        else
        {
            ...
            if (!StringSupport.equals(journalCur.CheckNumber, ""))
            {
                textCheckNumber.Text = journalCur.CheckNumber;
            }
             
            if (!StringSupport.equals(journalOther.CheckNumber, ""))
            {
    ...
    private void checkSimple_Click(Object sender, EventArgs e) throws Exception {
        ...
        else
        {
            ...
            createTwoEntries();
            //never fails.  Just does the best it can
            fillCompound();
        } 
    }
    ...
    private void gridMain_CellDoubleClick(Object sender, OpenDental.UI.ODGridClickEventArgs e) throws Exception {
        FormJournalEntryEdit FormJ = new FormJournalEntryEdit();
        FormJ.EntryCur = (JournalEntry)JournalList[e.getRow()];
        FormJ.ShowDialog();
        if (FormJ.EntryCur == null)
        {
            //user deleted
            JournalList.RemoveAt(e.getRow());
        }
    ...
    private void butAttachDep_Click(Object sender, EventArgs e) throws Exception {
        if (TransCur.DepositNum == 0)
        {
            //trying to attach
            ...
            textSourceDeposit.Text = FormD.SelectedDeposit.DateDeposit.ToShortDateString() + "  " + FormD.SelectedDeposit.Amount.ToString("c");
            butAttachDep.Text = Lan.g(this,"Detach");
        }
        else
        {
    ...
    private void butAttachPay_Click(Object sender, EventArgs e) throws Exception {
        if (TransCur.PayNum == 0)
        {
        }
        else
    ...
    /**
    * When leaving simple view, this function takes the info from the screen and creates two journal entries.  Never fails.  One of the journal entries might accountNum=0, and they might both have amounts of 0.  This is all in preparation for either saving or for viewing as compound.
    */
    private void createTwoEntries() throws Exception {
        JournalEntry entry;
        //first, for account of origin
        entry = new JournalEntry();
        entry.TransactionNum = TransCur.TransactionNum;
        if (StringSupport.equals(textDate.Text, "") || !StringSupport.equals(textDate.errorProvider1.GetError(textDate), ""))
        {
            entry.DateDisplayed = DateTime.Today;
        }
        else
    ...
    private void butDelete_Click(Object sender, EventArgs e) throws Exception {
        ...
        if (!IsNew)
        {
            ...
            JournalList = JournalEntries.getForTrans(TransCur.TransactionNum);
            //because they were cleared when laying out
            securityentry = Lan.g(this,"Deleted: ") + ((JournalEntry)JournalList[0]).DateDisplayed.ToShortDateString() + " ";
            double tot = 0;
            for (int i = 0;i < JournalList.Count;i++)
            {
        ...
        try
        {
            //in case it fails, we don't want to leave this list around.
            Transactions.delete(TransCur);
        }
        catch (ApplicationException ex)
        {
            JournalList = JournalEntries.getForTrans(TransCur.TransactionNum);
            //Refreshes list so that the journal entries are not deleted by the update later.
    ...
    private void butOK_Click(Object sender, System.EventArgs e) throws Exception {
        ...
        for (int i = 0;i < JournalList.Count;i++)
        {
            ...
            for (int j = 0;j < JournalList.Count;j++)
            {
                ...
                if (!StringSupport.equals(splits, ""))
                {
                    splits += "\r\n";
                }
                ...
                //don't show the amount if there is only two splits, because the amount is the same.
        ...
        //catch{
        //}
        Transactions.update(TransCur);
        //this catches DepositNum, the only user-editable field.
        double tot = 0;
        for (int i = 0;i < JournalList.Count;i++)
        {
            tot += ((JournalEntry)JournalList[i]).DebitAmt;
        ...
        if (IsNew)
        {
            SecurityLogs.MakeLogEntry(Permissions.AccountingCreate, 0, date.ToShortDateString() + " " + AccountOfOrigin.Description + " " + tot.ToString("c"));
        }
        else
        {
