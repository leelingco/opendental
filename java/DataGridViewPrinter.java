...
{
    private DataGridView TheDataGridView = new DataGridView();
    // The DataGridView Control which will be printed
    private PrintDocument ThePrintDocument = new PrintDocument();
    // The PrintDocument to be used for printing
    private boolean IsCenterOnPage = new boolean();
    // Determine if the report will be printed in the Top-Center of the page
    private boolean IsWithTitle = new boolean();
    // Determine if the page contain title text
    private String TheTitleText = new String();
    // The title text to be printed in each page (if IsWithTitle is set to true)
    private Font TheTitleFont = new Font();
    // The font to be used with the title text (if IsWithTitle is set to true)
    private Color TheTitleColor = new Color();
    // The color to be used with the title text (if IsWithTitle is set to true)
    private boolean IsWithPaging = new boolean();
    // Determine if paging is used
    static int CurrentRow = new int();
    // A static parameter that keep track on which Row (in the DataGridView control) that should be printed
    static int PageNumber = new int();
    private int PageWidth = new int();
    private int PageHeight = new int();
    private int LeftMargin = new int();
    private int TopMargin = new int();
    private int RightMargin = new int();
    private int BottomMargin = new int();
    ...
    private float RowHeaderHeight = new float();
    private List<float> RowsHeight = new List<float>();
    private List<float> ColumnsWidth = new List<float>();
    private float TheDataGridViewWidth = new float();
    ...
    private List<int[]> mColumnPoints = new List<int[]>();
    private List<float> mColumnPointsWidth = new List<float>();
    private int mColumnPoint = new int();
    // The class constructor
    public DataGridViewPrinter(DataGridView aDataGridView, PrintDocument aPrintDocument, boolean CenterOnPage, boolean WithTitle, String aTitleText, Font aTitleFont, Color aTitleColor, boolean WithPaging) throws Exception {
    ...
    private void calculate(Graphics g) throws Exception {
        if (PageNumber == 0)
        {
            // Just calculate once
            SizeF tmpSize = new SizeF();
            Font tmpFont = new Font();
            float tmpWidth = new float();
    ...
    private boolean drawRows(Graphics g) throws Exception {
        // Setting the LinePen that will be used to draw lines and rectangles (derived from the GridColor property of the DataGridView control)
        Pen TheLinePen = new Pen(TheDataGridView.GridColor, 1);
        // The style paramters that will be used to print each cell
        Font RowFont = new Font();
        Color RowForeColor = new Color();
        Color RowBackColor = new Color();
        SolidBrush RowForeBrush = new SolidBrush();
        SolidBrush RowBackBrush = new SolidBrush();
        SolidBrush RowAlternatingBackBrush = new SolidBrush();
    ...
    public boolean drawDataGridView(Graphics g) throws Exception {
        try
        {
            calculate(g);
            drawHeader(g);
            boolean bContinue = drawRows(g);
            return bContinue;
        }
        catch (Exception ex)
        {
