...
import OpenDentBusiness.CqmItemAbbreviation;
...
/**
* Used in Ehr quality measures.
*/
public class QualityMeasures   
{
    private static String _elapsedtimetext = new String();
    ...
    */
    ...
    /**
    * OID: 2.16.840.1.113883.6.88Set each time GenerateQrda_xml() is called. Used by helper functions to avoid sending the provider as a parameter to each helper function.
    */
    private static Provider _provOutQrda = null;
    /**
    * Instantiated each time GenerateQrda_xml() is called. Used by helper functions to avoid sending the writer as a parameter to each helper function.
    */
    private static XmlWriter _w = null;
    /**
    * Instantiated each time GenerateQrda_xml() is called. Used by helper functions to avoid sending the writer as a parameter to each helper function.  Second writer to generate patient data entries in text form and xml form simultaneously.  The helper functions will take a bool to determine which variable to modify.
    */
    private static XmlWriter _x = null;
    /**
    * This global variable tells the helper functions which of the above XmlWriters to use.  If true, then the _w writer is modified, false=_x.
    */
    private static boolean _isWriterW = true;
    //These are the internal OID roots used for "id" entries in GenerateQrda_xml().
    private static String _strOIDInternalRoot = new String();
    private static String _strOIDInternalCQMRoot = new String();
    private static String _strOIDInternalPatRoot = new String();
    private static String _strOIDInternalProvRoot = new String();
    /**
    * Instantiated each time GenerateQrda_xml() is called. Used to generate unique "id" element "root" attribute identifiers. The Ids in this list are random GUIDs which are 36 characters in length.  They are used to uniquely identify the QRDA documents.
    */
    private static HashSet<String> _hashQrdaGuids = new HashSet<String>();
    private static List<int> _listExtraPopIndxs = new List<int>();
    /**
    * Generates a list of all the quality measures for 2011.  Performs all calculations and manipulations.  Returns list for viewing/output.
    */
    public static List<QualityMeasure> getAll(DateTime dateStart, DateTime dateEnd, long provNum) throws Exception {
        if (RemotingClient.RemotingRole == RemotingRole.ClientWeb)
        {
            return Meth.<List<QualityMeasure>>GetObject(MethodBase.GetCurrentMethod(), dateStart, dateEnd, provNum);
        }
         
        List<QualityMeasure> list = new List<QualityMeasure>();
        //add one of each type
        QualityMeasure measure;
        for (int i = 0;i < Enum.GetValues(QualityType.class).Length;i++)
        {
            measure = new QualityMeasure();
            measure.Type = QualityType.values()[i];
    ...
    /**
    * Generates a list of all the quality measures for 2014.  Performs all calculations and manipulations.  Returns list for viewing/output.
    */
    public static List<QualityMeasure> getAll2014(DateTime dateStart, DateTime dateEnd, long provNum) throws Exception {
        if (RemotingClient.RemotingRole == RemotingRole.ClientWeb)
        {
            return Meth.<List<QualityMeasure>>GetObject(MethodBase.GetCurrentMethod(), dateStart, dateEnd, provNum);
        }
        ...
        for (int i = 0;i < Enum.GetValues(QualityType2014.class).Length;i++)
        {
            measureCur = getEhrCqmData(QualityType2014.values()[i],dateStart,dateEnd,provNum);
            measureCur.Type2014 = QualityType2014.values()[i];
            ...
            {
                if (QualityType2014.values()[i] == QualityType2014.Influenza)
                {
                ...
                else if (QualityType2014.values()[i] == QualityType2014.MedicationsEntered)
                {
                ...
                if (QualityType2014.values()[i] == QualityType2014.MedicationsEntered)
                {
                ...
                {
                    measureCur.ReportingRate = Math.Round(((double)((measureCur.Numerator + measureCur.Exclusions + measureCur.Exceptions) * 100) / (double)(measureCur.Denominator)), 1, MidpointRounding.AwayFromZero);
                }
                ...
                {
                    measureCur.PerformanceRate = Math.Round(((double)(measureCur.Numerator * 100) / (double)(measureCur.Denominator - measureCur.Exclusions - measureCur.Exceptions)), 1, MidpointRounding.AwayFromZero);
                }
    ...
    private static String getId(QualityType qtype) throws Exception {
        switch(qtype)
        {
            case WeightOver65: 
                return "0421a";
            ...
            case TobaccoUse: 
                return "0028a";
            case TobaccoCessation: 
                return "0028b";
            case InfluenzaAdult: 
                return "0041";
            case WeightChild_1_1: 
                return "0024-1.1";
            case WeightChild_1_2: 
                return "0024-1.2";
            case WeightChild_1_3: 
                return "0024-1.3";
            case WeightChild_2_1: 
                return "0024-2.1";
            case WeightChild_2_2: 
                return "0024-2.2";
            case WeightChild_2_3: 
                return "0024-2.3";
            case WeightChild_3_1: 
                return "0024-3.1";
            case WeightChild_3_2: 
                return "0024-3.2";
            case WeightChild_3_3: 
                return "0024-3.3";
            case ImmunizeChild_1: 
                return "0038-1";
            case ImmunizeChild_2: 
                return "0038-2";
            case ImmunizeChild_3: 
                return "0038-3";
            case ImmunizeChild_4: 
                return "0038-4";
            case ImmunizeChild_5: 
                return "0038-5";
            case ImmunizeChild_6: 
                return "0038-6";
            case ImmunizeChild_7: 
                return "0038-7";
            case ImmunizeChild_8: 
                return "0038-8";
            case ImmunizeChild_9: 
                return "0038-9";
            case ImmunizeChild_10: 
                return "0038-10";
            case ImmunizeChild_11: 
                return "0038-11";
            case ImmunizeChild_12: 
                return "0038-12";
            case Pneumonia: 
                return "0043";
            case DiabetesBloodPressure: 
                return "0061";
            case BloodPressureManage: 
                return "0018";
            default: 
    ...
    private static String getId2014(QualityType2014 qtype) throws Exception {
        switch(qtype)
        {
            case MedicationsEntered: 
                return "68";
            case WeightOver65: 
                return "69AgeOver64";
            case WeightAdult: 
            ...
            case CariesPrevent: 
                return "74All";
            case CariesPrevent_1: 
                return "74Age0To5";
            case CariesPrevent_2: 
                return "74Age6To12";
            case CariesPrevent_3: 
                return "74Age13To20";
            case ChildCaries: 
                return "75";
            case Pneumonia: 
    ...
    /**
    * Used in reporting, and only for certain types.
    */
    public static String getPQRIMeasureNumber(QualityType qtype) throws Exception {
        switch(qtype)
        {
            case WeightOver65: 
                return "128";
    ...
    //"0043";
    //"0061";
    private static String getDescript(QualityType qtype) throws Exception {
        switch(qtype)
        {
            case WeightOver65: 
    ...
    private static String getDescript2014(QualityType2014 qtype) throws Exception {
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    public static DataTable getTable(QualityType qtype, DateTime dateStart, DateTime dateEnd, long provNum) throws Exception {
        if (RemotingClient.RemotingRole == RemotingRole.ClientWeb)
        {
        ...
        switch(qtype)
        {
            case WeightOver65: 
                ...
                Db.nonQ(command);
                //on the first pass, all we can obtain is the date of the visit
                //because we want to restrict to only results with procedurelog
                //complete
                command = "INSERT INTO tempehrquality (PatNum,LName,FName,DateVisit) SELECT patient.PatNum,LName,FName," + "MAX(ProcDate) " + "FROM patient " + "INNER JOIN procedurelog " + "ON Patient.PatNum=procedurelog.PatNum " + "AND procedurelog.ProcStatus=2 " + "AND procedurelog.ProvNum=" + POut.long(provNum) + " " + "AND procedurelog.ProcDate >= " + POut.date(dateStart) + " " + "AND procedurelog.ProcDate <= " + POut.date(dateEnd) + " " + "WHERE Birthdate > '1880-01-01' AND Birthdate <= " + POut.Date(DateTime.Today.AddYears(-65)) + " " + "GROUP BY patient.PatNum";
                //65 or older
                //there will frequently be multiple procedurelog events
                Db.nonQ(command);
                //now, find BMIs within 6 months of each visit date. No logic for picking one of multiple BMIs.
                command = "UPDATE tempehrquality,vitalsign " + "SET tempehrquality.Height=vitalsign.Height, " + "tempehrquality.Weight=vitalsign.Weight, " + "tempehrquality.HasFollowupPlan=vitalsign.HasFollowupPlan, " + "tempehrquality.IsIneligible=vitalsign.IsIneligible, " + "tempehrquality.Documentation=vitalsign.Documentation " + "WHERE tempehrquality.PatNum=vitalsign.PatNum " + "AND vitalsign.DateTaken <= tempehrquality.DateVisit " + "AND vitalsign.DateTaken >= DATE_SUB(tempehrquality.DateVisit,INTERVAL 6 MONTH)";
                //we could also easily get the BMI date if we wanted.
                Db.nonQ(command);
            ...
            case WeightAdult: 
                ...
                //complete
                //18+
                command = "INSERT INTO tempehrquality (PatNum,LName,FName,DateVisit) SELECT patient.PatNum,LName,FName," + "MAX(ProcDate) " + "FROM patient " + "INNER JOIN procedurelog " + "ON Patient.PatNum=procedurelog.PatNum " + "AND procedurelog.ProcStatus=2 " + "AND procedurelog.ProvNum=" + POut.long(provNum) + " " + "AND procedurelog.ProcDate >= " + POut.date(dateStart) + " " + "AND procedurelog.ProcDate <= " + POut.date(dateEnd) + " " + "WHERE Birthdate <= " + POut.Date(DateTime.Today.AddYears(-18)) + " " + "AND Birthdate > " + POut.Date(DateTime.Today.AddYears(-65)) + " " + "GROUP BY patient.PatNum";
                //less than 65
                //there will frequently be multiple procedurelog events
                Db.nonQ(command);
                //now, find BMIs within 6 months of each visit date. No logic for picking one of multiple BMIs.
            ...
            case Hypertension: 
                ...
                // most recent visit
                //complete
                //starts with 401 through 404
                //+"LEFT JOIN icd9 ON icd9.ICD9Num=disease.ICD9Num "
                //+"AND icd9.ICD9Code REGEXP '^40[1-4]' "//starts with 401 through 404
                command = "INSERT INTO tempehrquality (PatNum,LName,FName,DateVisit,VisitCount,Icd9Code) " + "SELECT patient.PatNum,LName,FName," + "MAX(ProcDate), " + "COUNT(DISTINCT ProcDate),diseasedef.ICD9Code " + "FROM patient " + "INNER JOIN procedurelog " + "ON Patient.PatNum=procedurelog.PatNum " + "AND procedurelog.ProcStatus=2 " + "AND procedurelog.ProvNum=" + POut.long(provNum) + " " + "LEFT JOIN disease ON disease.PatNum=patient.PatNum " + "AND disease.DiseaseDefNum IN (SELECT DiseaseDefNum FROM diseasedef WHERE ICD9Code REGEXP '^40[1-4]') " + "LEFT JOIN diseasedef ON diseasedef.DiseaseDefNum=disease.DiseaseDefNum " + "WHERE Birthdate <= " + POut.Date(DateTime.Today.AddYears(-18)) + " " + "GROUP BY patient.PatNum";
                //18+
                Db.nonQ(command);
                //now, find BMIs in measurement period.
                command = "UPDATE tempehrquality,vitalsign " + "SET tempehrquality.DateBpEntered=vitalsign.DateTaken " + "WHERE tempehrquality.PatNum=vitalsign.PatNum " + "AND vitalsign.BpSystolic != 0 " + "AND vitalsign.BpDiastolic != 0 " + "AND vitalsign.DateTaken >= " + POut.date(dateStart) + " " + "AND vitalsign.DateTaken <= " + POut.date(dateEnd);
                Db.nonQ(command);
                command = "SELECT * FROM tempehrquality";
                tableRaw = Db.getTable(command);
                command = "DROP TABLE IF EXISTS tempehrquality";
            ...
            case TobaccoUse: 
                ...
                // most recent visit
                //complete
                command = "INSERT INTO tempehrquality (PatNum,LName,FName,DateVisit,VisitCount) " + "SELECT patient.PatNum,LName,FName," + "MAX(ProcDate), " + "COUNT(DISTINCT ProcDate) " + "FROM patient " + "INNER JOIN procedurelog " + "ON Patient.PatNum=procedurelog.PatNum " + "AND procedurelog.ProcStatus=2 " + "AND procedurelog.ProvNum=" + POut.long(provNum) + " " + "WHERE Birthdate <= " + POut.Date(DateTime.Today.AddYears(-18)) + " " + "GROUP BY patient.PatNum";
                //18+
                Db.nonQ(command);
                //now, find most recent tobacco assessment date.  We will check later that it is within 2 years of last exam.
                //,ehrmeasureevent "
                command = "UPDATE tempehrquality " + "SET tempehrquality.DateAssessment=(SELECT MAX(DATE(ehrmeasureevent.DateTEvent)) " + "FROM ehrmeasureevent " + "WHERE tempehrquality.PatNum=ehrmeasureevent.PatNum " + "AND ehrmeasureevent.EventType=" + POut.int(((Enum)EhrMeasureEventType.TobaccoUseAssessed).ordinal()) + ")";
                Db.nonQ(command);
            ...
            case TobaccoCessation: 
                ...
                //complete
                //18+
                command = "INSERT INTO tempehrquality (PatNum,LName,FName,DateVisit) " + "SELECT patient.PatNum,LName,FName," + "MAX(ProcDate) " + "FROM patient " + "INNER JOIN procedurelog " + "ON Patient.PatNum=procedurelog.PatNum " + "AND procedurelog.ProcStatus=2 " + "AND procedurelog.ProvNum=" + POut.long(provNum) + " " + "WHERE Birthdate <= " + POut.Date(DateTime.Today.AddYears(-18)) + " " + "AND patient.SmokingSnoMed IN('" + POut.String(SmokingSnoMed._449868002.ToString().Substring(1)) + "','" + POut.String(SmokingSnoMed._428041000124106.ToString().Substring(1)) + "','" + POut.String(SmokingSnoMed._428061000124105.ToString().Substring(1)) + "','" + POut.String(SmokingSnoMed._428071000124103.ToString().Substring(1)) + "') " + "GROUP BY patient.PatNum";
                //CurrentEveryDay,CurrentSomeDay,LightSmoker,HeavySmoker
                Db.nonQ(command);
                ...
                command = "UPDATE tempehrquality " + "SET tempehrquality.DateCessation=(SELECT MAX(DATE(ehrmeasureevent.DateTEvent)) " + "FROM ehrmeasureevent " + "WHERE tempehrquality.PatNum=ehrmeasureevent.PatNum " + "AND ehrmeasureevent.EventType=" + POut.int(((Enum)EhrMeasureEventType.TobaccoCessation).ordinal()) + ")";
                Db.nonQ(command);
                ...
                command = "UPDATE tempehrquality " + "SET Documentation=(SELECT ehrmeasureevent.MoreInfo " + "FROM ehrmeasureevent " + "WHERE tempehrquality.PatNum=ehrmeasureevent.PatNum " + "AND ehrmeasureevent.EventType=" + POut.int(((Enum)EhrMeasureEventType.TobaccoCessation).ordinal()) + " " + "AND DATE(ehrmeasureevent.DateTEvent)=tempehrquality.DateCessation) " + "WHERE DateCessation > '1880-01-01'";
                Db.nonQ(command);
                command = "SELECT * FROM tempehrquality";
                tableRaw = Db.getTable(command);
                command = "DROP TABLE IF EXISTS tempehrquality";
            ...
            case InfluenzaAdult: 
                ...
                Db.nonQ(command);
                //complete
                command = "INSERT INTO tempehrquality (PatNum,LName,FName) SELECT patient.PatNum,LName,FName " + "FROM patient " + "INNER JOIN procedurelog " + "ON Patient.PatNum=procedurelog.PatNum " + "AND procedurelog.ProcStatus=2 " + "AND procedurelog.ProvNum=" + POut.long(provNum) + " " + "AND procedurelog.ProcDate >= " + POut.date(dateStart) + " " + "AND procedurelog.ProcDate <= " + POut.date(dateEnd) + " " + "WHERE Birthdate > '1880-01-01' AND Birthdate <= " + POut.Date(DateTime.Today.AddYears(-50)) + " " + "GROUP BY patient.PatNum";
                //50 or older
                Db.nonQ(command);
                //find most recent vaccine date
                command = "UPDATE tempehrquality " + "SET tempehrquality.DateVaccine=(SELECT MAX(DATE(vaccinepat.DateTimeStart)) " + "FROM vaccinepat,vaccinedef " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND vaccinedef.CVXCode IN('135','15'))";
                Db.nonQ(command);
                ...
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET Documentation=Note, " + "tempehrquality.NotGiven=vaccinepat.NotGiven " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND DATE(vaccinepat.DateTimeStart)=tempehrquality.DateVaccine " + "AND vaccinedef.CVXCode IN('135','15')";
                Db.nonQ(command);
                command = "SELECT * FROM tempehrquality";
                tableRaw = Db.getTable(command);
                command = "DROP TABLE IF EXISTS tempehrquality";
            ...
            case WeightChild_1_3: 
                ...
                //complete
                //2+
                command = "INSERT INTO tempehrquality (PatNum,LName,FName) SELECT patient.PatNum,LName,FName " + "FROM patient " + "INNER JOIN procedurelog " + "ON Patient.PatNum=procedurelog.PatNum " + "AND procedurelog.ProcStatus=2 " + "AND procedurelog.ProvNum=" + POut.long(provNum) + " " + "AND procedurelog.ProcDate >= " + POut.date(dateStart) + " " + "AND procedurelog.ProcDate <= " + POut.date(dateEnd) + " " + "WHERE Birthdate <= " + POut.Date(DateTime.Today.AddYears(-2)) + " " + "AND Birthdate > " + POut.Date(DateTime.Today.AddYears(-17)) + " " + "GROUP BY patient.PatNum";
                //less than 17
                //there will frequently be multiple procedurelog events
                Db.nonQ(command);
                //find any BMIs within the period that indicate pregnancy
                command = "UPDATE tempehrquality,vitalsign " + "SET tempehrquality.IsPregnant=1 " + "WHERE tempehrquality.PatNum=vitalsign.PatNum " + "AND vitalsign.DateTaken >= " + POut.date(dateStart) + " " + "AND vitalsign.DateTaken <= " + POut.date(dateEnd) + " " + "AND vitalsign.IsIneligible=1";
                Db.nonQ(command);
                //find any BMIs within the period with a valid BMI
                command = "UPDATE tempehrquality,vitalsign " + "SET tempehrquality.HasBMI=1 " + "WHERE tempehrquality.PatNum=vitalsign.PatNum " + "AND vitalsign.DateTaken >= " + POut.date(dateStart) + " " + "AND vitalsign.DateTaken <= " + POut.date(dateEnd) + " " + "AND vitalsign.Height > 0 " + "AND vitalsign.Weight > 0";
                Db.nonQ(command);
                //find any BMIs within the period that indicate ChildGotNutrition
                command = "UPDATE tempehrquality,vitalsign " + "SET tempehrquality.ChildGotNutrition=1 " + "WHERE tempehrquality.PatNum=vitalsign.PatNum " + "AND vitalsign.DateTaken >= " + POut.date(dateStart) + " " + "AND vitalsign.DateTaken <= " + POut.date(dateEnd) + " " + "AND vitalsign.ChildGotNutrition=1";
                Db.nonQ(command);
                //find any BMIs within the period that indicate ChildGotPhysCouns
                command = "UPDATE tempehrquality,vitalsign " + "SET tempehrquality.ChildGotPhysCouns=1 " + "WHERE tempehrquality.PatNum=vitalsign.PatNum " + "AND vitalsign.DateTaken >= " + POut.date(dateStart) + " " + "AND vitalsign.DateTaken <= " + POut.date(dateEnd) + " " + "AND vitalsign.ChildGotPhysCouns=1";
                Db.nonQ(command);
                command = "SELECT * FROM tempehrquality";
                tableRaw = Db.getTable(command);
                command = "DROP TABLE IF EXISTS tempehrquality";
            ...
            case WeightChild_2_3: 
                ...
                //complete
                //2+
                command = "INSERT INTO tempehrquality (PatNum,LName,FName) SELECT patient.PatNum,LName,FName " + "FROM patient " + "INNER JOIN procedurelog " + "ON Patient.PatNum=procedurelog.PatNum " + "AND procedurelog.ProcStatus=2 " + "AND procedurelog.ProvNum=" + POut.long(provNum) + " " + "AND procedurelog.ProcDate >= " + POut.date(dateStart) + " " + "AND procedurelog.ProcDate <= " + POut.date(dateEnd) + " " + "WHERE Birthdate <= " + POut.Date(DateTime.Today.AddYears(-2)) + " " + "AND Birthdate > " + POut.Date(DateTime.Today.AddYears(-11)) + " " + "GROUP BY patient.PatNum";
                //less than 11
                //there will frequently be multiple procedurelog events
                Db.nonQ(command);
                //find any BMIs within the period that indicate pregnancy
            ...
            case WeightChild_3_3: 
                ...
                //complete
                //11+
                command = "INSERT INTO tempehrquality (PatNum,LName,FName) SELECT patient.PatNum,LName,FName " + "FROM patient " + "INNER JOIN procedurelog " + "ON Patient.PatNum=procedurelog.PatNum " + "AND procedurelog.ProcStatus=2 " + "AND procedurelog.ProvNum=" + POut.long(provNum) + " " + "AND procedurelog.ProcDate >= " + POut.date(dateStart) + " " + "AND procedurelog.ProcDate <= " + POut.date(dateEnd) + " " + "WHERE Birthdate <= " + POut.Date(DateTime.Today.AddYears(-11)) + " " + "AND Birthdate > " + POut.Date(DateTime.Today.AddYears(-17)) + " " + "GROUP BY patient.PatNum";
                //less than 17
                //there will frequently be multiple procedurelog events
                Db.nonQ(command);
                //find any BMIs within the period that indicate pregnancy
            ...
            case ImmunizeChild_12: 
                ...
                Db.nonQ(command);
                //complete
                //second birthdate is in meas period
                command = "INSERT INTO tempehrquality (PatNum,LName,FName,Birthdate) SELECT patient.PatNum,LName,FName,Birthdate " + "FROM patient " + "INNER JOIN procedurelog " + "ON Patient.PatNum=procedurelog.PatNum " + "AND procedurelog.ProcStatus=2 " + "AND procedurelog.ProvNum=" + POut.long(provNum) + " " + "AND procedurelog.ProcDate >= " + POut.date(dateStart) + " " + "AND procedurelog.ProcDate <= " + POut.date(dateEnd) + " " + "WHERE DATE_ADD(Birthdate,INTERVAL 2 YEAR) >= " + POut.date(dateStart) + " " + "AND DATE_ADD(Birthdate,INTERVAL 2 YEAR) <= " + POut.date(dateEnd) + " " + "GROUP BY patient.PatNum";
                Db.nonQ(command);
                //Count1, DTaP
                command = "UPDATE tempehrquality " + "SET Count1=(SELECT COUNT(DISTINCT VaccinePatNum) FROM vaccinepat " + "LEFT JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=0 " + "AND DATE(DateTimeStart) >= DATE_ADD(tempehrquality.Birthdate,INTERVAL 42 DAY) " + "AND DATE(DateTimeStart) < DATE_ADD(tempehrquality.Birthdate,INTERVAL 2 YEAR) " + "AND vaccinedef.CVXCode IN('110','120','20','50'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET NotGiven1=1,Documentation1=Note " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=1 " + "AND vaccinedef.CVXCode IN('110','120','20','50')";
                Db.nonQ(command);
                //Count2, IPV
                command = "UPDATE tempehrquality " + "SET Count2=(SELECT COUNT(DISTINCT VaccinePatNum) FROM vaccinepat " + "LEFT JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=0 " + "AND DATE(DateTimeStart) >= DATE_ADD(tempehrquality.Birthdate,INTERVAL 42 DAY) " + "AND DATE(DateTimeStart) < DATE_ADD(tempehrquality.Birthdate,INTERVAL 2 YEAR) " + "AND vaccinedef.CVXCode IN('10','120'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET NotGiven2=1,Documentation2=Note " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=1 " + "AND vaccinedef.CVXCode IN('10','120')";
                Db.nonQ(command);
                //Count3, MMR
                command = "UPDATE tempehrquality " + "SET Count3=(SELECT COUNT(DISTINCT VaccinePatNum) FROM vaccinepat " + "LEFT JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=0 " + "AND DATE(DateTimeStart) < DATE_ADD(tempehrquality.Birthdate,INTERVAL 2 YEAR) " + "AND vaccinedef.CVXCode IN('03','94'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET NotGiven3=1,Documentation3=Note " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=1 " + "AND vaccinedef.CVXCode IN('03','94')";
                Db.nonQ(command);
                //Count3a, measles
                command = "UPDATE tempehrquality " + "SET Count3a=(SELECT COUNT(DISTINCT VaccinePatNum) FROM vaccinepat " + "LEFT JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=0 " + "AND DATE(DateTimeStart) < DATE_ADD(tempehrquality.Birthdate,INTERVAL 2 YEAR) " + "AND vaccinedef.CVXCode IN('05'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET NotGiven3a=1,Documentation3a=Note " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=1 " + "AND vaccinedef.CVXCode IN('05')";
                Db.nonQ(command);
                //Count3b, mumps
                command = "UPDATE tempehrquality " + "SET Count3b=(SELECT COUNT(DISTINCT VaccinePatNum) FROM vaccinepat " + "LEFT JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=0 " + "AND DATE(DateTimeStart) < DATE_ADD(tempehrquality.Birthdate,INTERVAL 2 YEAR) " + "AND vaccinedef.CVXCode IN('07'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET NotGiven3b=1,Documentation3b=Note " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=1 " + "AND vaccinedef.CVXCode IN('07')";
                Db.nonQ(command);
                //Count3c, rubella
                command = "UPDATE tempehrquality " + "SET Count3c=(SELECT COUNT(DISTINCT VaccinePatNum) FROM vaccinepat " + "LEFT JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=0 " + "AND DATE(DateTimeStart) < DATE_ADD(tempehrquality.Birthdate,INTERVAL 2 YEAR) " + "AND vaccinedef.CVXCode IN('06'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET NotGiven3c=1,Documentation3c=Note " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=1 " + "AND vaccinedef.CVXCode IN('06')";
                Db.nonQ(command);
                //Count4, HiB
                command = "UPDATE tempehrquality " + "SET Count4=(SELECT COUNT(DISTINCT VaccinePatNum) FROM vaccinepat " + "LEFT JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=0 " + "AND DATE(DateTimeStart) >= DATE_ADD(tempehrquality.Birthdate,INTERVAL 42 DAY) " + "AND DATE(DateTimeStart) < DATE_ADD(tempehrquality.Birthdate,INTERVAL 2 YEAR) " + "AND vaccinedef.CVXCode IN('120','46','47','48','49','50','51'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET NotGiven4=1,Documentation4=Note " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=1 " + "AND vaccinedef.CVXCode IN('120','46','47','48','49','50','51')";
                Db.nonQ(command);
                //Count5, HepB
                command = "UPDATE tempehrquality " + "SET Count5=(SELECT COUNT(DISTINCT VaccinePatNum) FROM vaccinepat " + "LEFT JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=0 " + "AND DATE(DateTimeStart) < DATE_ADD(tempehrquality.Birthdate,INTERVAL 2 YEAR) " + "AND vaccinedef.CVXCode IN('08','110','44','51'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET NotGiven5=1,Documentation5=Note " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=1 " + "AND vaccinedef.CVXCode IN('08','110','44','51')";
                Db.nonQ(command);
                //Count6, VZV
                command = "UPDATE tempehrquality " + "SET Count6=(SELECT COUNT(DISTINCT VaccinePatNum) FROM vaccinepat " + "LEFT JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=0 " + "AND DATE(DateTimeStart) < DATE_ADD(tempehrquality.Birthdate,INTERVAL 2 YEAR) " + "AND vaccinedef.CVXCode IN('21','94'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET NotGiven6=1,Documentation6=Note " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=1 " + "AND vaccinedef.CVXCode IN('21','94')";
                Db.nonQ(command);
                //Count7, pneumococcal
                command = "UPDATE tempehrquality " + "SET Count7=(SELECT COUNT(DISTINCT VaccinePatNum) FROM vaccinepat " + "LEFT JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=0 " + "AND DATE(DateTimeStart) >= DATE_ADD(tempehrquality.Birthdate,INTERVAL 42 DAY) " + "AND DATE(DateTimeStart) < DATE_ADD(tempehrquality.Birthdate,INTERVAL 2 YEAR) " + "AND vaccinedef.CVXCode IN('100','133'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET NotGiven7=1,Documentation7=Note " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=1 " + "AND vaccinedef.CVXCode IN('100','133')";
                Db.nonQ(command);
                //Count8, HepA
                command = "UPDATE tempehrquality " + "SET Count8=(SELECT COUNT(DISTINCT VaccinePatNum) FROM vaccinepat " + "LEFT JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=0 " + "AND DATE(DateTimeStart) < DATE_ADD(tempehrquality.Birthdate,INTERVAL 2 YEAR) " + "AND vaccinedef.CVXCode IN('83'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET NotGiven8=1,Documentation8=Note " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=1 " + "AND vaccinedef.CVXCode IN('83')";
                Db.nonQ(command);
                //Count9, rotavirus
                command = "UPDATE tempehrquality " + "SET Count9=(SELECT COUNT(DISTINCT VaccinePatNum) FROM vaccinepat " + "LEFT JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=0 " + "AND DATE(DateTimeStart) >= DATE_ADD(tempehrquality.Birthdate,INTERVAL 42 DAY) " + "AND DATE(DateTimeStart) < DATE_ADD(tempehrquality.Birthdate,INTERVAL 2 YEAR) " + "AND vaccinedef.CVXCode IN('116','119'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET NotGiven9=1,Documentation9=Note " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=1 " + "AND vaccinedef.CVXCode IN('116','119')";
                Db.nonQ(command);
                //Count10, influenza
                command = "UPDATE tempehrquality " + "SET Count10=(SELECT COUNT(DISTINCT VaccinePatNum) FROM vaccinepat " + "LEFT JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=0 " + "AND DATE(DateTimeStart) >= DATE_ADD(tempehrquality.Birthdate,INTERVAL 180 DAY) " + "AND DATE(DateTimeStart) < DATE_ADD(tempehrquality.Birthdate,INTERVAL 2 YEAR) " + "AND vaccinedef.CVXCode IN('135','15'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality,vaccinepat,vaccinedef " + "SET NotGiven10=1,Documentation10=Note " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND NotGiven=1 " + "AND vaccinedef.CVXCode IN('135','15')";
                Db.nonQ(command);
                command = "SELECT * FROM tempehrquality";
                tableRaw = Db.getTable(command);
                command = "DROP TABLE IF EXISTS tempehrquality";
            ...
            case Pneumonia: 
                ...
                Db.nonQ(command);
                //complete
                command = "INSERT INTO tempehrquality (PatNum,LName,FName) SELECT patient.PatNum,LName,FName " + "FROM patient " + "INNER JOIN procedurelog " + "ON Patient.PatNum=procedurelog.PatNum " + "AND procedurelog.ProcStatus=2 " + "AND procedurelog.ProvNum=" + POut.long(provNum) + " " + "AND procedurelog.ProcDate >= " + POut.Date(dateEnd.AddYears(-1)) + " " + "AND procedurelog.ProcDate <= " + POut.date(dateEnd) + " " + "WHERE Birthdate > '1880-01-01' AND Birthdate <= " + POut.Date(dateStart.AddYears(-65)) + " " + "GROUP BY patient.PatNum";
                //65 or older as of dateEnd
                Db.nonQ(command);
                //find most recent vaccine date
                command = "UPDATE tempehrquality " + "SET tempehrquality.DateVaccine=(SELECT MAX(DATE(vaccinepat.DateTimeStart)) " + "FROM vaccinepat,vaccinedef " + "WHERE vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "AND tempehrquality.PatNum=vaccinepat.PatNum " + "AND vaccinepat.NotGiven=0 " + "AND vaccinedef.CVXCode IN('33','100','133'))";
                Db.nonQ(command);
                command = "UPDATE tempehrquality SET DateVaccine='0001-01-01' WHERE DateVaccine='0000-00-00'";
                Db.nonQ(command);
                command = "SELECT * FROM tempehrquality";
            ...
            case DiabetesBloodPressure: 
                ...
                Db.nonQ(command);
                //complete
                command = "INSERT INTO tempehrquality (PatNum,LName,FName) SELECT patient.PatNum,LName,FName " + "FROM patient " + "INNER JOIN procedurelog " + "ON Patient.PatNum=procedurelog.PatNum " + "AND procedurelog.ProcStatus=2 " + "AND procedurelog.ProvNum=" + POut.long(provNum) + " " + "AND procedurelog.ProcDate >= " + POut.date(dateStart) + " " + "AND procedurelog.ProcDate <= " + POut.date(dateEnd) + " " + "WHERE Birthdate <= " + POut.Date(dateStart.AddYears(-17)) + " " + "AND Birthdate >= " + POut.Date(dateStart.AddYears(-74)) + " " + "GROUP BY patient.PatNum";
                //17-74 before dateStart
                Db.nonQ(command);
                //Medication
                //alph-glucosidas
                //amylin analogs
                ...
                //Sulfonylureas
                //Thiazolidinediones
                command = "UPDATE tempehrquality " + "SET HasMedication = 1 " + "WHERE (SELECT COUNT(*) " + "FROM medicationpat " + "WHERE medicationpat.PatNum=tempehrquality.PatNum " + "AND medicationpat.RxCui IN(199149, 199150, 200132, 205329, 205330, 205331, 401938," + "200256, 200257, 200258, 311919, 314142, 389139, 861035, 861039, 861042, 861044, 861787, 861790," + "744863 , 847910 , 847915," + "602544, 602549, 602550, 647237, 647239, 706895, 706896, 861731, 861736, 861740, 861743, 861748, 861753, 861760, 861763, 861769, 861783, 861787, 861790, 861795, 861806, 861816, 861819, 861822," + "665033, 665038, 665042, 860975, 860978, 860981, 860984, 860996, 860999, 861004, 861007, 861010, 861021, 861025, 861731, 861736, 861740, 861743, 861748, 861753, 861760, 861763, 861769, 861783, 861787, 861790, 861795, 861806, 861816, 861819, 861822," + "205314, 237527, 242120, 242916, 242917, 259111, 260265, 283394, 311040, 311041, 311053, 311054, 311055, 311056, 311057, 311058, 311059, 311060, 311061, 314038, 317800, 351297, 358349, 484322, 485210, 544614, 763002, 763007, 763013, 763014, 833159, 847191, 847207, 847211, 847230, 847239, 847252, 847259, 847263, 847416," + "200256, 200257, 200258, 311919, 314142, 389139," + "105374, 153842, 197306, 197307, 197495, 197496, 197737, 198291, 198292, 198293, 198294, 199245, 199246, 199247, 199825, 199984, 199985, 200065, 252960, 310488, 310489, 310490, 310534, 310536, 310537, 310539, 312440, 312441, 312859, 312860, 312861, 313418, 313419, 314000, 314006, 315107, 315239, 317573, 379804, 389137, 602544, 602549, 602550, 647237, 647239, 706895, 706896, 757710, 757712, 844809, 844824, 844827, 861731, 861736, 861740, 861743, 861748, 861753, 861760, 861763, 861783, 861795, 861806, 861816, 861822," + "312440, 312441, 312859, 312860, 312861, 317573) " + "AND (DateStop >= " + POut.Date(dateEnd.AddYears(-2)) + " " + "OR DateStop < '1880-01-01') " + ") > 0";
                //med active <= 2 years before or simultaneous to end date
                //or still active
                Db.nonQ(command);
                //HasDiagnosisDiabetes
                //if there is a start date, it can't be after the period end.
                //no startdate
                command = "UPDATE tempehrquality " + "SET HasDiagnosisDiabetes = 1 " + "WHERE (SELECT COUNT(*) " + "FROM disease,diseasedef " + "WHERE disease.DiseaseDefNum=diseasedef.DiseaseDefNum " + "AND disease.PatNum=tempehrquality.PatNum " + "AND (diseasedef.ICD9Code LIKE '250%' " + "OR diseasedef.ICD9Code LIKE '357.2' " + "OR diseasedef.ICD9Code LIKE '362.0%' " + "OR diseasedef.ICD9Code LIKE '366.41' " + "OR diseasedef.ICD9Code LIKE '648.0%') " + "AND (disease.DateStart <= " + POut.date(dateEnd) + " " + "OR disease.DateStart < '1880-01-01') " + "AND (disease.DateStop >= " + POut.Date(dateEnd.AddYears(-2)) + " " + "OR disease.DateStop < '1880-01-01') " + ") > 0";
                //if there's a datestop, it can't have stopped more than 2 years ago.
                //Specs say: diagnosis active <= 2 years before or simultaneous to end date
                //or still active
                Db.nonQ(command);
                //DateBP
                command = "UPDATE tempehrquality " + "SET tempehrquality.DateBP=(SELECT MAX(vitalsign.DateTaken) " + "FROM vitalsign " + "WHERE tempehrquality.PatNum=vitalsign.PatNum " + "AND vitalsign.BpSystolic != 0 " + "AND vitalsign.BpDiastolic != 0 " + "GROUP BY vitalsign.PatNum)";
                Db.nonQ(command);
                command = "UPDATE tempehrquality SET DateBP='0001-01-01' WHERE DateBP='0000-00-00'";
                Db.nonQ(command);
                //Systolic and diastolic
                command = "UPDATE tempehrquality,vitalsign " + "SET Systolic=BpSystolic, " + "Diastolic=BpDiastolic " + "WHERE tempehrquality.PatNum=vitalsign.PatNum " + "AND tempehrquality.DateBP=vitalsign.DateTaken";
                Db.nonQ(command);
                //HasDiagnosisPolycystic
                command = "UPDATE tempehrquality " + "SET HasDiagnosisPolycystic = 1 " + "WHERE (SELECT COUNT(*) " + "FROM disease,diseasedef " + "WHERE disease.DiseaseDefNum=diseasedef.DiseaseDefNum " + "AND disease.PatNum=tempehrquality.PatNum " + "AND diseasedef.ICD9Code = '256.4' " + "AND (disease.DateStart <= " + POut.date(dateEnd) + " " + "OR disease.DateStart < '1880-01-01') " + ") > 0";
                //if there's a datestart, it can't be after period end
                ...
                //steroid induced
                //gestational
                command = "UPDATE tempehrquality " + "SET HasDiagnosisAcuteDiabetes = 1 " + "WHERE (SELECT COUNT(*) " + "FROM disease,diseasedef " + "WHERE disease.DiseaseDefNum=diseasedef.DiseaseDefNum " + "AND disease.PatNum=tempehrquality.PatNum " + "AND (diseasedef.ICD9Code LIKE '249%' OR diseasedef.ICD9Code='251.8' OR diseasedef.ICD9Code='962.0' " + "OR diseasedef.ICD9Code LIKE '648.8%') " + "AND (disease.DateStart <= " + POut.date(dateEnd) + " " + "OR disease.DateStart < '1880-01-01') " + ") > 0";
                //if there's a datestart, it can't be after period end
                //no restrictions on datestop.  It could still be active or could have stopped before or after the period end.
                Db.nonQ(command);
                command = "SELECT * FROM tempehrquality";
            ...
            case BloodPressureManage: 
                ...
                command = "UPDATE tempehrquality " + "SET HasDiagnosisHypertension = 1 " + "WHERE (SELECT COUNT(*) " + "FROM disease,diseasedef " + "WHERE disease.DiseaseDefNum=diseasedef.DiseaseDefNum " + "AND disease.PatNum=tempehrquality.PatNum " + "AND diseasedef.ICD9Code LIKE '401%' " + "AND (disease.DateStart <= " + POut.Date(dateStart.AddMonths(6)) + " " + "OR disease.DateStart < '1880-01-01') " + ") > 0";
                //if there is a start date, it can't be after this point
                //no startdate
                //no restrictions on datestop.  It could still be active or could have stopped before or after the period end.
                Db.nonQ(command);
                command = "DELETE FROM tempehrquality WHERE HasDiagnosisHypertension=0";
                //for speed
                Db.nonQ(command);
                //HasProcedureESRD
                //ESRD
                command = "UPDATE tempehrquality " + "SET HasProcedureESRD = 1 " + "WHERE (SELECT COUNT(*) " + "FROM procedurelog,procedurecode " + "WHERE procedurelog.CodeNum=procedurecode.CodeNum " + "AND procedurelog.PatNum=tempehrquality.PatNum " + "AND procedurecode.ProcCode IN ('36145','36147','36148','36800', '36810','36815','36818','36819','36820', '36821','36831', '36832', '36833', '50300', '50320','50340','50360','50365','50370', '50380','90920','90921','90924','90925', '90935','90937', '90940','90945', '90947', '90957', '90958','90959','90960','90961','90962','90965','90966','90969','90970','90989','90993','90997','90999','99512') " + "AND procedurelog.ProcDate >= " + POut.date(dateStart) + " " + "AND procedurelog.ProcDate <= " + POut.date(dateEnd) + " " + ") > 0";
                Db.nonQ(command);
                //HasDiagnosisPregnancy
                //active during the period
                //if there is a start date, it can't be after the period end.
                //no startdate
                command = "UPDATE tempehrquality " + "SET HasDiagnosisPregnancy = 1 " + "WHERE (SELECT COUNT(*) " + "FROM disease,diseasedef " + "WHERE disease.DiseaseDefNum=diseasedef.DiseaseDefNum " + "AND disease.PatNum=tempehrquality.PatNum " + "AND (diseasedef.ICD9Code LIKE '63%' " + "OR diseasedef.ICD9Code LIKE '64%' " + "OR diseasedef.ICD9Code LIKE '65%' " + "OR diseasedef.ICD9Code LIKE '66%' " + "OR diseasedef.ICD9Code LIKE '67%' " + "OR diseasedef.ICD9Code LIKE 'V22%' " + "OR diseasedef.ICD9Code LIKE 'V23%' " + "OR diseasedef.ICD9Code LIKE 'V28%') " + "AND (disease.DateStart <= " + POut.date(dateEnd) + " " + "OR disease.DateStart < '1880-01-01') " + "AND (disease.DateStop >= " + POut.date(dateStart) + " " + "OR disease.DateStop < '1880-01-01') " + ") > 0";
                //if there's a datestop, it can't have stopped before the period.
                //or still active
                Db.nonQ(command);
                //HasDiagnosisESRD
                //active during the period
                command = "UPDATE tempehrquality " + "SET HasDiagnosisESRD = 1 " + "WHERE (SELECT COUNT(*) " + "FROM disease,diseasedef " + "WHERE disease.DiseaseDefNum=diseasedef.DiseaseDefNum " + "AND disease.PatNum=tempehrquality.PatNum " + "AND (diseasedef.ICD9Code LIKE '38.95' " + "OR diseasedef.ICD9Code LIKE '39%' " + "OR diseasedef.ICD9Code LIKE '54.98' " + "OR diseasedef.ICD9Code LIKE '55.6%' " + "OR diseasedef.ICD9Code LIKE '585%' " + "OR diseasedef.ICD9Code LIKE 'V42.0%' " + "OR diseasedef.ICD9Code LIKE 'V45.1%' " + "OR diseasedef.ICD9Code LIKE 'V56%') " + "AND (disease.DateStart <= " + POut.date(dateEnd) + " " + "OR disease.DateStart < '1880-01-01') " + "AND (disease.DateStop >= " + POut.date(dateStart) + " " + "OR disease.DateStop < '1880-01-01') " + ") > 0";
                Db.nonQ(command);
                //DateBP
                command = "UPDATE tempehrquality " + "SET tempehrquality.DateBP=(SELECT MAX(vitalsign.DateTaken) " + "FROM vitalsign " + "WHERE tempehrquality.PatNum=vitalsign.PatNum " + "AND vitalsign.BpSystolic != 0 " + "AND vitalsign.BpDiastolic != 0 " + "GROUP BY vitalsign.PatNum)";
                Db.nonQ(command);
        ...
        for (int i = 0;i < tableRaw.Rows.Count;i++)
        {
            ...
            float height = 0;
            float bmi = 0;
            DateTime dateVisit = new DateTime();
            int visitCount = new int();
            switch(qtype)
            {
                case WeightOver65: 
                    //WeightOver65-----------------------------------------------------------------------------------------------------------------
                    ...
                    boolean hasFollowupPlan = PIn.Bool(tableRaw.Rows[i]["HasFollowupPlan"].ToString());
                    boolean isIneligible = PIn.Bool(tableRaw.Rows[i]["IsIneligible"].ToString());
                ...
                case Hypertension: 
                    ...
                    if (dateVisit < dateStart || dateVisit > dateEnd)
                    {
                        continue;
                    }
                     
                    //no visits in the measurement period
                    //don't add this row.  Not part of denominator.
                    if (visitCount < 2)
                    {
                        continue;
                    }
                     
                    if (StringSupport.equals(icd9code, ""))
                    {
                        continue;
                    }
                     
                    if (datePbEntered.Year < 1880)
                    {
                        //no bp entered
                ...
                case TobaccoCessation: 
                    ...
                    else //no visits in the measurement period
                    //don't add this row.  Not part of denominator.
                    if (dateAssessment < dateVisit.AddYears(-2))
                    {
                        continue;
                    }
                    else //no assessment within 24 months, so not part of denominator.
                    if (DateCessation.Year < 1880)
                    {
                ...
                case InfluenzaAdult: 
                    ...
                    boolean notGiven = PIn.Bool(tableRaw.Rows[i]["NotGiven"].ToString());
                    documentation = tableRaw.Rows[i]["Documentation"].ToString();
                ...
                case WeightChild_1_1: 
                    //WeightChild_1_1----------------------------------------------------------------------------------------------------------------
                    boolean isPregnant = PIn.Bool(tableRaw.Rows[i]["IsPregnant"].ToString());
                    boolean hasBMI = PIn.Bool(tableRaw.Rows[i]["HasBMI"].ToString());
                    if (isPregnant)
                    {
                        continue;
                    }
                ...
                case WeightChild_1_2: 
                    ...
                    boolean ChildGotNutrition = PIn.Bool(tableRaw.Rows[i]["ChildGotNutrition"].ToString());
                    if (isPregnant)
                    {
                        continue;
                    }
                ...
                case WeightChild_1_3: 
                    //WeightChild_1_3----------------------------------------------------------------------------------------------------------------
                    isPregnant = PIn.Bool(tableRaw.Rows[i]["IsPregnant"].ToString());
                    boolean ChildGotPhysCouns = PIn.Bool(tableRaw.Rows[i]["ChildGotPhysCouns"].ToString());
                    if (isPregnant)
                    {
                        continue;
                    }
                ...
                case ImmunizeChild_3: 
                    ...
                    boolean notGiven3a = PIn.Bool(tableRaw.Rows[i]["NotGiven3a"].ToString());
                    ...
                    boolean notGiven3b = PIn.Bool(tableRaw.Rows[i]["NotGiven3b"].ToString());
                    ...
                    boolean notGiven3c = PIn.Bool(tableRaw.Rows[i]["NotGiven3c"].ToString());
                ...
                case ImmunizeChild_11: 
                    ...
                    boolean notGiven1 = PIn.Bool(tableRaw.Rows[i]["NotGiven1"].ToString());
                    boolean notGiven2 = PIn.Bool(tableRaw.Rows[i]["NotGiven2"].ToString());
                    boolean notGiven3 = PIn.Bool(tableRaw.Rows[i]["NotGiven3"].ToString());
                    ...
                    boolean notGiven4 = PIn.Bool(tableRaw.Rows[i]["NotGiven4"].ToString());
                    boolean notGiven5 = PIn.Bool(tableRaw.Rows[i]["NotGiven5"].ToString());
                    boolean notGiven6 = PIn.Bool(tableRaw.Rows[i]["NotGiven6"].ToString());
                    if (notGiven1 || notGiven2 || notGiven3 || notGiven3a || notGiven3b || notGiven3c || notGiven4 || notGiven5 || notGiven6)
                    {
                        row["exclusion"] = "X";
                        row["explanation"] += "Not given.";
                    }
                    else //too complicated to document.
                    if (count1 >= 4 && count2 >= 3 && (count3 >= 1 || (count3a >= 1 && count3b >= 1 && count3c >= 1)) && count4 >= 2 && count5 >= 3 && count6 >= 1)
                    {
                        row["numerator"] = "X";
                ...
                case ImmunizeChild_12: 
                    ...
                    boolean notGiven7 = PIn.Bool(tableRaw.Rows[i]["NotGiven7"].ToString());
                    ...
                    if (count1 >= 4 && count2 >= 3 && (count3 >= 1 || (count3a >= 1 && count3b >= 1 && count3c >= 1)) && count4 >= 2 && count5 >= 3 && count6 >= 1 && count7 >= 4)
                    {
                        row["numerator"] = "X";
                        row["explanation"] = "All vaccinations given.";
                    }
                ...
                case DiabetesBloodPressure: 
                    //DiabetesBloodPressure---------------------------------------------------------------------------------------------------
                    boolean hasMedication = PIn.Bool(tableRaw.Rows[i]["HasMedication"].ToString());
                    boolean HasDiagnosisDiabetes = PIn.Bool(tableRaw.Rows[i]["HasDiagnosisDiabetes"].ToString());
                    ...
                    boolean HasDiagnosisPolycystic = PIn.Bool(tableRaw.Rows[i]["HasDiagnosisPolycystic"].ToString());
                    boolean HasDiagnosisAcuteDiabetes = PIn.Bool(tableRaw.Rows[i]["HasDiagnosisAcuteDiabetes"].ToString());
                    if (!hasMedication && !HasDiagnosisDiabetes)
                    {
                        continue;
                    }
                     
                    //not part of denominator
                ...
                case BloodPressureManage: 
                    //BloodPressureManage-------------------------------------------------------------------------------------------------------
                    boolean HasDiagnosisHypertension = PIn.Bool(tableRaw.Rows[i]["HasDiagnosisHypertension"].ToString());
                    boolean HasProcedureESRD = PIn.Bool(tableRaw.Rows[i]["HasProcedureESRD"].ToString());
                    boolean HasDiagnosisPregnancy = PIn.Bool(tableRaw.Rows[i]["HasDiagnosisPregnancy"].ToString());
                    boolean HasDiagnosisESRD = PIn.Bool(tableRaw.Rows[i]["HasDiagnosisESRD"].ToString());
    ...
    /**
    * Only called from GetAll2014.  Once the EhrCqmData object is created, all of the data relevant to the measure and required by the QRDA category 1 and category 3 reporting is part of the object.
    */
    private static QualityMeasure getEhrCqmData(QualityType2014 qtype, DateTime dateStart, DateTime dateEnd, long provNum) throws Exception {
        if (RemotingClient.RemotingRole == RemotingRole.ClientWeb)
        {
            return Meth.<QualityMeasure>GetObject(MethodBase.GetCurrentMethod(), qtype, dateStart, dateEnd, provNum);
        }
         
        //these queries only work for mysql
        String command = "SELECT GROUP_CONCAT(provider.ProvNum) FROM provider WHERE provider.EhrKey=" + "(SELECT pv.EhrKey FROM provider pv WHERE pv.ProvNum=" + POut.long(provNum) + ")";
        ...
        List<String> listReasonOIDs = new List<String>();
        List<long> listEhrPatNums = new List<long>();
        ...
        //valid birthdate
        String encounterSelectWhere = "SELECT encounter.* FROM encounter " + "INNER JOIN patient ON patient.PatNum=encounter.PatNum " + "WHERE YEAR(patient.Birthdate)>1880 " + "AND encounter.ProvNum IN(" + POut.string(provs) + ") " + "AND DATE(encounter.DateEncounter) BETWEEN " + POut.date(dateStart) + " AND " + POut.date(dateEnd) + " ";
        String encounterWhereAdults = "AND patient.Birthdate<=" + POut.date(dateStart) + "-INTERVAL 18 YEAR ";
        //18 or over at start of measurement period
        ...
        switch(qtype)
        {
            case MedicationsEntered: 
                ...
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.600.1.1834");
                //Medications Encounter Code Set Grouping Value Set
                ...
                //Current Medications Documented SNMD SNOMED-CT Value Set
                ...
                listValueSetOIDs = new List<String>();
                //Current Medications Documented SNMD SNOMED-CT Value Set
                listReasonOIDs = new List<String>();
                //Medical or Other reason not done SNOMED-CT Value Set
            ...
            case WeightOver65: 
                ...
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.600.1.1751");
                //BMI Encounter Code Set Grouping Value Set
                String encsWhere65 = "AND patient.Birthdate<" + POut.date(dateStart) + "-INTERVAL 65 YEAR ";
                //65 or over at the start of the measurement period
                String encsWhereLessThan64 = "AND patient.Birthdate>" + POut.date(dateStart) + "-INTERVAL 64 YEAR ";
                //< 64 years old at the start of the measurement period
                String encCommand = "";
                ...
                listValueSetOIDs = new List<String>();
                //Palliative Care Grouping Value Set
                listValueSetOIDs.Add("2.16.840.1.113883.3.600.1.1623");
                //Pregnancy Dx Grouping Value Set
                ...
                listValueSetOIDs = new List<String>();
                //BMI LOINC Value LOINC Value Set
                listReasonOIDs = new List<String>();
                //Medical or Other reason not done SNOMED-CT Value Set
                listReasonOIDs.Add("2.16.840.1.113883.3.600.1.1503");
                //Patient Reason Refused SNOMED-CT Value Set
                ...
                for (Object __dummyForeachVar0 : measureCur.DictPatNumListEncounters)
                {
                    ...
                    KeyValuePair<long, List<EhrCqmEncounter>> patNumListEncs = (KeyValuePair<long, List<EhrCqmEncounter>>)__dummyForeachVar0;
                    //loop through every patient with an encounter list
                    if (!measureCur.DictPatNumListProblems.ContainsKey(patNumListEncs.Key))
                    {
                        continue;
                    }
                     
                    //if no palliative care problem, move to next patient
                    List<EhrCqmProblem> listProbsCur = measureCur.DictPatNumListProblems[patNumListEncs.Key];
                    for (int i = patNumListEncs.Value.Count - 1;i > -1;i--)
                    {
                        for (int j = 0;j < listProbsCur.Count;j++)
                        {
                            //loop through encounter list for this patient
                            //loop through palliative care problem list for this patient\
                            if (!StringSupport.equals(listProbsCur[j].ValueSetOID, "2.16.840.1.113883.3.600.1.1579"))
                            {
                                continue;
                            }
                             
                            //if not Palliative Care, move to next problem (problem list for this measure can contain pregnancy dx or palliative care 'procedures')
                            if (listProbsCur[j].DateStart.Date <= patNumListEncs.Value[i].DateEncounter.Date)
                            {
                                //if palliative care dx starts before or on encounter date
                                patNumListEncs.Value.RemoveAt(i);
                                break;
                            }
                ...
                for (Object __dummyForeachVar1 : measureCur.DictPatNumListEncounters)
                {
                    //remove the encounter
                    //break out of problem list loop, move to next encounter
                    ...
                    KeyValuePair<long, List<EhrCqmEncounter>> patNumListEncs = (KeyValuePair<long, List<EhrCqmEncounter>>)__dummyForeachVar1;
                    //loop through every patient with an encounter list
                    if (!measureCur.DictPatNumListNotPerfs.ContainsKey(patNumListEncs.Key))
                    {
                        continue;
                    }
                     
                    //if no not performed items, move to next patient
                    List<EhrCqmNotPerf> listNotPerfsCur = measureCur.DictPatNumListNotPerfs[patNumListEncs.Key];
                    for (int i = patNumListEncs.Value.Count - 1;i > -1;i--)
                    {
                        for (int j = 0;j < listNotPerfsCur.Count;j++)
                        {
                            //the not performed items are guaranteed to have valid reasons
                            //loop through encounters for this patient
                            //loop through not performed items for this patient
                            if (listNotPerfsCur[j].DateEntry.Date == patNumListEncs.Value[i].DateEncounter.Date)
                            {
                                //compare encounter date to not perfomed date
                                patNumListEncs.Value.RemoveAt(i);
                                break;
                            }
                ...
                //remove encounter if not performed item on same date
                //break out of not performed loop to move to next encounter
                ...
                command = "SELECT * FROM intervention " + "WHERE DATE(DateEntry) BETWEEN " + POut.date(dateStart) + "-INTERVAL 6 MONTH AND " + POut.date(dateEnd) + " ";
                ...
                listValueSetOIDs = new List<String>();
                //Above Normal Follow-up Grouping Value Set
                listValueSetOIDs.Add("2.16.840.1.113883.3.600.1.1528");
                //Below Normal Follow up Grouping Value Set
                listValueSetOIDs.Add("2.16.840.1.113883.3.600.1.1527");
                //Referrals where weight assessment may occur Grouping Value Set
                ...
                listValueSetOIDs = new List<String>();
                //Above Normal Medications RxNorm Value Set
                listValueSetOIDs.Add("2.16.840.1.113883.3.600.1.1499");
                //Below Normal Medications RxNorm Value Set
            ...
            case CariesPrevent_3: 
                ...
                //No exceptions
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1001");
                //Office Visit Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.125.12.1003");
                //Clinical Oral Evaluation Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1022");
                //Preventive Care- Initial Office Visit, 0 to 17 Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1023");
                //Preventive Care Services-Initial Office Visit, 18 and Up Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1024");
                //Preventive Care - Established Office Visit, 0 to 17 Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1025");
                //Preventive Care Services - Established Office Visit, 18 and Up Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1048");
                //Face-to-Face Interaction Grouping Value Set
                //age >= 0 at start of measurement period
                //age < 20 at start of measurement period
                String child0To19Command = encounterSelectWhere + "AND patient.Birthdate<=" + POut.date(dateStart) + " " + "AND patient.Birthdate>" + POut.date(dateStart) + "-INTERVAL 20 YEAR " + encounterOrder;
                //age >= 0 at start of measurement period
                //age <= 5 at start of measurement period
                String child0To5Command = encounterSelectWhere + "AND patient.Birthdate<=" + POut.date(dateStart) + " " + "AND patient.Birthdate>" + POut.date(dateStart) + "-INTERVAL 6 YEAR " + encounterOrder;
                //age >= 6 at start of measurement period
                //age <= 12 at start of measurement period
                String child6To12Command = encounterSelectWhere + "AND patient.Birthdate<=" + POut.date(dateStart) + "-INTERVAL 6 YEAR " + "AND patient.Birthdate>" + POut.date(dateStart) + "-INTERVAL 13 YEAR " + encounterOrder;
                //age >= 13 at start of measurement period
                //age < 20 at start of measurement period
                String child13To19Command = encounterSelectWhere + "AND patient.Birthdate<=" + POut.date(dateStart) + "-INTERVAL 13 YEAR " + "AND patient.Birthdate>" + POut.date(dateStart) + "-INTERVAL 20 YEAR " + encounterOrder;
                ...
                listEhrPatNums = getListPatNums(measureCur.ListEhrPats);
                listValueSetOIDs = new List<String>();
                //Fluoride Varnish Application for Children Grouping Value Set
            ...
            case ChildCaries: 
                ...
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1048");
                //Face-to-Face Interaction Grouping Value Set
                //age >= 0 at start of measurement period (born before or on start date)
                //age < 20 at start of measurement period
                child0To19Command = encounterSelectWhere + "AND patient.Birthdate<=" + POut.date(dateStart) + " " + "AND patient.Birthdate>" + POut.date(dateStart) + "-INTERVAL 20 YEAR " + encounterOrder;
                measureCur.DictPatNumListEncounters = GetEncountersWithOneOfAndTwoOfOIDs(child0To19Command, listOneOfEncOIDs, listTwoOfEncOIDs);
                ...
                listEhrPatNums = getListPatNums(measureCur.ListEhrPats);
                listValueSetOIDs = new List<String>();
                //Dental Caries Grouping Value Set
            ...
            case Pneumonia: 
                ...
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.526.3.1240");
                //Annual Wellness Visit Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1001");
                //Office Visit Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1016");
                //Home Healthcare Services Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1023");
                //Preventive Care Services-Initial Office Visit, 18 and Up Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1025");
                //Preventive Care Services - Established Office Visit, 18 and Up Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1048");
                //Face-to-Face Interaction Grouping Value Set
                //65 or over at start of measurement period
                encCommand = encounterSelectWhere + "AND patient.Birthdate<" + POut.date(dateStart) + "-INTERVAL 65 YEAR " + encounterOrder;
                measureCur.DictPatNumListEncounters = GetEncountersWithOneOfAndTwoOfOIDs(encCommand, listOneOfEncOIDs, listTwoOfEncOIDs);
                //if no eligible encounters, no need to go on
                if (measureCur.DictPatNumListEncounters.Count == 0)
                {
                ...
                measureCur.ListEhrPats = getEhrPatsFromEncsOrProcs(measureCur.DictPatNumListEncounters);
                listEhrPatNums = getListPatNums(measureCur.ListEhrPats);
                //for restricting the following list of procs, problems, and vaccinepats
                listValueSetOIDs = new List<String>();
                //Pneumococcal Vaccine Grouping Value Set
                ...
                listValueSetOIDs = new List<String>();
                //Pneumococcal Vaccine Administered Grouping Value Set
                ...
                listValueSetOIDs = new List<String>();
                //History of Pneumococcal Vaccine Grouping Value Set
            ...
            case TobaccoCessation: 
                ...
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1025");
                //Preventive Care Services - Established Office Visit, 18 and Up Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1026");
                //Preventive Care Services-Individual Counseling Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1027");
                //Preventive Care Services - Group Counseling Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1030");
                //Preventive Care Services - Other Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1048");
                //Face-to-Face Interaction Grouping Value Set
                //add two of encounter OIDs to list
                listTwoOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1001");
                //Office Visit Grouping Value Set
                listTwoOfEncOIDs.Add("2.16.840.1.113883.3.526.3.1011");
                //Occupational Therapy Evaluation Grouping Value Set
                listTwoOfEncOIDs.Add("2.16.840.1.113883.3.526.3.1020");
                //Health & Behavioral Assessment - Individual Grouping Value Set
                listTwoOfEncOIDs.Add("2.16.840.1.113883.3.526.3.1141");
                //Psychoanalysis Grouping Value Set
                listTwoOfEncOIDs.Add("2.16.840.1.113883.3.526.3.1245");
                //Health and Behavioral Assessment - Initial Grouping Value Set
                listTwoOfEncOIDs.Add("2.16.840.1.113883.3.526.3.1285");
                //Ophthalmological Services Grouping Value Set
                listTwoOfEncOIDs.Add("2.16.840.1.113883.3.526.3.1492");
                //Psych Visit - Diagnostic Evaluation Grouping Value Set
                listTwoOfEncOIDs.Add("2.16.840.1.113883.3.526.3.1496");
                //Psych Visit - Psychotherapy Grouping Value Set
                //measureCur.ListEncounters will include all encounters that belong to the OneOf and TwoOf lists, so a patient will appear more than once
                //if they had more than one encounter from those sets in date range
                measureCur.DictPatNumListEncounters = GetEncountersWithOneOfAndTwoOfOIDs(adultEncCommand, listOneOfEncOIDs, listTwoOfEncOIDs);
                //if no eligible encounters, no need to go on
                ...
                command = "SELECT * FROM intervention " + "WHERE DATE(DateEntry) BETWEEN " + POut.date(dateEnd) + "-INTERVAL 24 MONTH AND " + POut.date(dateEnd) + " ";
                if (listEhrPatNums != null && listEhrPatNums.Count > 0)
                {
                    command += "AND intervention.PatNum IN(" + String.Join(",", listEhrPatNums) + ") ";
                }
                ...
                command += "ORDER BY PatNum,DateEntry DESC";
                listValueSetOIDs = new List<String>();
                //Tobacco Use Cessation Counseling Grouping Value Set
                measureCur.DictPatNumListInterventions = GetInterventions(command, listValueSetOIDs);
                //Get a list of all tobacco cessation meds active/ordered within 24 months of end of measurement period
                listValueSetOIDs = new List<String>();
                /**
                * /Tobacco Use Cessation Pharmacotherapy Grouping Value Set
                */
                ...
                listValueSetOIDs = new List<String>();
                //Tobacco Use Screening Grouping Value Set
                listReasonOIDs = new List<String>();
                //Medical Reason Grouping Value Set
                measureCur.DictPatNumListNotPerfs = GetNotPerformeds(listEhrPatNums, listValueSetOIDs, listReasonOIDs, dateStart, dateEnd);
                listValueSetOIDs = new List<String>();
                //Limited Life Expectancy Grouping Value Set
            ...
            case Influenza: 
                ...
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.526.3.1240");
                //Annual Wellness Visit Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1012");
                //Nursing Facility Visit Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1013");
                //Discharge Services - Nursing Facility Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1022");
                //Preventive Care- Initial Office Visit, 0 to 17 Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1023");
                //Preventive Care Services-Initial Office Visit, 18 and Up Grouping Value Set
                ...
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1030");
                //Preventive Care Services - Other Grouping Value Set
                listTwoOfEncOIDs.Add("2.16.840.1.113883.3.526.3.1012");
                //Patient Provider Interaction Grouping Value Set
                listTwoOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1001");
                //Office Visit Grouping Value Set
                listTwoOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1008");
                //Outpatient Consultation Grouping Value Set
                listTwoOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1014");
                //Care Services in Long-Term Residential Facility Grouping Value Set
                listTwoOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1016");
                //Home Healthcare Services Grouping Value Set
                //>= 6 months at start of measurement period
                encCommand = encounterSelectWhere + "AND patient.Birthdate<" + POut.date(dateStart) + "-INTERVAL 6 MONTH " + encounterOrder;
                measureCur.DictPatNumListEncounters = GetEncountersWithOneOfAndTwoOfOIDs(encCommand, listOneOfEncOIDs, listTwoOfEncOIDs);
                listValueSetOIDs = new List<String>();
                //Hemodialysis Grouping Value Set
                listValueSetOIDs.Add("2.16.840.1.113883.3.526.3.1084");
                //Peritoneal Dialysis Grouping Value Set
                ...
                Dictionary<long, List<EhrCqmEncounter>> dictPatNumListDenomEncs = new Dictionary<long, List<EhrCqmEncounter>>();
                if (listEhrPatNums != null && listEhrPatNums.Count == 0)
                {
                ...
                {
                    listOneOfEncOIDs = new List<String>();
                    //Encounter-Influenza Grouping Value Set
                    listTwoOfEncOIDs = new List<String>();
                    //valid birthdate
                    encCommand = "SELECT encounter.* FROM encounter " + "INNER JOIN patient ON patient.PatNum=encounter.PatNum " + "WHERE YEAR(patient.Birthdate)>1880 " + "AND encounter.ProvNum IN(" + POut.string(provs) + ") " + "AND DATE(encounter.DateEncounter) BETWEEN " + POut.Date(dateStart.AddDays(-92)) + " AND " + POut.Date(dateStart.AddDays(91)) + " ";
                    if (listEhrPatNums != null && listEhrPatNums.Count > 0)
                    {
                ...
                for (int i = 0;i < measureCur.ListEhrPats.Count;i++)
                {
                    ...
                    if (dictPatNumListDenomEncs.ContainsKey(patNumCur))
                    {
                        ...
                        {
                            //no encounters from initial patient population, must be in ipp from procedure, add new list to include these encounters
                ...
                listValueSetOIDs = new List<String>();
                //Influenza Vaccination Grouping Value Set
                ...
                listValueSetOIDs = new List<String>();
                //Influenza Vaccine Grouping Value Set
                measureCur.DictPatNumListMedPats = GetVaccines(listEhrPatNums, listValueSetOIDs, DateTime.MinValue, dateStart.AddDays(91), false);
                //false means could be vaccines given or not given
                ...
                listValueSetOIDs = new List<String>();
                //Previous Receipt of Influenza Vaccine Grouping Value Set
                //this code is only one eligible SNOMEDCT code used for exceptions, but we will add it to the list here
                listValueSetOIDs.Add("2.16.840.1.113883.3.526.3.1255");
                //Influenza Vaccination Declined Grouping Value Set
                ...
                listValueSetOIDs = new List<String>();
                //Diagnosis, Active: Allergy to eggs Grouping Value Set, all SNOMEDCT codes
                listValueSetOIDs.Add("2.16.840.1.113883.3.526.3.1256");
                //Diagnosis, Active: Allergy to Influenza Vaccine Grouping Value Set, SNOMEDCT codes
                listValueSetOIDs.Add("2.16.840.1.113883.3.526.3.1257");
                //Diagnosis, Active: Intolerance to Influenza Vaccine Grouping Value Set, SNOMEDCT codes
                ...
                listValueSetOIDs = new List<String>();
                //Medication, Administered: Influenza Vaccine Grouping Value Set
                listValueSetOIDs.Add("2.16.840.1.113883.3.526.3.402");
                //Procedure, Performed: Influenza Vaccination Grouping Value Set
                listReasonOIDs = new List<String>();
                //Medical Reason Grouping Value Set
                listReasonOIDs.Add("2.16.840.1.113883.3.526.3.1008");
                //Patient Reason Grouping Value Set
                listReasonOIDs.Add("2.16.840.1.113883.3.526.3.1009");
                //System Reason Grouping Value Set
            ...
            case WeightChild_3_1: 
                ...
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1022");
                //Preventive Care- Initial Office Visit, 0 to 17 Grouping Value Set
                //age >= 3 at start of measurement period
                //age < 17 at start of measurement period
                String child3To16Command = encounterSelectWhere + "AND patient.Birthdate<=" + POut.date(dateStart) + "-INTERVAL 3 YEAR " + "AND patient.Birthdate>" + POut.date(dateStart) + "-INTERVAL 17 YEAR " + encounterOrder;
                //age >= 3 at start of measurement period
                //age <= 11 at start of measurement period
                String child3To11Command = encounterSelectWhere + "AND patient.Birthdate<=" + POut.date(dateStart) + "-INTERVAL 3 YEAR " + "AND patient.Birthdate>" + POut.date(dateStart) + "-INTERVAL 12 YEAR " + encounterOrder;
                //age >= 12 at start of measurement period
                //age < 17 at start of measurement period
                String child12To16Command = encounterSelectWhere + "AND patient.Birthdate<=" + POut.date(dateStart) + "-INTERVAL 12 YEAR " + "AND patient.Birthdate>" + POut.date(dateStart) + "-INTERVAL 17 YEAR " + encounterOrder;
                ...
                else
                {
                    ...
                    command = "SELECT * FROM intervention " + "WHERE DATE(DateEntry) BETWEEN " + POut.date(dateStart) + " AND " + POut.date(dateEnd) + " ";
                    if (listEhrPatNums != null && listEhrPatNums.Count > 0)
                    {
                        command += "AND intervention.PatNum IN(" + String.Join(",", listEhrPatNums) + ") ";
                    }
                    ...
                    command += "ORDER BY PatNum,DateEntry DESC";
                    listValueSetOIDs = new List<String>();
                    //Counseling for Nutrition Grouping Value Set
                    listValueSetOIDs.Add("2.16.840.1.113883.3.464.1003.118.12.1035");
                    //Counseling for Physical Activity Grouping Value Set
                    measureCur.DictPatNumListInterventions = GetInterventions(command, listValueSetOIDs);
                } 
                if (qtype == QualityType2014.WeightChild_1_1)
                {
            ...
            case BloodPressureManage: 
                ...
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1001");
                //Office Visit Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1008");
                //Outpatient Consultation Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1016");
                //Home Healthcare Services Grouping Value Set
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1023");
                ...
                listOneOfEncOIDs.Add("2.16.840.1.113883.3.464.1003.101.12.1048");
                //Face-to-Face Interaction Grouping Value Set
                //< 85 years old at the start of the measurement period
                String encsWhere18To85 = encounterSelectWhere + "AND patient.Birthdate>" + POut.date(dateStart) + "-INTERVAL 85 YEAR " + encounterOrder;
                measureCur.DictPatNumListEncounters = GetEncountersWithOneOfAndTwoOfOIDs(encsWhere18To85, listOneOfEncOIDs, listTwoOfEncOIDs);
                listValueSetOIDs = new List<String>();
                //Essential Hypertension Grouping Value Set
                ...
                listValueSetOIDs = new List<String>();
                //Essential Hypertension Grouping Value Set
                listValueSetOIDs.Add("2.16.840.1.113883.3.526.3.378");
                //Pregnancy Grouping Value Set
                listValueSetOIDs.Add("2.16.840.1.113883.3.526.3.353");
                //End Stage Renal Disease Grouping Value Set
                listValueSetOIDs.Add("2.16.840.1.113883.3.526.3.1002");
                //Chronic Kidney Disease, Stage 5 Grouping Value Set
                measureCur.DictPatNumListProblems = GetProblems(listEhrPatNums, listValueSetOIDs, dateStart, dateEnd);
                if (listEhrPatNums != null && listEhrPatNums.Count == 0)
                {
                    measureCur.DictPatNumListInterventions = new Dictionary<long, List<EhrCqmIntervention>>();
                ...
                {
                    listValueSetOIDs = new List<String>();
                    //Other Services Related to Dialysis Grouping Value Set
                    listValueSetOIDs.Add("2.16.840.1.113883.3.464.1003.109.12.1016");
                    //Dialysis Education Grouping Value Set
                    //Get all interventions for eligible value sets that occurred before or during the measurement period
                    command = "SELECT * FROM intervention " + "WHERE DATE(DateEntry)<=" + POut.date(dateEnd) + " ";
                    if (listEhrPatNums != null && listEhrPatNums.Count > 0)
                    {
                        command += "AND intervention.PatNum IN(" + String.Join(",", listEhrPatNums) + ") ";
                    }
                ...
                listValueSetOIDs = new List<String>();
                //Vascular Access for Dialysis Grouping Value Set
                listValueSetOIDs.Add("2.16.840.1.113883.3.464.1003.109.12.1012");
                //Kidney Transplant Grouping Value Set
                listValueSetOIDs.Add("2.16.840.1.113883.3.464.1003.109.12.1013");
                //Dialysis Services Grouping Value Set
                measureCur.DictPatNumListProcs = GetProcs(listEhrPatNums, listValueSetOIDs, DateTime.MinValue, dateEnd);
                ...
                listValueSetOIDs = new List<String>();
                //ESRD Monthly Outpatient Services Grouping Value Set
                //valid birthdate
                command = "SELECT encounter.* FROM encounter " + "INNER JOIN patient ON patient.PatNum=encounter.PatNum " + "WHERE YEAR(patient.Birthdate)>1880 " + "AND encounter.ProvNum IN(" + POut.string(provs) + ") " + "AND DATE(encounter.DateEncounter)<=" + POut.date(dateEnd) + " ";
                if (listEhrPatNums != null && listEhrPatNums.Count > 0)
                {
                    command += "AND encounter.PatNum IN(" + String.Join(",", listEhrPatNums) + ") ";
                }
    ...
    /**
    * Simple helper function to get a list of PatNums from a list of EhrCqmPatient objects.  Used to limit the number of records returned in other Get functions below, like GetProcs.
    */
    private static List<long> getListPatNums(List<EhrCqmPatient> listEhrCqmPats) throws Exception {
    ...
    */
    private static Dictionary<long, List<EhrCqmEncounter>> getEncountersWithOneOfAndTwoOfOIDs(String command, List<String> listOneOfEncOIDs, List<String> listTwoOfEncOIDs) throws Exception {
        ...
        for (int i = listEncs.Count - 1;i > -1;i--)
        {
            ...
            if (!isTwoOf)
            {
                //not oneOf or twoOf encounter, remove from list
                listEncs.RemoveAt(i);
            }
             
        }
        for (Object __dummyForeachVar2 : dictPatNumAndTwoOfCount)
        {
            //not an eligible encounter
            //add the patients who had 2 or more of the TwoOf encounters to the list of patients
            KeyValuePair<long, int> kpairCur = (KeyValuePair<long, int>)__dummyForeachVar2;
        ...
        for (int i = 0;i < listEncs.Count;i++)
        {
            ...
            String descript = "";
            descript = ehrEncCur.Description;
            //in case not in table default to EhrCode object description
            ...
            System.String __dummyScrutVar8 = ehrEncCur.CodeSystemName;
            if (__dummyScrutVar8.equals("CDT"))
            {
                descript = ProcedureCodes.getProcCode(ehrEncCur.CodeValue).Descript;
            }
            else if (__dummyScrutVar8.equals("CPT"))
            {
            ...
            else if (__dummyScrutVar8.equals("HCPCS"))
            {
            ...
            else if (__dummyScrutVar8.equals("SNOMEDCT"))
            {
            ...
            {
                retval.Add(ehrEncCur.PatNum, new List<EhrCqmEncounter>());
            } 
        }
        return retval;
    }

    private static List<EhrCqmPatient> getEhrPatsFromEncsOrProcs(Dictionary<long, List<EhrCqmEncounter>> dictPatNumListEncounters) throws Exception {
        return GetEhrPatsFromEncsOrProcs(dictPatNumListEncounters, null);
    }

    /**
    * Get relevant demographic and supplemental patient data required for CQM reporting for each unique patient in the list of eligible encounters in the dictionary of PatNums linked to a list of encounters for each PatNum.
    */
    private static List<EhrCqmPatient> getEhrPatsFromEncsOrProcs(Dictionary<long, List<EhrCqmEncounter>> dictPatNumListEncounters, Dictionary<long, List<EhrCqmProc>> dictPatNumListProcs) throws Exception {
        ...
        List<EhrCode> listEhrCodesForPayerTypeOID = EhrCodes.GetForValueSetOIDs(new List<String>(), false);
        for (int i = 0;i < uniquePats.Length;i++)
        {
            ...
            if (payerTypeCur != null)
            {
                ...
                {
                    if (StringSupport.equals(listEhrCodesForPayerTypeOID[j].CodeValue, payerTypeCur.SopCode))
                    {
                        //add payer information if it is in the value set
    ...
    /**
    * Get ehrmeasureevents of type TobaccoAssessment where the event code is in the Tobacco Use Screening value set and the assessment resulted in categorizing the patient as a user or non-user and the screening was within 24 months of the measurement period end date.  Ordered by PatNum, DateTEvent.
    */
    private static Dictionary<long, List<EhrCqmMeasEvent>> getTobaccoAssessmentEvents(List<long> listPatNums, DateTime dateEnd) throws Exception {
        ...
        Dictionary<String, String> dictEventCodesAndSystems = EhrCodes.GetCodeAndCodeSystem(new List<String>(), false);
        //Tobacco Use Screening Value Set
        List<String> listTobaccoStatusOIDs = new List<String>();
        listTobaccoStatusOIDs.Add("2.16.840.1.113883.3.526.3.1170");
        //Tobacco User Grouping Value Set
        listTobaccoStatusOIDs.Add("2.16.840.1.113883.3.526.3.1189");
        //Tobacco Non-User Grouping Value Set
        ...
        for (int i = tableEvents.Rows.Count - 1;i > -1;i--)
        {
            ...
            if (dictEventCodesAndSystems.ContainsKey(tableEvents.Rows[i]["CodeValueEvent"].ToString()) && dictEventCodesAndSystems[tableEvents.Rows[i]["CodeValueEvent"].ToString()] == tableEvents.Rows[i]["CodeSystemEvent"].ToString())
            {
                isValidEvent = true;
            }
            ...
            {
                if (listTobaccoStatusCodes[j].CodeValue == tableEvents.Rows[i]["CodeValueResult"].ToString() && listTobaccoStatusCodes[j].CodeSystem == tableEvents.Rows[i]["CodeSystemResult"].ToString())
                {
                    indexStatus = j;
                    break;
                }
        ...
        for (int i = 0;i < tableEvents.Rows.Count;i++)
        {
            ...
            String descript = tableEvents.Rows[i]["Description"].ToString();
            //if code is not in snomed table we will use description of EhrCode object
            ...
            {
                descript = ehrCodeCur.Description;
            }
             
            //default to description of EhrCode object
            ...
            {
                retval.Add(tobaccoAssessCur.PatNum, new List<EhrCqmMeasEvent>());
            } 
        }
        return retval;
    }
    ...
    /**
    * Get all data needed for reporting QRDA's for interventions from the supplied command where the code belongs to the value set(s) sent in.  Command orders interventions by patnum, then date entered so the first one found for patient is most recent intervention when looping through table.
    */
    private static Dictionary<long, List<EhrCqmIntervention>> getInterventions(String command, List<String> listValueSetOIDs) throws Exception {
        ...
        List<EhrCode> listAllInterventionCodes = EhrCodes.GetForValueSetOIDs(listValueSetOIDs, false);
        //Tobacco Use Cessation Counseling Grouping Value Set
        Dictionary<long, EhrCode> dictInterventionNumEhrCode = new Dictionary<long, EhrCode>();
        for (int i = listInterventions.Count - 1;i > -1;i--)
        {
            boolean isValidIntervention = false;
            for (int j = 0;j < listAllInterventionCodes.Count;j++)
            {
                if (listInterventions[i].CodeValue == listAllInterventionCodes[j].CodeValue && listInterventions[i].CodeSystem == listAllInterventionCodes[j].CodeSystem)
                {
        ...
        for (int i = 0;i < listInterventions.Count;i++)
        {
            ...
            //if not in table or not a CPT, ICD9CM, ICD10CM, HCPCS, or SNOMEDCT code, default to EhrCode object description
            CodeSystem __dummyScrutVar9 = listInterventions[i].CodeSystem;
            if (__dummyScrutVar9.equals("CPT"))
            {
            ...
            else if (__dummyScrutVar9.equals("HCPCS"))
            {
            ...
            else if (__dummyScrutVar9.equals("ICD9CM"))
            {
            ...
            else if (__dummyScrutVar9.equals("ICD10CM"))
            {
            ...
            else if (__dummyScrutVar9.equals("SNOMEDCT"))
            {
            ...
            {
                retval.Add(interventionCur.PatNum, new List<EhrCqmIntervention>());
            } 
        }
        return retval;
    }
    ...
    /**
    * Get the medication information for medications where the code belongs to one of the value sets in the supplied list and the medication start date is in the supplied date range.  Ordered by PatNum,DateStart so first found for patient is most recent to make calculation easier.  If there is a PatNote, this is a Medication Order.  If there is no note and there is either no stop date or the stop date is after the measurement period end date, this is an active Medication.
    */
    private static Dictionary<long, List<EhrCqmMedicationPat>> getMedPats(List<long> listPatNums, List<String> listValueSetOIDs, DateTime dateStart, DateTime dateEnd) throws Exception {
        ...
        String command = "SELECT medicationpat.MedicationPatNum,medicationpat.PatNum,medicationpat.DateStart,medicationpat.DateStop,medicationpat.PatNote," + "(CASE WHEN medication.RxCui IS NULL THEN medicationpat.RxCui ELSE medication.RxCui END) AS RxCui " + "FROM medicationpat " + "LEFT JOIN medication ON medication.MedicationNum=medicationpat.MedicationNum " + "WHERE (medication.RxCui IS NOT NULL OR medicationpat.RxCui>0) " + "AND DATE(medicationpat.DateStart) BETWEEN " + POut.date(dateStart) + " AND " + POut.date(dateEnd) + " ";
        ...
        if (!StringSupport.equals(rxcuiCodes, ""))
        {
        ...
        for (int i = 0;i < tableAllMedPats.Rows.Count;i++)
        {
            ...
            ehrMedPatCur.Description = descript;
            //description either from rxnorm table or, if not in table, default to EhrCode object description
            ...
            {
                retval.Add(ehrMedPatCur.PatNum, new List<EhrCqmMedicationPat>());
            } 
        }
        return retval;
    }
    ...
    */
    private static Dictionary<long, List<EhrCqmNotPerf>> getNotPerformeds(List<long> listPatNums, List<String> listValueSetOIDs, List<String> listReasonOIDs, DateTime dateStart, DateTime dateEnd) throws Exception {
        ...
        if (!StringSupport.equals(itemCodes, ""))
        {
            command += "AND ehrnotperformed.CodeValue IN(" + itemCodes + ") ";
        }
         
        if (!StringSupport.equals(reasonCodes, ""))
        {
            command += "AND ehrnotperformed.CodeValueReason IN(" + reasonCodes + ") ";
        }
         
        command += "GROUP BY ehrnotperformed.EhrNotPerformedNum " + "ORDER BY ehrnotperformed.PatNum,ehrnotperformed.DateEntry DESC";
        //just in case a code was in one of the code system tables more than once, should never happen
        ...
        for (int i = tableNotPerfs.Rows.Count - 1;i > -1;i--)
        {
            for (int j = 0;j < listItems.Count;j++)
            {
                ...
                if (tableNotPerfs.Rows[i]["CodeValue"].ToString() == listItems[j].CodeValue && tableNotPerfs.Rows[i]["CodeSystem"].ToString() == listItems[j].CodeSystem)
                {
            ...
            {
                if (tableNotPerfs.Rows[i]["CodeValueReason"].ToString() == listReasons[j].CodeValue && tableNotPerfs.Rows[i]["CodeSystemReason"].ToString() == listReasons[j].CodeSystem)
                {
        ...
        for (int i = 0;i < tableNotPerfs.Rows.Count;i++)
        {
            ...
            if (StringSupport.equals(descript, ""))
            {
                //just in case not found in table, will default to description of EhrCode object
            ...
            if (StringSupport.equals(reasonDescript, ""))
            {
                //just in case not found in table, will default to description of EhrCode object
            ...
            {
                retval.Add(ehrNotPerfCur.PatNum, new List<EhrCqmNotPerf>());
            } 
        }
        return retval;
    }
    ...
    /**
    * Get all problems that started before or during the date range, that have a code that belong to the value sets in listProbOIDs, and that either have no stop date or the stop date is after dateStart.
    */
    private static Dictionary<long, List<EhrCqmProblem>> getProblems(List<long> listPatNums, List<String> listValueSetOIDs, DateTime dateStart, DateTime dateEnd) throws Exception {
        ...
        {
            if (StringSupport.equals(listValidProbs[i].CodeSystem, "ICD9CM"))
            {
                if (!StringSupport.equals(icd9CodeList, ""))
                {
                    icd9CodeList += ",";
                }
            ...
            else if (StringSupport.equals(listValidProbs[i].CodeSystem, "ICD10CM"))
            {
                if (!StringSupport.equals(icd10CodeList, ""))
                {
                    icd10CodeList += ",";
                }
            ...
            else if (StringSupport.equals(listValidProbs[i].CodeSystem, "SNOMEDCT"))
            {
                if (!StringSupport.equals(snomedCodeList, ""))
                {
                    snomedCodeList += ",";
                }
        ...
        String command = "SELECT disease.DiseaseNum,disease.PatNum,disease.DateStart,disease.DateStop," + "diseasedef.SnomedCode,diseasedef.ICD9Code,diseasedef.Icd10Code," + "COALESCE(snomed.Description,icd9.Description,icd10.Description,diseasedef.DiseaseName) AS Description " + "FROM disease INNER JOIN diseasedef ON disease.DiseaseDefNum=diseasedef.DiseaseDefNum " + "LEFT JOIN snomed ON snomed.SnomedCode=diseasedef.SnomedCode " + "LEFT JOIN icd9 ON icd9.ICD9Code=diseasedef.ICD9Code " + "LEFT JOIN icd10 ON icd10.Icd10Code=diseasedef.Icd10Code " + "WHERE disease.DateStart<=" + POut.date(dateEnd) + " " + "AND (YEAR(disease.DateStop)<1880 OR disease.DateStop>" + POut.date(dateStart) + ") ";
        if (listPatNums != null && listPatNums.Count > 0)
        {
            command += "AND disease.PatNum IN(" + String.Join(",", listPatNums) + ") ";
        }
         
        if (!StringSupport.equals(icd9CodeList, "") || !StringSupport.equals(icd10CodeList, "") || !StringSupport.equals(snomedCodeList, ""))
        {
            command += "AND (";
            if (!StringSupport.equals(icd9CodeList, ""))
            {
            ...
            if (!StringSupport.equals(snomedCodeList, ""))
            {
                if (!StringSupport.equals(icd9CodeList, "") || !StringSupport.equals(icd10CodeList, ""))
                {
                    command += "OR ";
                }
        ...
        {
            for (int j = 0;j < listValidProbs.Count;j++)
            {
                //all problems are either SNOMED, ICD9, or ICD10 codes
                if ((StringSupport.equals(listValidProbs[j].CodeSystem, "SNOMEDCT") && tableAllProbs.Rows[i]["SnomedCode"].ToString() == listValidProbs[j].CodeValue) || (StringSupport.equals(listValidProbs[j].CodeSystem, "ICD9CM") && tableAllProbs.Rows[i]["ICD9Code"].ToString() == listValidProbs[j].CodeValue) || (StringSupport.equals(listValidProbs[j].CodeSystem, "ICD10CM") && tableAllProbs.Rows[i]["Icd10Code"].ToString() == listValidProbs[j].CodeValue))
                {
                    dictDiseaseNumEhrCode.Add(PIn.Long(tableAllProbs.Rows[i]["DiseaseNum"].ToString()), listValidProbs[j]);
                    break;
                }
        ...
        {
            //link the problem to the EhrCode object for retrieving information
            ...
            ehrProblemCur.CodeValue = ehrCodeCur.CodeValue;
            //use the code value from the ehrcode object because diseasedef can have an ICD9CM, ICD10CM, and SNOMEDCT code, and the codes do not have to be for the same thing, so use the code that belongs to the ValueSetOID that makes it valid for this measure
            ...
            {
                retval.Add(ehrProblemCur.PatNum, new List<EhrCqmProblem>());
            } 
        }
        return retval;
    }
    ...
    /**
    * Get all medication documented procedures that happened in the date range that belong to the value set OIDs.  These 'procedures' are actually in the ehrmeasureevent table and can only possibly be one code (restricted by value set OID), SNOMEDCT - 428191000124101 - Documentation of current medications (procedure).  Ordered by PatNum, DateTEvent DESC for making CQM calc easier, most recent 'procedure' will be the first one found for the patient in list.
    */
    private static Dictionary<long, List<EhrCqmMeasEvent>> getMedDocumentedProcs(List<long> listPatNums, List<String> listValueSetOIDs, DateTime dateStart, DateTime dateEnd) throws Exception {
        Dictionary<long, List<EhrCqmMeasEvent>> retval = new Dictionary<long, List<EhrCqmMeasEvent>>();
        //if no patients, return a new empty dictionary
        if (listPatNums != null && listPatNums.Count == 0)
        {
        ...
        String command = "SELECT ehrmeasureevent.*,COALESCE(snomed.Description,'') AS Description " + "FROM ehrmeasureevent " + "LEFT JOIN snomed ON snomed.SnomedCode=ehrmeasureevent.CodeValueEvent AND ehrmeasureevent.CodeSystemEvent='SNOMEDCT' " + "WHERE EventType=" + POut.int(((Enum)EhrMeasureEventType.CurrentMedsDocumented).ordinal()) + " " + "AND DATE(DateTEvent) BETWEEN " + POut.date(dateStart) + " AND " + POut.date(dateEnd) + " ";
        if (listPatNums != null && listPatNums.Count > 0)
        {
            command += "AND ehrmeasureevent.PatNum IN(" + String.Join(",", listPatNums) + ") ";
        }
        ...
        for (int i = tableEvents.Rows.Count - 1;i > -1;i--)
        {
            ...
            for (int j = 0;j < listEhrCodes.Count;j++)
            {
                //currently this can only be one code, SNOMEDCT - 428191000124101, but we will treat it like a list in case that changes
        ...
        for (int i = 0;i < tableEvents.Rows.Count;i++)
        {
            ...
            {
                retval.Add(ehrProcCur.PatNum, new List<EhrCqmMeasEvent>());
            } 
        }
        return retval;
    }
    ...
    /**
    * Used in measure 69, BMI Screening and Follow-up and measure 155, Weight Assessment and Counseling for Nutrition and Physical Activity for Children and Adolescents.  Get all vitalsigns with DateTaken in the date range with valid height and weight.  Only one code available for a BMI exam - LOINC 39156-5 Body mass index (BMI) [Ratio].  Any vitalsign object with valid height and weight is assumed to be a LOINC 39156-5, not stored explicitly.  Results ordered by PatNum then DateTaken DESC, so MOST RECENT for each patient will be the first one in the list for that pat (i.e. dict[PatNum][0]).
    */
    private static Dictionary<long, List<EhrCqmVitalsign>> getVitalsignsForBMI(List<long> listPatNums, DateTime dateStart, DateTime dateEnd) throws Exception {
        ...
        String command = "SELECT * FROM vitalsign " + "WHERE DATE(DateTaken) BETWEEN " + POut.date(dateStart) + " AND " + POut.date(dateEnd) + " " + "AND vitalsign.Height>0 AND vitalsign.Weight>0 ";
        if (listPatNums != null && listPatNums.Count > 0)
        {
        ...
        for (int i = 0;i < listVitalsigns.Count;i++)
        {
            ...
            ehrVitalsignCur.BMI = Math.Round((double)((w * 703) / (h * h)), 2, MidpointRounding.AwayFromZero);
            ...
            ehrVitalsignCur.BMIExamCode = listVitalsigns[i].BMIExamCode;
            //percentile code
            ...
            {
                retval.Add(ehrVitalsignCur.PatNum, new List<EhrCqmVitalsign>());
            } 
        }
        return retval;
    }
    ...
    /**
    * Used in measure 165, Controlling High Blood Pressure.  Get all vitalsigns with DateTaken in the date range with valid BP.  Only one code available for Systolic BP, LOINC 8480-6, and one code for Diastolic, LOINC 8462-4.  Results ordered by PatNum then DateTaken DESC, so MOST RECENT for each patient will be the first one in the list for that pat (i.e. dict[PatNum][0]).
    */
    private static Dictionary<long, List<EhrCqmVitalsign>> getVitalsignsForBP(List<long> listPatNums, DateTime dateStart, DateTime dateEnd) throws Exception {
        Dictionary<long, List<EhrCqmVitalsign>> retval = new Dictionary<long, List<EhrCqmVitalsign>>();
        //if no patients, return a new empty dictionary
        if (listPatNums != null && listPatNums.Count == 0)
        {
        ...
        String command = "SELECT * FROM vitalsign " + "WHERE DATE(DateTaken) BETWEEN " + POut.date(dateStart) + " AND " + POut.date(dateEnd) + " " + "AND vitalsign.BpSystolic>0 AND vitalsign.BpDiastolic>0 ";
        if (listPatNums != null && listPatNums.Count > 0)
        {
            command += "AND vitalsign.PatNum IN(" + String.Join(",", listPatNums) + ") ";
        }
        ...
        for (int i = 0;i < listVitalsigns.Count;i++)
        {
            ...
            ehrVitalsignCur.EhrCqmVitalsignNum = listVitalsigns[i].VitalsignNum;
            ehrVitalsignCur.PatNum = listVitalsigns[i].PatNum;
            ehrVitalsignCur.BpSystolic = listVitalsigns[i].BpSystolic;
            //LOINC 8480-6
            ehrVitalsignCur.BpDiastolic = listVitalsigns[i].BpDiastolic;
            //LOINC 8462-4
            ehrVitalsignCur.DateTaken = listVitalsigns[i].DateTaken;
            if (retval.ContainsKey(ehrVitalsignCur.PatNum))
            {
    ...
    /**
    * Get all procedures with ProcDate in the date range and ProcCode in the list of codes that belong to one of the value sets in listValueSetOIDs.
    */
    private static Dictionary<long, List<EhrCqmProc>> getProcs(List<long> listPatNums, List<String> listValueSetOIDs, DateTime dateStart, DateTime dateEnd) throws Exception {
        ...
        String command = "SELECT procedurelog.ProcNum,procedurelog.PatNum,procedurelog.ProvNum,procedurelog.ProcDate," + "procedurecode.ProcCode,procedurecode.Descript FROM procedurelog " + "INNER JOIN procedurecode ON procedurelog.CodeNum=procedurecode.CodeNum " + "WHERE procedurelog.ProcStatus=2 " + "AND procedurelog.ProcDate BETWEEN " + POut.date(dateStart) + " AND " + POut.date(dateEnd) + " ";
        if (listPatNums != null && listPatNums.Count > 0)
        {
            command += "AND procedurelog.PatNum IN (" + String.Join(",", listPatNums) + ")";
        }
         
        if (!StringSupport.equals(codeList, ""))
        {
        ...
        for (int i = 0;i < tableAllProcs.Rows.Count;i++)
        {
            ...
            {
                retval.Add(ehrProcCur.PatNum, new List<EhrCqmProc>());
            } 
        }
        return retval;
    }
    ...
    /**
    * Returns all vaccinepat objects for pneumonia and influenza CQMs.  These are basically just medicationpat objects, so we will use the same object with two optional fields to identify them as vaccinepats instead of medicationpats.  The isGiven bool set to true will return only those with the CompletionStatus=0 (complete).  If isGiven=false, all will be returned and the logic to determine whether it was given or not will take place in calculation.  The only time we want vaccines with a status other than complete is when we are looking for vaccines NotAdministered in the influenza vaccine measure.  These NotAdministered vaccines will be due to an intolerance or allergy and entered as such.
    */
    private static Dictionary<long, List<EhrCqmMedicationPat>> getVaccines(List<long> listPatNums, List<String> listValueSetOIDs, DateTime dateStart, DateTime dateEnd, boolean isGiven) throws Exception {
        Dictionary<long, List<EhrCqmMedicationPat>> retval = new Dictionary<long, List<EhrCqmMedicationPat>>();
        //if no patients, return a new empty dictionary
        if (listPatNums != null && listPatNums.Count == 0)
        {
        ...
        String command = "SELECT vaccinepat.VaccinePatNum,vaccinepat.PatNum,vaccinepat.DateTimeStart,vaccinepat.DateTimeEnd,vaccinedef.CVXCode,vaccinepat.CompletionStatus " + "FROM vaccinepat " + "INNER JOIN vaccinedef ON vaccinepat.VaccineDefNum=vaccinedef.VaccineDefNum " + "WHERE DATE(vaccinepat.DateTimeStart) BETWEEN " + POut.date(dateStart) + " AND " + POut.date(dateEnd) + " ";
        if (isGiven)
        {
            command += "AND vaccinepat.CompletionStatus=0 ";
        }
        else
        {
            //CompletionStatus=0 (complete)
            command += "AND vaccinepat.CompletionStatus IN(0,2) ";
        } 
        //CompletionStatus=0 (complete) or 2 (NotAdministered), we do not care about refused or partially administered for our measures
        if (listPatNums != null && listPatNums.Count > 0)
        {
        ...
        List<EhrCode> listEhrCodes = EhrCodes.GetForValueSetOIDs(listValueSetOIDs, false);
        //this list will only contain one code, CVX 33 - pneumococcal polysaccharide vaccine, 23 valent
        ...
        for (int i = 0;i < tableAllVaccinePats.Rows.Count;i++)
        {
            ...
            ehrVacPatCur.Description = descript;
            //description either from cvx table or, if not in table, default to EhrCode object description
            ...
            {
                retval.Add(ehrVacPatCur.PatNum, new List<EhrCqmMedicationPat>());
            } 
        }
        return retval;
    }
    ...
    /**
    * Using the data in alldata, determine if the patients in alldata.ListEhrPats are in the 'Numerator', 'Exclusion', or 'Exception' category for this measure and enter an explanation if applicable.  All of the patients in ListEhrPats are the initial patient population (almost always equal to the Denominator).
    */
    private static void classifyPatients(QualityMeasure alldata, QualityType2014 qtype, DateTime dateStart, DateTime dateEnd) throws Exception {
        switch(qtype)
        {
            case MedicationsEntered: 
                ...
                for (Object __dummyForeachVar3 : alldata.DictPatNumListEncounters)
                {
                    //used to indicate that the encounter has been included in the numerator, so do not try to apply exception
                    KeyValuePair<long, List<EhrCqmEncounter>> kvpair = (KeyValuePair<long, List<EhrCqmEncounter>>)__dummyForeachVar3;
                    ...
                    for (int i = 0;i < kvpair.Value.Count;i++)
                    {
                        ...
                        //for every encounter for this patient
                        isClassified = false;
                        for (int j = 0;j < listMedsDocProcsCur.Count;j++)
                        {
                            //this might be an empty list if no meds documented procs for this patient
                            if (kvpair.Value[i].DateEncounter.Date == listMedsDocProcsCur[j].DateTEvent.Date)
                            {
                                //if meds documented proc on the same day as the encounter, then numerator
                                isClassified = true;
                        ...
                        for (int j = 0;j < listNotPerfCur.Count;j++)
                        {
                            //don't try to apply exception criteria if already in the numerator
                            //if encounter is not in the numerator, try to apply the exception criteria
                        ...
                        //it is an exception encounter, move to next encounter
            ...
            case WeightOver65: 
                for (int i = 0;i < alldata.ListEhrPats.Count;i++)
                {
                    ...
                    for (int j = 0;j < listProbsCur.Count;j++)
                    {
                        if (StringSupport.equals(listProbsCur[j].ValueSetOID, "2.16.840.1.113883.3.600.1.1623"))
                        {
                            //Pregnancy Dx Grouping Value Set
                    ...
                    if (listVitalsignsCur.Count == 0)
                    {
                        //no vitalsign exams within 6 months of start of measurement period, but encounters exist or they would not be in the IPP
                    ...
                    {
                        if (dateMostRecentExam.Date > listVitalsignsCur[j].DateTaken.Date)
                        {
                            continue;
                        }
                        ...
                        {
                            //most recent exam date already set and set to a date more recent than current, continue
                            if (listVitalsignsCur[j].DateTaken.Date <= listEncsCur[k].DateEncounter.Date && listVitalsignsCur[j].DateTaken.Date >= listEncsCur[k].DateEncounter.AddMonths(-6).Date && listVitalsignsCur[j].DateTaken.Date >= dateMostRecentExam.Date)
                            {
                    ...
                    if (qtype == QualityType2014.WeightOver65 && listVitalsignsCur[indexMostRecentExam].BMI >= 23m && listVitalsignsCur[indexMostRecentExam].BMI < 30m)
                    {
                        //'m' converts to decimal
                    ...
                    if (qtype == QualityType2014.WeightAdult && listVitalsignsCur[indexMostRecentExam].BMI >= 18.5m && listVitalsignsCur[indexMostRecentExam].BMI < 25m)
                    {
                        //'m' converts to decimal
                        alldata.ListEhrPats[i].IsNumerator = true;
                        alldata.ListEhrPats[i].Explanation = "BMI in normal range.  Most recent BMI exam date: " + listVitalsignsCur[indexMostRecentExam].DateTaken.ToShortDateString() + ".  BMI result: " + listVitalsignsCur[indexMostRecentExam].BMI.ToString();
                    ...
                    for (int j = 0;j < listEncsCur.Count;j++)
                    {
                        ...
                        if (listEncsCur[j].DateEncounter.Date < listVitalsignsCur[indexMostRecentExam].DateTaken.Date || listEncsCur[j].DateEncounter.Date > listVitalsignsCur[indexMostRecentExam].DateTaken.AddMonths(6).Date)
                        {
                            continue;
                        }
                        ...
                        for (int k = 0;k < listInterventionsCur.Count;k++)
                        {
                            ...
                            if (listInterventionsCur[k].DateEntry.Date <= listEncsCur[j].DateEncounter.Date && listInterventionsCur[k].DateEntry.Date >= listEncsCur[j].DateEncounter.AddMonths(-6).Date)
                            {
                        ...
                        for (int k = 0;k < listMedPatsCur.Count;k++)
                        {
                            ...
                            if (listMedPatsCur[k].DateStart.Date <= listEncsCur[j].DateEncounter.Date && listMedPatsCur[k].DateStart.Date >= listEncsCur[j].DateEncounter.AddMonths(-6).Date)
                            {
                                alldata.ListEhrPats[i].IsNumerator = true;
                                alldata.ListEhrPats[i].Explanation = "Most recent exam on " + listVitalsignsCur[indexMostRecentExam].DateTaken.ToShortDateString() + " with encounter on " + listEncsCur[j].DateEncounter.ToShortDateString() + " resulted in a BMI of " + listVitalsignsCur[indexMostRecentExam].BMI.ToString() + " " + "and medication order on " + listMedPatsCur[k].DateStart.ToShortDateString() + ".";
                                isCategorized = true;
                                break;
                            }
                    ...
                    alldata.ListEhrPats[i].Explanation = "Most recent exam on " + listVitalsignsCur[indexMostRecentExam].DateTaken.ToShortDateString() + " with BMI of " + listVitalsignsCur[indexMostRecentExam].BMI.ToString() + " had valid encounters within 6 months of the exam but no valid intervention or medication order.";
                }
                break;
            case CariesPrevent: 
            case CariesPrevent_1: 
            ...
            case CariesPrevent_3: 
                for (int i = 0;i < alldata.ListEhrPats.Count;i++)
                {
                    ...
                    {
                        alldata.ListEhrPats[i].IsNumerator = true;
                        alldata.ListEhrPats[i].Explanation = "This patient had an eligible encounter in the measurement period and a flouride varnish application procedure with code " + alldata.DictPatNumListProcs[patNumCur][0].ProcCode + " on " + alldata.DictPatNumListProcs[patNumCur][0].ProcDate.ToShortDateString() + ".";
                        continue;
                    }
            ...
            case ChildCaries: 
                for (int i = 0;i < alldata.ListEhrPats.Count;i++)
                {
                    ...
                    {
                        alldata.ListEhrPats[i].IsNumerator = true;
                        alldata.ListEhrPats[i].Explanation = "This patient had an eligible encounter in the measurement period and had an active diagnosis of caries with code " + alldata.DictPatNumListProblems[patNumCur][0].CodeValue + " that started on " + alldata.DictPatNumListProblems[patNumCur][0].DateStart.ToShortDateString() + ".";
                        continue;
                    }
            ...
            case Pneumonia: 
                for (int i = 0;i < alldata.ListEhrPats.Count;i++)
                {
                    ...
                    {
                        if (StringSupport.equals(listMedPatsCur[j].ValueSetOID, "2.16.840.1.113883.3.464.1003.110.12.1027"))
                        {
                            //Pneumococcal Vaccine Grouping Value Set
                            alldata.ListEhrPats[i].IsNumerator = true;
                    ...
                    {
                        if (StringSupport.equals(listProcsCur[j].ValueSetOID, "2.16.840.1.113883.3.464.1003.110.12.1034"))
                        {
                            //Pneumococcal Vaccine Administered Grouping Value Set
                            alldata.ListEhrPats[i].IsNumerator = true;
                    ...
                    for (int j = 0;j < listProbsCur.Count;j++)
                    {
                        if (StringSupport.equals(listProbsCur[j].ValueSetOID, "2.16.840.1.113883.3.464.1003.110.12.1028"))
                        {
                            //History of Pneumococcal Vaccine Grouping Value Set
                            alldata.ListEhrPats[i].IsNumerator = true;
            ...
            case TobaccoCessation: 
                for (int i = 0;i < alldata.ListEhrPats.Count;i++)
                {
                    ...
                    if (mostRecentAssessDate > DateTime.MinValue && StringSupport.equals(mostRecentAssessValueSetOID, "2.16.840.1.113883.3.526.3.1189"))
                    {
                        //Non-User
                        alldata.ListEhrPats[i].IsNumerator = true;
                    ...
                    if (mostRecentAssessDate > DateTime.MinValue && StringSupport.equals(mostRecentAssessValueSetOID, "2.16.840.1.113883.3.526.3.1170"))
                    {
                        //User
                        ...
                        if (listMedPatsCur.Count > 0)
                        {
                            ...
                            if (!StringSupport.equals(listMedPatsCur[0].PatNote, ""))
                            {
                                //PatNote means Medication Order, otherwise Medication Active
                                activeOrOrder = "order";
                            }
                    ...
                    else if (StringSupport.equals(mostRecentAssessValueSetOID, "2.16.840.1.113883.3.526.3.1170"))
                    {
                        //User
            ...
            case Influenza: 
                for (int i = 0;i < alldata.ListEhrPats.Count;i++)
                {
                    ...
                    if (!alldata.ListEhrPats[i].IsDenominator)
                    {
                        continue;
                    }
                     
                    //if not in denominator, they will not show in grid of FormEhrQualityMeasureEdit2014, skip this patient, only in IPP
                    long patNumCur = alldata.ListEhrPats[i].EhrCqmPat.PatNum;
                    boolean isCategorized = false;
                    ...
                    {
                        if (!StringSupport.equals(listAllEncsCur[j].ValueSetOID, "2.16.840.1.113883.3.526.3.1252"))
                        {
                            continue;
                        }
                    ...
                    {
                        //Hemodialysis Grouping Value Set
                        if (!StringSupport.equals(listAllProcsCur[j].ValueSetOID, "2.16.840.1.113883.3.526.3.1083") && !StringSupport.equals(listAllProcsCur[j].ValueSetOID, "2.16.840.1.113883.3.526.3.1084"))
                        {
                            continue;
                        }
                    ...
                    for (int j = 0;j < listAllProcsCur.Count;j++)
                    {
                        ...
                        if (!StringSupport.equals(listAllProcsCur[j].ValueSetOID, "2.16.840.1.113883.3.526.3.402"))
                        {
                            continue;
                        }
                    ...
                    for (int j = 0;j < listAllMedPatsCur.Count;j++)
                    {
                        ...
                        //already only consists of meds from value set 2.16.840.1.113883.3.526.3.1254 - Influenza Vaccine Grouping Value Set
                        if (listAllMedPatsCur[j].CompletionStatus == VaccineCompletionStatus.NotAdministered)
                        {
                            continue;
                        }
                        ...
                        for (int k = 0;k < listOccurrenceADates.Count;k++)
                        {
                            //status other than complete, skip
                    ...
                    for (int j = 0;j < listAllProbsCur.Count;j++)
                    {
                        ...
                        if (!StringSupport.equals(listAllProbsCur[j].ValueSetOID, "2.16.840.1.113883.3.526.3.1185"))
                        {
                            continue;
                        }
                    ...
                    for (int j = 0;j < listAllProbsCur.Count;j++)
                    {
                        ...
                        if (!StringSupport.equals(listAllProbsCur[j].ValueSetOID, "2.16.840.1.113883.3.526.3.1255"))
                        {
                            continue;
                        }
                    ...
                    for (int j = 0;j < listAllProbsCur.Count;j++)
                    {
                        ...
                        //Allergy to eggs Grouping Value Set
                        //Allergy to Influenza Vaccine Grouping Value Set
                        if (!StringSupport.equals(listAllProbsCur[j].ValueSetOID, "2.16.840.1.113883.3.526.3.1253") && !StringSupport.equals(listAllProbsCur[j].ValueSetOID, "2.16.840.1.113883.3.526.3.1256") && !StringSupport.equals(listAllProbsCur[j].ValueSetOID, "2.16.840.1.113883.3.526.3.1257"))
                        {
                            continue;
                        }
                        ...
                        for (int k = 0;k < listOccurrenceADates.Count;k++)
                        {
                            //Intolerance to Influenza Vaccine Grouping Value Set
                    ...
                    for (int j = 0;j < listAllMedPatsCur.Count;j++)
                    {
                        if (listAllMedPatsCur[j].CompletionStatus == VaccineCompletionStatus.Complete)
                        {
                            continue;
                        }
                        ...
                        for (int k = 0;k < listOccurrenceADates.Count;k++)
                        {
                            //Only looking for NotAdministered, if status=Complete, skip
            ...
            case WeightChild_3_1: 
                for (int i = 0;i < alldata.ListEhrPats.Count;i++)
                {
                    ...
                    for (int j = 0;j < listProbsCur.Count;j++)
                    {
                        if (StringSupport.equals(listProbsCur[j].ValueSetOID, "2.16.840.1.113883.3.526.3.378"))
                        {
                            //Pregnancy Grouping Value Set
                            alldata.ListEhrPats[i].IsExclusion = true;
                            alldata.ListEhrPats[i].Explanation = "The patient had a pregnancy diagnosis that started on " + listProbsCur[j].DateStart.ToShortDateString() + " that is either still active or ended after the start of the measurement period.";
                    ...
                    List<EhrCode> listBMIExamCodes = EhrCodes.GetForValueSetOIDs(new List<String>(), false);
                    //BMI percentile Grouping Value Set
                    List<EhrCode> listHeightExamCodes = EhrCodes.GetForValueSetOIDs(new List<String>(), false);
                    //Height Grouping Value Set
                    List<EhrCode> listWeightExamCodes = EhrCodes.GetForValueSetOIDs(new List<String>(), false);
                    for (int j = 0;j < listVitalsignsCur.Count;j++)
                    {
                        //Weight Grouping Value Set
                        //loop through vitalsign exams looking for valid height, weight, and BMI exam codes (percentile) based on value set OIDs
                        boolean isBMIExamCodeValid = false;
                        boolean isHeightExamCodeValid = false;
                        boolean isWeightExamCodeValid = false;
                        for (int k = 0;k < listBMIExamCodes.Count;k++)
                        {
                            ...
                            if (listVitalsignsCur[j].BMIPercentile == -1)
                            {
                                continue;
                            }
                             
                            //-1 if not in age range or if BMI percentile was not calculated correctly
            ...
            case WeightChild_3_3: 
                ...
                String interventionType = "physical activity";
                //used for generating explanation string, 1_3, 2_3, and 3_3 are phys activity
                String valueSetCur = "2.16.840.1.113883.3.464.1003.118.12.1035";
                //Counseling for Physical Activity Grouping Value Set
                if (qtype == QualityType2014.WeightChild_1_2 || qtype == QualityType2014.WeightChild_2_2 || qtype == QualityType2014.WeightChild_3_2)
                {
                    ...
                    valueSetCur = "2.16.840.1.113883.3.464.1003.195.12.1003";
                }
                ...
                for (int i = 0;i < alldata.ListEhrPats.Count;i++)
                {
                    ...
                    for (int j = 0;j < listIntervenCur.Count;j++)
                    {
                        ...
                        if (StringSupport.equals(listIntervenCur[j].ValueSetOID, valueSetCur))
                        {
                            alldata.ListEhrPats[i].IsNumerator = true;
            ...
            case BloodPressureManage: 
                for (int i = 0;i < alldata.ListEhrPats.Count;i++)
                {
                    ...
                    for (int j = 0;j < listProbsCur.Count;j++)
                    {
                        ...
                        if (StringSupport.equals(listProbsCur[j].ValueSetOID, "2.16.840.1.113883.3.526.3.353"))
                        {
                            //End Stage Renal Disease Grouping Value Set
                            alldata.ListEhrPats[i].IsExclusion = true;
                        ...
                        if (StringSupport.equals(listProbsCur[j].ValueSetOID, "2.16.840.1.113883.3.526.3.1002"))
                        {
                            //Chronic Kidney Disease, Stage 5 Grouping Value Set
                            alldata.ListEhrPats[i].IsExclusion = true;
                    ...
                    {
                        if (StringSupport.equals(listEncountersCur[j].ValueSetOID, "2.16.840.1.113883.3.464.1003.109.12.1014"))
                        {
                            //ESRD Monthly Outpatient Services Grouping Value Set
                            alldata.ListEhrPats[i].IsExclusion = true;
                    ...
                    {
                        alldata.ListEhrPats[i].IsNumerator = true;
                        alldata.ListEhrPats[i].Explanation = "The patient had a vitalsign exam on " + dateMostRecentEnc.ToShortDateString() + " with systolic blood pressure " + recentSystolic.ToString() + " mmHg and diastolic blood pressure " + recentDiastolic.ToString() + " mmHg.";
                        continue;
                    }
                    ...
                    if (recentSystolic == 0 || recentDiastolic == 0)
                    {
                        //BP was not recorded during most recent encounter
                    ...
                    {
                        alldata.ListEhrPats[i].Explanation += " had a corresponding vitalsign exam with systolic blood pressure " + recentSystolic.ToString() + " mmHg and diastolic blood pressure " + recentDiastolic.ToString() + " mmHg.";
                    } 
                }
                break;
            default: 
    ...
    /**
    * Just counts up the number of rows with an X in the numerator column.  Very simple.
    */
    public static int calcNumerator(DataTable table) throws Exception {
        ...
        {
            if (StringSupport.equals(table.Rows[i]["numerator"].ToString(), "X"))
            {
                retVal++;
            }
    ...
    /**
    * Just counts up the number of EhrPatients with IsNumerator=true.
    */
    private static int calcNumerator2014(List<EhrCqmPatient> listPats) throws Exception {
        int retval = 0;
    ...
    /**
    * Just counts up the number of EhrPatients with IsDenominator=true.  The only measure that may have some patients in the initial patient population that are not in the denominator is the influenza vaccine measure, CMS147v2.
    */
    private static int calcDenominator2014(List<EhrCqmPatient> listPats) throws Exception {
        int retval = 0;
        for (int i = 0;i < listPats.Count;i++)
        {
    ...
    /**
    * This is only used by measure 68, current meds documented.  This is the only measure that is not patient based, but encounter based.  So every eligible encounter is the denominator/ipp.  We will have to handle this case carefully.
    */
    private static int calcDenominator2014_Encs(Dictionary<long, List<EhrCqmEncounter>> dictPatNumListEncs) throws Exception {
        int retval = 0;
        for (Object __dummyForeachVar4 : dictPatNumListEncs)
        {
            KeyValuePair<long, List<EhrCqmEncounter>> kvpair = (KeyValuePair<long, List<EhrCqmEncounter>>)__dummyForeachVar4;
            for (int i = 0;i < kvpair.Value.Count;i++)
            {
                retval++;
            }
    ...
    /**
    * Just counts up the number of rows with an X in the exclusion column.  Very simple.
    */
    public static int calcExclusions(DataTable table) throws Exception {
        //No need to check RemotingRole; no call to db.
        int retVal = 0;
        for (int i = 0;i < table.Rows.Count;i++)
        {
            if (StringSupport.equals(table.Rows[i]["exclusion"].ToString(), "X"))
            {
                retVal++;
            }
    ...
    /**
    * Just counts up the number of EhrPatients with IsException=true.
    */
    private static int calcExceptions2014(List<EhrCqmPatient> listPats) throws Exception {
        int retval = 0;
        for (int i = 0;i < listPats.Count;i++)
        {
    ...
    /**
    * This is only used by measure 68, current meds documented.  This is the only measure that is not patient based, but encounter based.  Simply loop through all encounters for all patients in the dictionary and count up the encounters with IsNumerator=true.
    */
    private static int calcNumerator2014_Encs(Dictionary<long, List<EhrCqmEncounter>> dictPatNumListEncs) throws Exception {
        int retval = 0;
        for (Object __dummyForeachVar5 : dictPatNumListEncs)
        {
            KeyValuePair<long, List<EhrCqmEncounter>> kvpair = (KeyValuePair<long, List<EhrCqmEncounter>>)__dummyForeachVar5;
            for (int i = 0;i < kvpair.Value.Count;i++)
            {
    ...
    /**
    * This is only used by measure 68, current meds documented.  This is the only measure that is not patient based, but encounter based.  Simply loop through all encounters for all patients in the dictionary and count up the encounters with IsException=true.
    */
    private static int calcException2014_Encs(Dictionary<long, List<EhrCqmEncounter>> dictPatNumListEncs) throws Exception {
        int retval = 0;
        for (Object __dummyForeachVar6 : dictPatNumListEncs)
        {
            KeyValuePair<long, List<EhrCqmEncounter>> kvpair = (KeyValuePair<long, List<EhrCqmEncounter>>)__dummyForeachVar6;
            for (int i = 0;i < kvpair.Value.Count;i++)
            {
    ...
    /**
    * Just counts up the number of EhrPatients with IsExclusion=true.
    */
    private static int calcExclusions2014(List<EhrCqmPatient> listPats) throws Exception {
        int retval = 0;
        for (int i = 0;i < listPats.Count;i++)
        {
    ...
    private static String getDenominatorExplain(QualityType qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case WeightOver65: 
    ...
    private static String getNumeratorExplain(QualityType qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case WeightOver65: 
    ...
    private static String getExclusionsExplain(QualityType qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case WeightOver65: 
    ...
    private static String getDenominatorExplain2014(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    private static String getNumeratorExplain2014(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    /**
    * The exclusions and exceptions are very similar, in fact no measure from our set of 9 CQM's has both an exclusion and an exception.  Possibly combine these into one text box called "Exclusions/Exceptions" to save space on FormQualityMeasureEdit2014.  We will have to report them in the correctly labeled section in our QRDA III report, so we probably have to keep them in separate functions.  The difference between them is when to take each into account in calculating measure.  Find denominator, apply exclusions, then classify using numerator criteria, then only if not in numerator apply exceptions and subtract from denominator.  Exceptions only apply if the patient/encounter doesn't meet the numerator criteria, don't immediately subtract from the denominator population.
    */
    private static String getExclusionsExplain2014(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    private static String getExceptionsExplain2014(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    private static String getEMeasureTitle(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    //: age 6 to 12
    //: age 13 to 20
    //: Height, Weight, and BMI: age 3 to 16
    //: Height, Weight, and BMI: age 3 to 11
    //: Height, Weight, and BMI: age 12 to 16
    //: Counseled for Nutrition: age 3 to 16
    //: Counseled for Nutrition: age 3 to 11
    //: Counseled for Nutrition: age 12 to 16
    //: Counseled for Physical Activity: age 3 to 16
    //: Counseled for Physical Activity: age 3 to 11
    //: Counseled for Physical Activity: age 12 to 16
    private static String getEMeasureNum(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    private static String getEMeasureVSpecificId(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    private static String getEMeasureVNeutralId(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    private static String getEMeasureVersion(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    private static String getEMeasureSetId(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    private static String getEMeasureIppId(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    private static String getEMeasureDenomId(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    private static String getEMeasureDenexId(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    private static String getEMeasureDenexcepId(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    private static String getEMeasureNumerId(QualityType2014 qtype) throws Exception {
        //No need to check RemotingRole; no call to db.
        switch(qtype)
        {
            case MedicationsEntered: 
    ...
    /**
    * listQMs will be a list of 9 QualityMeasure objects for the selected CQMs.
    ...
    * The Category III document contains the aggregate information for each of the selected measures.
    */
    public static void generateQRDA(List<QualityMeasure> listQMs, long provNum, DateTime dateStart, DateTime dateEnd, String folderRoot) throws Exception {
        //Set global variables for use by the GenerateCatOne and GenerateCatThree functions
        if (listQMs == null || listQMs.Count == 0)
        {
        ...
        if (StringSupport.equals(_strOIDInternalCQMRoot, ""))
        {
        ...
        if (StringSupport.equals(_strOIDInternalProvRoot, ""))
        {
        ...
        if (StringSupport.equals(_strOIDInternalPatRoot, ""))
        {
            throw new ApplicationException("Internal Patient OID root is missing.  Go to Setup | EHR and add OIDs.");
        }
         
        _hashQrdaGuids = new HashSet<String>();
        //The GUIDs are only used to uniquely identify the documents themselves, for other IDs we are using the OIDInternal root and extension
        ...
        _listExtraPopIndxs.Add(((Enum)QualityType2014.CariesPrevent_1).ordinal());
        _listExtraPopIndxs.Add(((Enum)QualityType2014.CariesPrevent_2).ordinal());
        _listExtraPopIndxs.Add(((Enum)QualityType2014.CariesPrevent_2).ordinal());
        _listExtraPopIndxs.Add(((Enum)QualityType2014.WeightAdult).ordinal());
        _listExtraPopIndxs.Add(((Enum)QualityType2014.WeightChild_1_2).ordinal());
        _listExtraPopIndxs.Add(((Enum)QualityType2014.WeightChild_1_3).ordinal());
        _listExtraPopIndxs.Add(((Enum)QualityType2014.WeightChild_2_1).ordinal());
        _listExtraPopIndxs.Add(((Enum)QualityType2014.WeightChild_2_2).ordinal());
        _listExtraPopIndxs.Add(((Enum)QualityType2014.WeightChild_2_3).ordinal());
        _listExtraPopIndxs.Add(((Enum)QualityType2014.WeightChild_3_1).ordinal());
        _listExtraPopIndxs.Add(((Enum)QualityType2014.WeightChild_3_2).ordinal());
        _listExtraPopIndxs.Add(((Enum)QualityType2014.WeightChild_3_3).ordinal());
        GenerateQRDACatOne(listQMs, dateStart, dateEnd, folderRoot);
        GenerateQRDACatThree(listQMs, dateStart, dateEnd, folderRoot);
    }

    /**
    * This method generates the entire Category III aggregate information document and places it in the path supplied in folderRoot\QRDA_Category_III.xml.
    */
    private static void generateQRDACatThree(List<QualityMeasure> listQMs, DateTime dateStart, DateTime dateEnd, String folderRoot) throws Exception {
        XmlWriterSettings xmlSettings = new XmlWriterSettings();
        xmlSettings.Encoding = Encoding.UTF8;
        xmlSettings.OmitXmlDeclaration = true;
        xmlSettings.Indent = true;
        xmlSettings.IndentChars = "\t";
        IDisposable __newVar0 = _w = XmlWriter.Create(folderRoot + "\\QRDA_Category_III.xml", xmlSettings);
        try
        {
            {
                ...
                //template id to assert use of the CDA standard
                ...
                startAndEnd("id","root",_strOIDInternalRoot);
                //This is the root assigned to the practice, based on the OD root 2.16.840.1.113883.3.4337
                _w.WriteElementString("name", PrefC.getString(PrefName.PracticeTitle));
                //Validated
                ...
                startAndEnd("id","root","2.16.840.1.113883.4.6","extension",provLegal.NationalProvID,"assigningAuthorityName","NPI");
                //Validated NPI
                start("representedOrganization");
                startAndEnd("id","root",_strOIDInternalRoot);
                //This is the root assigned to the practice, based on the OD root 2.16.840.1.113883.3.4337
                ...
                //PCPR is HL7ActClass code for Care Provision
                ...
                int s1Indx = ((Enum)QualityType2014.CariesPrevent_1).ordinal();
                int s2Indx = ((Enum)QualityType2014.CariesPrevent_2).ordinal();
                int s3Indx = ((Enum)QualityType2014.CariesPrevent_3).ordinal();
                //get the index of the WeightAdult (population 2)
                int pop2Indx = ((Enum)QualityType2014.WeightAdult).ordinal();
                //get the indexes of the additional two numerators and 2 stratifications
                int numer2 = ((Enum)QualityType2014.WeightChild_1_2).ordinal();
                int numer3 = ((Enum)QualityType2014.WeightChild_1_3).ordinal();
                int s1numer1Indx = ((Enum)QualityType2014.WeightChild_2_1).ordinal();
                int s1numer2Indx = ((Enum)QualityType2014.WeightChild_2_2).ordinal();
                int s1numer3Indx = ((Enum)QualityType2014.WeightChild_2_3).ordinal();
                int s2numer1Indx = ((Enum)QualityType2014.WeightChild_3_1).ordinal();
                int s2numer2Indx = ((Enum)QualityType2014.WeightChild_3_2).ordinal();
                int s2numer3Indx = ((Enum)QualityType2014.WeightChild_3_3).ordinal();
                int numPops = 1;
                for (int i = 0;i < listQMs.Count;i++)
                {
                    //if set to two or three, this will generate additional sets of population data (measure 69 has two populations, measure 155 has 3 numerators)
                    ...
                    switch(i)
                    {
                        case ((Enum)QualityType2014.WeightOver65).ordinal(): 
                            //this wil create 2 populations, one for weight ages 18-64 and one for weight 65+
                            numPops = 2;
                            break;
                        case ((Enum)QualityType2014.WeightChild_1_1).ordinal(): 
                    ...
                    {
                        //usually only runs 1 loop, measure 69 has two populations, and measure 155 has 3 numerators, in which case it will run more than once
                        int measureIndx = i;
                        if (numPops == 2)
                        {
                            if (p == 0)
                            ...
                            if (p > 0)
                            {
                                //fill stratification data with supplemental data from other population (only used by measure 69)
                        ...
                        else if (numPops == 3)
                        {
                            ...
                            if (p == 1)
                            {
                                //second iteration, fill dictionaries with numerator 2 data for stratifications
                                measureIndx = numer2;
                                listPopCountDicts = FillDictPopCounts(listQMs[measureIndx].ListEhrPats, null);
                            }
                            ...
                            if (p == 2)
                            {
                                //third iteration, fill dictionaries with numerator 3 data for stratifications
                                measureIndx = numer3;
                                listPopCountDicts = FillDictPopCounts(listQMs[measureIndx].ListEhrPats, null);
                            }
                        ...
                        {
                            start("list");
                            if (i == ((Enum)QualityType2014.CariesPrevent).ordinal() || i == ((Enum)QualityType2014.WeightChild_1_1).ordinal())
                            {
                                start("item");
                                start("content","styleCode","Bold");
                                ...
                                if (p == 0)
                                {
                                    if (i == ((Enum)QualityType2014.CariesPrevent).ordinal())
                                    {
                            ...
                            if (p == 0 && i == ((Enum)QualityType2014.CariesPrevent).ordinal())
                            {
                                start("item");
                                start("content","styleCode","Bold");
                            ...
                            for (Object __dummyForeachVar7 : listPopCountDicts[0])
                            {
                                KeyValuePair<String, int> kvpair = (KeyValuePair<String, int>)__dummyForeachVar7;
                                if (kvpair.Value == 0 || StringSupport.equals(kvpair.Key, "All"))
                                {
                                    continue;
                                }
                        ...
                        if (listPopCountDicts[1]["All"] > 0)
                        {
                            ...
                            for (Object __dummyForeachVar8 : listPopCountDicts[1])
                            {
                                KeyValuePair<String, int> kvpair = (KeyValuePair<String, int>)__dummyForeachVar8;
                                if (kvpair.Value == 0 || StringSupport.equals(kvpair.Key, "All"))
                                {
                                    continue;
                                }
                        ...
                        if (listPopCountDicts[2]["All"] > 0)
                        {
                            ...
                            for (Object __dummyForeachVar9 : listPopCountDicts[2])
                            {
                                KeyValuePair<String, int> kvpair = (KeyValuePair<String, int>)__dummyForeachVar9;
                                if (kvpair.Value == 0 || StringSupport.equals(kvpair.Key, "All"))
                                {
                                    continue;
                                }
                        ...
                        if (listPopCountDicts[3]["All"] > 0)
                        {
                            ...
                            for (Object __dummyForeachVar10 : listPopCountDicts[3])
                            {
                                KeyValuePair<String, int> kvpair = (KeyValuePair<String, int>)__dummyForeachVar10;
                                if (kvpair.Value == 0 || StringSupport.equals(kvpair.Key, "All"))
                                {
                                    continue;
                                }
                        ...
                        if (listPopCountDicts[4]["All"] > 0)
                        {
                            ...
                            for (Object __dummyForeachVar11 : listPopCountDicts[4])
                            {
                                KeyValuePair<String, int> kvpair = (KeyValuePair<String, int>)__dummyForeachVar11;
                                if (kvpair.Value == 0 || StringSupport.equals(kvpair.Key, "All"))
                                {
                                    continue;
                                }
                ...
                for (int i = 0;i < listQMs.Count;i++)
                {
                    ...
                    for (int p = 0;p < numPops;p++)
                    {
                        ...
                        if (numPops == 2 && p > 0)
                        {
                            //fill stratification data with supplemental data from other population
                            measureIndx = pop2Indx;
                            listPopCountDicts = FillDictPopCounts(listQMs[measureIndx].ListEhrPats, null);
                        }
                        ...
                        if (numPops == 3 && p > 0)
                        {
                            //only 3 populations for measure 155, Numerators 1-3
                            if (p == 1)
                            {
                                //second iteration, fill dictionaries with numerator 2 data for stratifications
                                measureIndx = numer2;
                        ...
                        StartAndEnd("id", "root", listQMs[measureIndx].eMeasureVSpecificId);
                        //version specific id
                        ...
                        StartAndEnd("setId", "root", listQMs[measureIndx].eMeasureVNeutralId);
                        //version neutral id
                        ...
                        if (listQMs[measureIndx].Type2014 == QualityType2014.MedicationsEntered)
                        {
                            //Measure 68 is the only one with a title that is not "None" or "NA"
                        ...
                        {
                            attribs("nullFlavor","NA");
                        }
                        else
                        {
                            //if no patients in denominator, then performance rate is null
                        ...
                        {
                            //if no patients in denominator, then reporting rate is null
                        ...
                        for (int j = 0;j < listPopCountDicts.Count;j++)
                        {
                            switch(j)
                            {
                                ...
                                case 1: 
                                    ...
                                    break;
                                case 2: 
                                    if (StringSupport.equals(listQMs[measureIndx].eMeasureDenexId, ""))
                                    {
                                        continue;
                                    }
                                     
                                    //if no exclusions defined for this measure, continue
                                ...
                                case 3: 
                                    ...
                                    break;
                                case 4: 
                                    if (StringSupport.equals(listQMs[measureIndx].eMeasureDenexcepId, ""))
                                    {
                                        continue;
                                    }
                                     
                                    //if no exceptions defined for this measure, continue
                            ...
                            else if (i == ((Enum)QualityType2014.WeightChild_1_1).ordinal())
                            {
                                iterations = 2;
                            }
                            ...
                            for (int s = 0;s < iterations;s++)
                            {
                                ...
                                else if (i == ((Enum)QualityType2014.WeightChild_1_1).ordinal())
                                {
                                    ...
                                    if (p == 1)
                                    {
                                        if (s == 0)
                                        {
                                            //first strata, second numerator
                                            stratIndx = s1numer2Indx;
                                        }
                                         
                                        if (s == 1)
                                        {
                                            //second strata, second population
                                    ...
                                    if (p == 2)
                                    {
                                        if (s == 0)
                                        {
                                            //first strata, third population
                                            stratIndx = s1numer3Indx;
                                        }
                                         
                                        if (s == 1)
                                        {
                                            //second strata, third population
                            ...
                            for (Object __dummyForeachVar12 : listPopCountDicts[j])
                            {
                                //loop through each of the population counts for this population and generate relevant supplemental data entries
                                KeyValuePair<String, int> kvpair = (KeyValuePair<String, int>)__dummyForeachVar12;
                                if (StringSupport.equals(kvpair.Key, "All") && kvpair.Value == 0)
                                {
                                    break;
                                }
                                 
                                //do not need to have these supplemental data entries if no initial patient population, break out of loop
                                if (kvpair.Value == 0 || StringSupport.equals(kvpair.Key, "All"))
                                {
                                    continue;
                                }
                                 
                                Key __dummyScrutVar35 = kvpair.Key;
                                if (__dummyScrutVar35.equals("Male") || __dummyScrutVar35.equals("Female") || __dummyScrutVar35.equals("Unknown"))
                                {
                                ...
                                else if (__dummyScrutVar35.equals("Hispanic") || __dummyScrutVar35.equals("NotHispanic"))
                                {
                                ...
                                else if (__dummyScrutVar35.equals("AfricanAmerican") || __dummyScrutVar35.equals("AmericanIndian") || __dummyScrutVar35.equals("Asian") || __dummyScrutVar35.equals("White") || __dummyScrutVar35.equals("HawaiiOrPacIsland") || __dummyScrutVar35.equals("Other"))
                                {
                                ...
                                else if (__dummyScrutVar35.equals("Medicare") || __dummyScrutVar35.equals("Medicaid") || __dummyScrutVar35.equals("Other Gvmt") || __dummyScrutVar35.equals("Private") || __dummyScrutVar35.equals("Self-pay"))
                                {
                                    ...
                                    start("value");
                                    _w.WriteAttributeString("xsi", "type", null, "CD");
                                    if (StringSupport.equals(kvpair.Key, "Medicare"))
                                    {
                                        attribs("code","1","displayName","Medicare");
                                    }
                                    else if (StringSupport.equals(kvpair.Key, "Medicaid"))
                                    {
                                        attribs("code","2","displayName","Medicaid");
                                    }
                                    else if (StringSupport.equals(kvpair.Key, "Other Gvmt"))
                                    {
                                        attribs("code","349","displayName","Other Government (Federal/State/Local) (excluding Department of Corrections)");
                                    }
                                    else if (StringSupport.equals(kvpair.Key, "Private"))
                                    {
    ...
    private static void generateQRDACatOne(List<QualityMeasure> listQMs, DateTime dateStart, DateTime dateEnd, String folderRoot) throws Exception {
        ...
        for (int i = 0;i < listQMs.Count;i++)
        {
            for (int j = 0;j < listQMs[i].ListEhrPats.Count;j++)
            {
                ...
                {
                    dictPatNumListQMs.Add(listQMs[i].ListEhrPats[j].EhrCqmPat.PatNum, new List<QualityMeasure>());
        ...
        for (int i = 0;i < listAllEhrPats.Count;i++)
        {
            ...
            Patient patCur = ehrPatCur.EhrCqmPat;
            //just to make referencing easier
            long patNumCur = ehrPatCur.EhrCqmPat.PatNum;
            //just to make referencing easier
            XmlWriterSettings xmlSettings = new XmlWriterSettings();
            xmlSettings.Encoding = Encoding.UTF8;
            xmlSettings.OmitXmlDeclaration = true;
            ...
            IDisposable __newVar1 = _w = XmlWriter.Create(strBuilder, xmlSettings);
            try
            {
                {
                    //Begin Clinical Document
                    ...
                    if (!StringSupport.equals(strHICNum, ""))
                    {
                    ...
                    if (patCur.SSN.Trim().Length == 9)
                    {
                        ...
                        StartAndEnd("id", "root", "2.16.840.1.113883.4.1", "extension", patCur.SSN.Trim());
                    }
                     
                    //HL7 SSN OID root with patient's SSN if they have a valid one
                    addressUnitedStates(patCur.Address,patCur.Address2,patCur.City,patCur.State,patCur.Zip,"HP");
                    //Validated
                    if (!StringSupport.equals(patCur.WirelessPhone.Trim(), ""))
                    {
                    ...
                    else if (!StringSupport.equals(patCur.HmPhone.Trim(), ""))
                    {
                    ...
                    else if (!StringSupport.equals(patCur.WkPhone.Trim(), ""))
                    {
                    ...
                    _w.WriteElementString("given", patCur.FName);
                    //Validated
                    if (!StringSupport.equals(patCur.MiddleI, ""))
                    {
                        _w.WriteElementString("given", patCur.MiddleI);
                    }
                     
                    _w.WriteElementString("family", patCur.LName);
                    //Validated
                    if (!StringSupport.equals(patCur.Title, ""))
                    {
                    ...
                    dateElement("birthTime",patCur.Birthdate);
                    //Validated
                    ...
                    String strRaceCode = "2131-1";
                    //default will be "Other Race"
                    ...
                    boolean isRaceNull = false;
                    if (ehrPatCur.ListPatientRaces.Count == 0)
                    {
                        isRaceNull = true;
                        strRaceDescript = "UNK";
                    }
                    else if (ehrPatCur.ListPatientRaces.Count == 1)
                    {
                        //if only one race is entered (already separated out the ethnicities) then we can construct the raceCode element with selected race
                        Race __dummyScrutVar37 = ehrPatCur.ListPatientRaces[0].Race;
                        if (__dummyScrutVar37.equals(PatRace.AfricanAmerican))
                        {
                            strRaceDescript = "Black or African American";
                            strRaceCode = ehrPatCur.ListPatientRaces[0].CdcrecCode;
                        }
                        else if (__dummyScrutVar37.equals(PatRace.AmericanIndian))
                        {
                            strRaceDescript = "American Indian or Alaska Native";
                            strRaceCode = ehrPatCur.ListPatientRaces[0].CdcrecCode;
                        }
                        else if (__dummyScrutVar37.equals(PatRace.Asian) || __dummyScrutVar37.equals(PatRace.White))
                        {
                            strRaceDescript = ehrPatCur.ListPatientRaces[0].Race.ToString();
                            strRaceCode = ehrPatCur.ListPatientRaces[0].CdcrecCode;
                        }
                        else if (__dummyScrutVar37.equals(PatRace.HawaiiOrPacIsland))
                        {
                            strRaceDescript = "Native Hawaiian or Other Pacific Islander";
                            strRaceCode = ehrPatCur.ListPatientRaces[0].CdcrecCode;
                        }
                        else if (__dummyScrutVar37.equals(PatRace.DeclinedToSpecifyRace))
                        {
                            isRaceNull = true;
                        ...
                        {
                            //any other race will be reported as "Other race"
                    ...
                    if (isRaceNull)
                    {
                        //either declined to specify (flavor "ASKU") or no race recorded (flavor "UNK")
                    ...
                    boolean isDeclinedEthnicity = false;
                    ...
                    else if (!StringSupport.equals(strEthnicityCode, "") && !StringSupport.equals(strEthnicityDescript, ""))
                    {
                    ...
                    //Validated
                    addressUnitedStates(PrefC.getString(PrefName.PracticeAddress),PrefC.getString(PrefName.PracticeAddress2),PrefC.getString(PrefName.PracticeCity),PrefC.getString(PrefName.PracticeST),PrefC.getString(PrefName.PracticeZip),"WP");
                    //Validated
                    end("representedCustodianOrganization");
                    end("assignedCustodian");
                    end("custodian");
                    ...
                    start("tbody");
                    IDisposable __newVar2 = _x = XmlWriter.Create(strBuilderPatDataEntries, xmlSettings);
                    try
                    {
                        {
                            ...
                            for (int j = 0;j < listQMsCur.Count;j++)
                            {
                                ...
                                for (int k = 0;k < listQMsCur[j].DictPatNumListInterventions[patNumCur].Count;k++)
                                {
                                    ...
                                    listUniqueItemExtensions.Add(extensCur);
                                    String descript = "";
                                    if (listQMsCur[j].Type2014 == QualityType2014.WeightAdult || listQMsCur[j].Type2014 == QualityType2014.WeightOver65)
                                    {
                                        //interventions in these two measures are ordered
                            ...
                            for (int j = 0;j < listQMsCur.Count;j++)
                            {
                                ...
                                for (int k = 0;k < listQMsCur[j].DictPatNumListMedPats[patNumCur].Count;k++)
                                {
                                    ...
                                    if (mPatCur.EhrCqmMedicationPatNum != 0)
                                    {
                                        ...
                                        if (!StringSupport.equals(mPatCur.PatNote, ""))
                                        {
                                            //PatNote means an order
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            //NotAdministered is due to an allergy or intolerance
                                            descript = "Medication, Allergy: ";
                                        }
                                        else
                                        {
                                            //only getting CompletionStatus=Complete or NotAdministered, the else is for Complete
                            ...
                            for (int j = 0;j < listQMsCur.Count;j++)
                            {
                                ...
                                for (int k = 0;k < listQMsCur[j].DictPatNumListNotPerfs[patNumCur].Count;k++)
                                {
                                    ...
                                    listUniqueItemExtensionsWithValueSetOIDs.Add(extensCur);
                                    String descript = "";
                                    if (StringSupport.equals(npCur.ValueSetOID, "2.16.840.1.113883.3.526.3.1254"))
                                    {
                                        descript = "Medication, Administered not done: ";
                                    }
                                    else if (StringSupport.equals(npCur.ValueSetOID, "2.16.840.1.113883.3.526.3.402") || StringSupport.equals(npCur.ValueSetOID, "2.16.840.1.113883.3.600.1.462"))
                                    {
                                        descript = "Procedure, Performed not done: ";
                                    }
                                    else if (StringSupport.equals(npCur.ValueSetOID, "2.16.840.1.113883.3.600.1.681"))
                                    {
                                        descript = "Physical Exam, Performed not done: ";
                                    }
                                    else
                                    {
                                        //must be a 2.16.840.1.113883.3.526.3.1278, not performed items restricted to one of these 5 value sets
                            ...
                            for (int j = 0;j < listQMsCur.Count;j++)
                            {
                                ...
                                for (int k = 0;k < listQMsCur[j].DictPatNumListProblems[patNumCur].Count;k++)
                                {
                                    ...
                                    {
                                        listUniqueItemExtensionsWithValueSetOIDs.Add(extensCur);
                                        System.String __dummyScrutVar38 = probCur.ValueSetOID;
                                        if (__dummyScrutVar38.equals("2.16.840.1.113883.3.526.3.1255"))
                                        {
                                            generateCommunicationEntry(probCur);
                                        }
                                        else if (__dummyScrutVar38.equals("2.16.840.1.113883.3.464.1003.110.12.1028"))
                                        {
                                            generateRiskAssessEntry(probCur);
                                        }
                                        else if (__dummyScrutVar38.equals("2.16.840.1.113883.3.600.1.1579"))
                                        {
                                    ...
                                    listUniqueItemExtensions.Add(extensCur);
                                    String descript = "";
                                    System.String __dummyScrutVar39 = probCur.ValueSetOID;
                                    if (__dummyScrutVar39.equals("2.16.840.1.113883.3.526.3.1255"))
                                    {
                                        descript = "Communication, From patient to provider: ";
                                    }
                                    else if (__dummyScrutVar39.equals("2.16.840.1.113883.3.464.1003.110.12.1028"))
                                    {
                                        descript = "Risk Category Assessment: ";
                                    }
                                    else if (__dummyScrutVar39.equals("2.16.840.1.113883.3.600.1.1579"))
                                    {
                    ...
                    {
                        if (__newVar2 != null)
                            Disposable.mkDisposable(__newVar2).dispose();
            ...
            {
                if (__newVar1 != null)
                    Disposable.mkDisposable(__newVar1).dispose();
                 
            }
            String xmlResult = strBuilder.ToString();
            dictPatNumXml.Add(patNumCur, xmlResult);
            SecurityLogs.makeLogEntry(OpenDentBusiness.Permissions.Copy,patNumCur,"QRDA Category I generated");
        }
        for (int i = 0;i < listAllEhrPats.Count;i++)
        {
            //Create audit log entry.
            Patient patCur = listAllEhrPats[i].EhrCqmPat;
            if (!dictPatNumXml.ContainsKey(patCur.PatNum))
            {
                continue;
            }
             
            //every patient will be in the dictionary, this is just in case
        ...
        try
        {
            ...
            for (int i = 0;i < allMeasDirs.Length;i++)
            {
                ...
                {
                    if (zipCur != null)
                        Disposable.mkDisposable(zipCur).dispose();
    ...
    /**
    * List of dictionaries that hold the count of supplemental data for each population.
    * Send in the measure IPP, which may be 0, and the list will be filled with supplemental counts for each population.Use the index of the dictionary to get the counts desired.0=IPP, 1=Denominator, 2=Denominator Exclusions, 3=Numerator, 4=Denominator Exceptions.Example, for counts of supplemental data for the denominator, get the dict[1].  dict[1] will have relevant counts for race, gender, ethnicity, and payer.
    */
    private static List<Dictionary<String, int>> fillDictPopCounts(List<EhrCqmPatient> listPats, Dictionary<long, List<EhrCqmEncounter>> dictPatNumListEncs) throws Exception {
        ...
        if (dictPatNumListEncs != null)
        {
            for (int i = 0;i < listPats.Count;i++)
            {
                //this will only be sent in for measure 68, current meds documented, since it is encounter based not patient based
    ...
    private static String getSupplementDataPrintString(String s) throws Exception {
        System.String __dummyScrutVar40 = s;
        if (__dummyScrutVar40.equals("AfricanAmerican"))
        {
            return "Black or African American";
        }
        else if (__dummyScrutVar40.equals("AmericanIndian"))
        {
            return "American Indian or Alaska Native";
        }
        else if (__dummyScrutVar40.equals("HawaiiOrPacIsland"))
        {
            return "Native Hawaiian or Other Pacific Islander";
        }
        else if (__dummyScrutVar40.equals("Other"))
        {
            return "Other Race";
        }
        else if (__dummyScrutVar40.equals("Hispanic"))
        {
            return "Hispanic or Latino";
        }
        else if (__dummyScrutVar40.equals("NotHispanic"))
        {
            return "Not Hispanic or Latino";
        }
        else if (__dummyScrutVar40.equals("Unknown"))
        {
            return "Undifferentiated";
        }
        else if (__dummyScrutVar40.equals("Other Gvmt"))
        {
            return "Other Government (Federal/State/Local) (excluding Department of Corrections)";
        }
        else if (__dummyScrutVar40.equals("Private"))
        {
    ...
    /**
    * Gets count of gender, race, ethnicity, and payer type.
    */
    private static Dictionary<String, int> getSupplementalCounts(List<EhrCqmPatient> listPats) throws Exception {
        ...
        for (int i = 0;i < listPats.Count;i++)
        {
            if (listPatNumsCounted.Contains(listPats[i].EhrCqmPat.PatNum))
            {
                continue;
            }
            else
            {
                //for measure 68, where listPats can contain the same patient multiple times for each encounter
            ...
            if (listPats[i].PayorSopCode == null || StringSupport.equals(listPats[i].PayorSopCode, ""))
            {
            ...
            {
                PayorSopCode.APPLY __dummyScrutVar41 = listPats[i].PayorSopCode.Substring(0, 1);
                //Get hierarchical code representing payer type main category, first character is type
                if (__dummyScrutVar41.equals("1"))
                {
                    dictCategoryCount["Medicare"]++;
                }
                else if (__dummyScrutVar41.equals("2"))
                {
                    dictCategoryCount["Medicaid"]++;
                }
                else if (__dummyScrutVar41.equals("3"))
                {
                    dictCategoryCount["Other Gvmt"]++;
                }
                else if (__dummyScrutVar41.equals("5"))
                {
                    dictCategoryCount["Private"]++;
                }
                else if (__dummyScrutVar41.equals("8"))
                {
                    dictCategoryCount["Self-pay"]++;
                }
                else
    ...
    /**
    * Generates one encounter xml entry and uses the global writer _x instead of the main writer _w.
    */
    private static void generateEncounterEntry(EhrCqmEncounter encCur) throws Exception {
        _isWriterW = false;
        start("entry","typeCode","DRIV");
    ...
    private static void generateVitalsignEntry(EhrCqmVitalsign vCur, String vType) throws Exception {
        _isWriterW = false;
        start("entry","typeCode","DRIV");
        start("observation","classCode","OBS","moodCode","EVN");
        ...
        System.String __dummyScrutVar42 = vType;
        ...
        if (__dummyScrutVar42.equals("BPd"))
        {
        ...
        else //for BP Systolic exam: CodeValue=8480-6, CodeSystemName=LOINC, CodeSystemOID=2.16.840.1.113883.6.1, Description=Systolic blood pressure, ValueSetName=Systolic Blood Pressure, ValueSetOID=2.16.840.1.113883.3.526.3.1032
        if (__dummyScrutVar42.equals("BPs"))
        {
        ...
        else //ValueSetOID=2.16.840.1.113883.3.464.1003.121.12.1014
        if (__dummyScrutVar42.equals("Ht"))
        {
        ...
        else //ValueSetOID=2.16.840.1.113883.3.464.1003.121.12.1015
        if (__dummyScrutVar42.equals("Wt"))
        {
        ...
        else //in kg/m2, if valid BMI value: CodeValue=39156-5, CodeSystemName=LOINC, CodeSystemOID=2.16.840.1.113883.6.1, Description=Body mass index (BMI) [Ratio], ValueSetName=BMI LOINC Value, ValueSetOID=2.16.840.1.113883.3.600.1.681
        if (__dummyScrutVar42.equals("BMI"))
        {
        ...
        else //ValueSetOID=2.16.840.1.113883.3.464.1003.121.12.1012
        if (__dummyScrutVar42.equals("BMIp"))
        {
    ...
    private static void generateDiagnosisEntry(EhrCqmProblem probCur) throws Exception {
        _isWriterW = false;
        start("entry","typeCode","DRIV");
        start("observation","classCode","OBS","moodCode","EVN");
    ...
    /**
    * Either a proc or a problem will be sent in, the other will be null.  If probCur then Procedure, Order: item, if procCur then Procedure, Perforemed: item.
    */
    private static void generateProcedureEntry(EhrCqmProc procCur, EhrCqmProblem probCur) throws Exception {
        _isWriterW = false;
        start("entry","typeCode","DRIV");
        if (probCur != null)
        {
    ...
    private static void generateRiskAssessEntry(EhrCqmProblem probCur) throws Exception {
        _isWriterW = false;
        start("entry","typeCode","DRIV");
        start("observation","classCode","OBS","moodCode","EVN");
    ...
    private static void generateCommunicationEntry(EhrCqmProblem probCur) throws Exception {
        _isWriterW = false;
        start("entry","typeCode","DRIV");
        start("act","classCode","ACT","moodCode","EVN");
    ...
    /**
    * if ValueSetOID=2.16.840.1.113883.3.526.3.1254, then Medication, Administered not done: Medical/Patient/System Reason (flu vaccine med)if ValueSetOID=2.16.840.1.113883.3.526.3.402, then Procedure, Performed not done: Medical/Patient/System Reason (flu vaccine proc)if ValueSetOID=2.16.840.1.113883.3.600.1.462, then Procedure, Performed not done: Medical or Other reason not done (current meds documented proc)if ValueSetOID=2.16.840.1.113883.3.600.1.681, then Physical Exam, Performed not done: Medical or Other reason not done/Patient Reason Refused (vitalsign exam)if ValueSetOID=2.16.840.1.113883.3.526.3.1278, then Risk Category Assessment not done: Medical Reason (tobacco assessment)Then use the negationInd="true" attribute to indicate that it was not performed
    */
    private static void generateNotPerfEntry(EhrCqmNotPerf npCur) throws Exception {
        _isWriterW = false;
        start("entry","typeCode","DRIV");
        if (StringSupport.equals(npCur.ValueSetOID, "2.16.840.1.113883.3.526.3.1254"))
        {
            //flu vaccine medication administered not done
            start("act","classCode","ACT","moodCode","EVN");
        ...
        {
            //flu vaccine proc or current meds documented proc performed not done
        ...
        {
            //must be a tobacco assessment not done
            start("observation","classCode","OBS","moodCode","EVN","negationInd","true");
    ...
    private static void generateMedPatsEntry(EhrCqmMedicationPat mPatCur) throws Exception {
        _isWriterW = false;
        start("entry","typeCode","DRIV");
        if (mPatCur.EhrCqmMedicationPatNum != 0)
        {
            //either Medication, Active: or Medication, Ordered:
            if (!StringSupport.equals(mPatCur.PatNote, ""))
            {
                //Medication, Order:
        ...
        {
            //if EhrCqmMedicationPat==0 then it is a vaccine, so EhrCqmVaccinePatNum!=0
    ...
    private static void generateMeasEventEntry(EhrCqmMeasEvent mCur) throws Exception {
        _isWriterW = false;
        start("entry","typeCode","DRIV");
        if (mCur.EventType == EhrMeasureEventType.TobaccoUseAssessed)
        {
            ...
            startAndEnd("code","code","ASSERTION","displayName","Assertion","codeSystem","2.16.840.1.113883.5.4","codeSystemName","ActCode");
            _x.WriteElementString("text", "Patient Characteristic: " + mCur.Description);
            //Description is of the tobacco assessment result, not the description of the assessment itself.  Example: Tobacco Assessment description = History of tobacco use Narrative, but description of result is Smoker (finding).  We want to display the result, Smoker.
            startAndEnd("statusCode","code","completed");
            start("effectiveTime");
    ...
    private static void generateInterventionEntry(EhrCqmIntervention iCur) throws Exception {
        _isWriterW = false;
        start("entry","typeCode","DRIV");
        ...
        if (StringSupport.equals(iCur.ValueSetOID, "2.16.840.1.113883.3.600.1.1525") || StringSupport.equals(iCur.ValueSetOID, "2.16.840.1.113883.3.600.1.1527") || StringSupport.equals(iCur.ValueSetOID, "2.16.840.1.113883.3.600.1.1528"))
        {
    ...
    /**
    * Helper for GenerateQRDA(). Builds an "id" element and writes a 36 character GUID string into the "root" attribute.
    * An example of how the uid might look: "5b010313-eff2-432c-9909-6193d8416fac"
    */
    private static void guid() throws Exception {
    ...
    /**
    * Helper for GenerateQRDA().  Modifies the global _w variable.  Performs a WriteStartElement, followed by any attributes.  Attributes must be in pairs: name, value.
    */
    private static void start(String elementName, String... attributes) throws Exception {
        if (_isWriterW)
        {
    ...
    /**
    * Helper for GenerateQRDA().  Modifies the global _w variable.  Performs a WriteEndElement.  The specified elementName is for readability only.
    */
    private static void end(String elementName) throws Exception {
        if (_isWriterW)
        {
            _w.WriteEndElement();
        }
    ...
    /**
    * Helper for GenerateQRDA().  Modifies the global _w variable.  Performs a WriteStartElement, followed by any attributes, followed by a WriteEndElement.  Attributes must be in pairs: name, value.
    */
    private static void startAndEnd(String elementName, String... attributes) throws Exception {
        if (_isWriterW)
        {
            _w.WriteStartElement(elementName);
            for (int i = 0;i < attributes.Length;i += 2)
    ...
    /**
    * Helper for GenerateQRDA().  Modifies the global _w variable.
    */
    private static void templateId(String rootNumber) throws Exception {
    ...
    * Helper for GenerateQRDA().  Modifies the global _w variable.
    */
    private static void templateId(String rootNumber, String authorityName) throws Exception {
        if (_isWriterW)
        {
            _w.WriteStartElement("templateId");
            _w.WriteAttributeString("root", rootNumber);
            if (!StringSupport.equals(authorityName, ""))
            {
    ...
    */
    private static void dateText(String strElementName, DateTime dateTime) throws Exception {
    ...
    */
    ...
    /**
    * Helper for GenerateQrda().  Performs a WriteAttributeString for each attribute.  Attributes must be in pairs: name, value.
    */
    private static void attribs(String... attributes) throws Exception {
        if (_isWriterW)
        {
            for (int i = 0;i < attributes.Length;i += 2)
            {
    ...
    * Will write nullFlavor="UNK" instead of value if year is before 1880.
    */
    private static void timeElement(String strElementName, DateTime dateTime) throws Exception {
        start(strElementName);
        if (dateTime.Year < 1880)
        {
            attribs("nullFlavor","UNK");
    ...
    private static void addressUnitedStates(String strAddress1, String strAddress2, String strCity, String strState, String strZip, String strUse) throws Exception {
        start("addr","use",strUse);
        if (StringSupport.equals(strUse, "HP"))
        {
            _w.WriteComment("HP is \"primary home\" from codeSystem 2.16.840.1.113883.5.1119");
        }
        else if (StringSupport.equals(strUse, "WP"))
        {
            _w.WriteComment("WP is \"work place\" from codeSystem 2.16.840.1.113883.5.1119");
        }
        else if (StringSupport.equals(strUse, "H"))
        {
        ...
        if (strZip.Length > 4)
        {
            _w.WriteElementString("postalCode", strZip.Substring(0, 5));
        }
         
        //postalCode is required if the country is US.  If country is not specified, it's assumed to be US. If country is something other than US, the postalCode MAY be present but MAY be bound to different vocabularies. Validated to be at least 5 characters long.
        _w.WriteElementString("country", "US");
        end("addr");
    }

    private static void measureEntry(String versionSpecificId, String measureTitle, String versionNeutralId, String versionNum) throws Exception {
        start("entry");
        start("organizer","classCode","CLUSTER","moodCode","EVN");
        _w.WriteComment("Measure Reference Template");
        templateId("2.16.840.1.113883.10.20.24.3.98");
    ...
    private static void measureTextTableRow(String measureTitle, String cmsId, String versionNeutralId, String versionNum, String versionSpecificId) throws Exception {
        start("tr");
    ...
    private static void patientDataTextTableRow(String description, String valueSetOID, String codeSystemName, String codeValue, DateTime dateStart, DateTime dateEnd, String result) throws Exception {
        start("tr");
        ...
        {
            //no end date, considered currently still 'active'
    ...
    /**
    * Checks data values for preferences and practice information to ensure required data is available for QRDA creation.
    * Returns empty string if no errors, otherwise returns a string containing error messages.
    */
    private static String validateSettings() throws Exception {
        String strErrors = "";
    ...
    * Returns empty string if no errors, otherwise returns a string containing error messages.
    */
    private static String validatePatient(Patient pat) throws Exception {
        String strErrors = "";
    ...
    /**
    * Validation rules:
    * 1. No embedded dashes or spaces.2. Must be alphanumeric.3. Alpha characters must be upper case.4. Length can't be > 12 or less than 7.5. If alphanumeric, all numbers cannot be 9s.6. If length 7: must be 1 alpha + 6 numeric.7. If length 8: must be 2 alpha + 6 numeric.8. If length 9: must be 3 alpha + 6 numeric.9. If length 10: can either be 1 alpha + 9 numeric, or 9 numeric + 1 alpha.10. If length 11: must be 2 alpha + 9 numeric, or 9 numeric + 1 alpha + 1 numeric, or 9 numeric + 2 alpha.11. If length 12: must be 3 alpha + 9 numeric.
    */
    private static String validateMedicaidID(String medicaidId) throws Exception {
        ...
        if (Regex.IsMatch(retval, "^[A-Z]{1,3}[0-9]{6}$") || Regex.IsMatch(retval, "^[A-Z]{1,3}[0-9]{9}$") || Regex.IsMatch(retval, "^[0-9]{9}[A-Z][A-Z0-9]{0,1}$"))
        {
            return retval;
        }
