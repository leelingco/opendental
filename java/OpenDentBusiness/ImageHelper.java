...
{
    /**
    * Takes in a mount object and finds all the images pertaining to the mount, then concatonates them together into one large, unscaled image and returns that image. Set imageSelected=-1 to unselect all images, or set to an image ordinal to highlight the image. The mount is rendered onto the given mountImage, so it must have been appropriately created by CreateBlankMountImage(). One can create a mount template by passing in arrays of zero length.
    */
    public static void renderMountImage(Bitmap mountImage, Bitmap[] originalImages, List<MountItem> mountItems, OpenDentBusiness.Document[] documents, int imageSelected) throws Exception {
        ...
        {
            {
                ...
                {
                    g.FillRectangle(Brushes.Black, mountItems[i].Xpos, mountItems[i].Ypos, mountItems[i].Width, mountItems[i].Height);
                    //draw box behind image
    ...
    /**
    * Renders the hallow rectangles which represent the individual image frames into the given mount image.
    */
    public static void renderMountFrames(Bitmap mountImage, List<MountItem> mountItems, int imageSelected) throws Exception {
        Graphics g = Graphics.FromImage(mountImage);
        try
        {
            {
                for (int i = 0;i < mountItems.Count;i++)
                {
                    ...
                    Pen highlight = new Pen();
                    if (i == imageSelected)
                    {
                        highlight = (Pen)Pens.Yellow.Clone();
                    }
                    else
                    {
                        //highlight desired image.
                        highlight = (Pen)Pens.SlateGray.Clone();
                    } 
                    //just surround other images with standard border.
                    highlight.Width = Math.Max(mountItems[i].Width, mountItems[i].Height) / 100.0f;
                    g.DrawRectangle(highlight, mountItems[i].Xpos - highlight.Width / 2, mountItems[i].Ypos - highlight.Width / 2, mountItems[i].Width + highlight.Width, mountItems[i].Height + highlight.Width);
                }
            }
        }
        finally
    ...
    */
    public static void renderImageIntoMount(Bitmap mountImage, MountItem mountItem, Bitmap mountItemImage, OpenDentBusiness.Document mountItemDoc) throws Exception {
        if (mountItem == null)
        {
            return ;
        }
        ...
        {
            {
                g.FillRectangle(Brushes.Black, mountItem.Xpos, mountItem.Ypos, mountItem.Width, mountItem.Height);
                //draw box behind image
    ...
    */
    ...
    /**
    * Applies the document specified cropping, flip, rotation, brightness and contrast transformations to the image and returns the resulting image. Zoom and translation must be handled by the calling code. The returned image is always a new image that can be modified without affecting the original image. The change in the image's center point is returned into deltaCenter, so that rotation offsets can be properly calculated when displaying the returned image.
    */
    public static Bitmap applyDocumentSettingsToImage(OpenDentBusiness.Document doc, Bitmap image, ImageSettingFlags settings) throws Exception {
        if (image == null)
        {
            return null;
        }
        ...
        //resets the resolution to 96, just like it does for docs 20 lines down.
        ...
        Rectangle cropResult = new Rectangle();
        //Crop not requested.
        if ((settings & ImageSettingFlags.CROP) != 0 && doc.CropW > 0 && doc.CropH > 0)
        {
            //No clip area yet defined, so no clipping is performed.
            //Intersect image rectangle with
            float[] cropDims = ODMathLib.IntersectRectangles(0, 0, image.Width, image.Height, doc.CropX, doc.CropY, doc.CropW, doc.CropH);
            //document crop rectangle.
            if (cropDims.Length == 0)
            {
                return null;
            }
            ...
            cropResult = new Rectangle((int)cropDims[0], (int)cropDims[1], (int)Math.Ceiling(cropDims[2]), (int)Math.Ceiling(cropDims[3]));
        }
        else
        {
            cropResult = new Rectangle(0, 0, image.Width, image.Height);
        } 
        //No cropping.
        ...
        //Do not apply color function if brightness/contrast have never been set (assume normal settings).
        if ((settings & ImageSettingFlags.COLORFUNCTION) != 0 && doc.WindowingMax != 0 && !(doc.WindowingMax == 255 && doc.WindowingMin == 0))
        {
            //Don't apply if brightness/contrast settings are normal.
            float[] inputValues = new float[]{ doc.WindowingMin / 255f, doc.WindowingMax / 255f };
            float[] outputValues = new float[]{ 0, 1 };
            BitmapData croppedData = null;
            try
            {
                croppedData = cropped.LockBits(new Rectangle(0, 0, cropped.Width, cropped.Height), ImageLockMode.ReadWrite, cropped.PixelFormat);
    ...
    */
    public static Bitmap getThumbnail(Image original, int size) throws Exception {
