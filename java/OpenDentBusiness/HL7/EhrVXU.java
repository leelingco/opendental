...
/**
* A VXU message is an Unsolicited Vaccination Record Update.  It is a message sent out from Open Dental detailing vaccinations that were given.
* Implementation based on HL7 version 2.5.1 Immunization Messaging Release 1.4 08/01/2012.  Data types defined on page 52.
* To view specific HL7 table definitions, see http://hl7.org/implement/standards/fhir/terminologies-v2.html.
*/
public class EhrVXU   
{
    /**
    ...
    * Set in constructor and must not be modified.
    */
    private List<VaccinePat> _vaccines = new List<VaccinePat>();
    /**
    ...
    */
    ...
    /**
    * A variable which is used in multiple places but always has the same value. At class level for convenience.
    */
    ...
    */
    ...
    /**
    * Creates the Message object and fills it with data.  Vaccines must all be for the same patient.
    * A list of vaccines is passed in so the user can select a subset of vaccines to send for the patient.
    * Throws an exception if validation fails.
    */
    public EhrVXU(Patient pat, List<VaccinePat> vaccines) throws Exception {
    ...
    private void initializeVariables() throws Exception {
        ...
        if (!PrefC.getBool(PrefName.EasyNoClinics) && _pat.ClinicNum != 0)
        {
    ...
    private void buildMessage() throws Exception {
        _msg = new MessageHL7(MessageTypeHL7.VXU);
        //Message format for VXU is on guide page 160.
        mSH();
        ...
        pD1();
        //PD1 segment.  Required if known.  Cardinality [0..1].
        nK1();
        for (int i = 0;i < _vaccines.Count;i++)
        {
            //NK1 segment.  Required if known.  Cardinality [0..1].
            ...
            //Begin Order group.
            ORC(_vaccines[i]);
            //ORC segment.  Required.  Cardinality [1..1].
            ...
            RXA(_vaccines[i]);
            //RXA segment.  Required.  Cardinality [1..1].
            RXR(_vaccines[i]);
            //RXR segment.  Required if known.  Cardinality [0..1].
            OBX(_vaccines[i]);
            //OBX segment.  Required if known.  Cardinality [0..*].
            nTE();
        }
    }

    //NTE segment.  Required if known.  Cardinality [0..1].
    //End Order group.
    /**
    * Message Segment Header segment.  Required.  Defines intent, source, destination and syntax of the message.  Guide page 104.
    */
    private void mSH() throws Exception {
        ...
        //MSH-3 Sending Application.  Required if known (length unspecified).  Value set HL70361  (guide page 229, "no suggested values defined").  Type HD (guide page 65).
        //MSH-4 Sending Facility.  Required if known (length unspecified).  Value set HL70362 (guide page 229, "no suggested values defined").  Type HD (guide page 65).
        ...
        //MSH-6 Receiving Facility.  Required if known (length unspecified).  Value set HL70362 (guide page 229, "no suggested values defined").  Type HD (guide page 65).
        //MSH-7 Date/Time of Message.  Required (length 12..19).
        //MSH-8 Security.  Optional (length unspecified).
        //MSH-9 Message Type. Required (length unspecified).
        _seg = new SegmentHL7("MSH" + "|" + "^~\\&" + "|Open Dental" + "|" + _sendingFacilityName + "|" + "|EHR Facility" + "|" + DateTime.Now.ToString("yyyyMMddHHmmss") + "|" + "|VXU^V04^VXU_V04" + "|OD-" + DateTime.Now.ToString("yyyyMMddHHmmss") + "-" + CodeBase.MiscUtils.createRandomAlphaNumericString(14) + "|P" + "|2.5.1" + "|" + "|" + "|NE" + "|NE");
        //MSH-10 Message Control ID.  Required (length 1..199).  Our IDs are 32 characters.
        //MSH-11 Processing ID.  Required (length unspecified).  P=production.
        //MSH-12 Version ID.  Required (length unspecified).  Must be exactly "2.5.1".
        //MSH-13 Sequence Number.  Optional (length unspecified).
        //MSH-14 Continuation Pointer.  Optional (length unspecified).
        //MSH-15 Accept Acknowledgement Type.  Required if known (length unspecified).  Value set HL70155 (AL=Always, ER=Error/rejection conditions only, NE=Never, SU=Successful completion only).
        //MSH-16 Application Acknowledgement Type.  Required if known (length unspecified).  Value set HL70155 (AL=Always, ER=Error/rejection conditions only, NE=Never, SU=Successful completion only).
    ...
    /**
    * Next of Kin segment.  Required if known.  Guide page 111.
    */
    private void nK1() throws Exception {
        List<Guardian> listGuardians = Guardians.refresh(_pat.PatNum);
        for (int i = 0;i < listGuardians.Count;i++)
        {
            //One NK1 segment for each relationship.
            _seg = new SegmentHL7(SegmentNameHL7.NK1);
            _seg.setField(0,"NK1");
            _seg.SetField(1, (i + 1).ToString());
            //NK1-1 Set ID.  Required (length unspecified).  Cardinality [1..1].
            ...
            else //This relationship type is not documented in the guide, but it is defined as part of HL7 0063 in a more recent publication.  We added because it seemed like a basic relationship type.
            if (relat == GuardianRelationship.Father)
            {
                strRelatCode = "FTH";
            }
            ...
            if (relat == GuardianRelationship.Grandchild)
            {
                strRelatCode = "GCH";
            }
            else //This relationship type is not documented in the guide, but it is defined as part of HL7 0063 in a more recent publication.  We added because it seemed like a basic relationship type.
            if (relat == GuardianRelationship.Grandfather)
            {
                //This status is from our older guardian implementation.
                strRelatCode = "GRD";
            }
            else //Grandparent
            if (relat == GuardianRelationship.Grandmother)
            {
                //This status is from our older guardian implementation.
                strRelatCode = "GRD";
            }
            else //Grandparent
            if (relat == GuardianRelationship.Grandparent)
            {
                strRelatCode = "GRD";
            }
            ...
            if (relat == GuardianRelationship.Mother)
            {
                strRelatCode = "MTH";
            }
            ...
            else if (relat == GuardianRelationship.Sitter)
            {
                //This status is from our older guardian implementation.
                strRelatCode = "CGV";
            }
            else //Caregiver
            if (relat == GuardianRelationship.Spouse)
            {
                strRelatCode = "SPO";
            }
            ...
            else if (relat == GuardianRelationship.Stepfather)
            {
                //This status is from our older guardian implementation.
                strRelatCode = "PAR";
            }
            else //parent
            if (relat == GuardianRelationship.Stepmother)
            {
                //This status is from our older guardian implementation.
                strRelatCode = "PAR";
            }
            ...
            //parent
            //Skip the entire segment if the relationship is not known.  Should not happen.
    ...
    /**
    * Note segment.  Required if known.  Guide page 116.
    */
    private void nTE() throws Exception {
    }

    /**
    * Observation Result segment.  Required if known.  The basic format is question and answer.  Guide page 116.
    */
    private void oBX(VaccinePat vaccine) throws Exception {
        ...
        for (int i = 0;i < listVaccineObservations.Count;i++)
        {
            ...
            _seg.setField(0,"OBX");
            _seg.SetField(1, (i + 1).ToString());
            //OBX-1 Set ID - OBX.  Required (length 1..4).  Cardinality [1..1].
            ...
            {
                //vaccineObs.ValType==VaccineObsType.Coded
                _seg.setField(2,"CE");
            }    
            ...
            {
                //vaccineObs.IdentifyingCode==VaccineObsIdentifier.DocumentType
            ...
            {
                //vaccineObs.VaccineObsNumGroup!=0
            ...
            {
                String codeDescript = vaccineObs.ValReported.Trim();
                //If we do not know the description, then the code will also be placed into the description. The testing tool required non-empty entries.
                ...
                {
                    if (StringSupport.equals(vaccineObs.ValReported.ToUpper(), "V01"))
                    {
                        codeDescript = "Not VFC eligible";
                    }
                    else if (StringSupport.equals(vaccineObs.ValReported.ToUpper(), "V02"))
                    {
                        codeDescript = "VFC eligible-Medicaid/Medicaid Managed Care";
                    }
                    else if (StringSupport.equals(vaccineObs.ValReported.ToUpper(), "V03"))
                    {
                        codeDescript = "VFC eligible- Uninsured";
                    }
                    else if (StringSupport.equals(vaccineObs.ValReported.ToUpper(), "V04"))
                    {
                        codeDescript = "VFC eligible- American Indian/Alaskan Native";
                    }
                    else if (StringSupport.equals(vaccineObs.ValReported.ToUpper(), "V05"))
                    {
                        codeDescript = "VFC eligible-Federally Qualified Health Center Patient (under-insured)";
                    }
                    else if (StringSupport.equals(vaccineObs.ValReported.ToUpper(), "V06"))
                    {
                        codeDescript = "Deprecated [VFC eligible- State specific eligibility (e.g. S-CHIP plan)]";
                    }
                    else if (StringSupport.equals(vaccineObs.ValReported.ToUpper(), "V07"))
                    {
                        codeDescript = "Local-specific eligibility";
                    }
                    else if (StringSupport.equals(vaccineObs.ValReported.ToUpper(), "V08"))
                    {
            ...
            {
                //vaccineObs.ValType==VaccineObsType.Text
            ...
            _seg.setField(11,"F");
            //OBX-11 Observation Result Status.  Required (length 1..1).  Cardinality [1..1].  Value set HL70085 (constrained, guide page 198).  We are expected to use value F=Final.
    ...
    /**
    * Order Request segment.  Required.  Guide page 126.
    */
    private void oRC(VaccinePat vaccine) throws Exception {
        _seg = new SegmentHL7(SegmentNameHL7.ORC);
        _seg.setField(0,"ORC");
        _seg.setField(1,"RE");
        //ORC-1 Order Control.  Required (length 2).  Cardinality [1..1].  Value set HL70119.  The only allowed value is "RE".
        ...
        Userod userod = Userods.getUser(vaccine.UserNum);
        //Can be null if vaccine.UserNum=0 for older records before the vaccine.UserNum column existed.
        if (userod != null)
        {
            if (userod.ProvNum != 0)
            {
        ...
        Provider provOrdering = Providers.getProv(vaccine.ProvNumOrdering);
        //Can be null if vaccine.ProvNumOrdering is zero.
    ...
    /**
    * Patient Demographic segment.  Required if known.  Additional demographics.  Guide page 132.
    */
    private void pD1() throws Exception {
        ...
        if (ehrPatient.VacShareOk == YN.No)
        {
            _seg.setField(12,"Y");
        }
    ...
    /**
    * Patient Identifier segment.  Required.  Guide page 137.
    */
    private void pID() throws Exception {
        _seg = new SegmentHL7(SegmentNameHL7.PID);
        _seg.setField(0,"PID");
        _seg.setField(1,"1");
        //PID-1 Set ID - PID.  Required if known.  Cardinality [0..1].  Must be "1" for the first occurrence.  Not sure why there would ever be more than one.
        ...
        //PID-3.4 Assigning Authority.  Required.  Value set HL70363.
        ...
        //PID-5 Patient Name.  Required (length unspecified).  Cardinality [1..*].  Type XPN.  The first repetition must contain the legal name.
        EhrPatient ehrPatient = EhrPatients.refresh(_pat.PatNum);
        writeXPN(6,ehrPatient.MotherMaidenFname,ehrPatient.MotherMaidenLname,"","M");
        //PID-6 Mother's Maiden Name.  Required if known (length unspecified).  Cardinality [0..1].  Type XPN.
        ...
        for (int i = 0;i < listPatRacesFiltered.Count;i++)
        {
            ...
            _seg.setOrRepeatField(10,strRaceCode,strRaceName,"HL70005");
        }
    ...
    /**
    * Pharmacy/Treatment Administration segment.  Required.  Guide page 149.
    */
    private void rXA(VaccinePat vaccine) throws Exception {
        _seg = new SegmentHL7(SegmentNameHL7.RXA);
        _seg.setField(0,"RXA");
        _seg.setField(1,"0");
        //RXA-1 Give Sub-ID Counter.  Required.  Must be "0".
        _seg.setField(2,"1");
        //RXA-2 Administration Sub-ID Counter.  Required.  Must be "1".
        _seg.SetField(3, vaccine.DateTimeStart.ToString("yyyyMMddHHmm"));
        //RXA-3 Date/Time Start of Administration.  Required.  This segment can also be used to planned vaccinations.
        if (vaccine.DateTimeEnd.Year > 1880)
        {
            _seg.SetField(4, vaccine.DateTimeEnd.ToString("yyyyMMddHHmm"));
        }
         
        //RXA-4 Date/Time End of Administration.  Required if known.  Must be same as RXA-3 or blank.  UI forces RXA-4 and RXA-3 to be equal.  This would be blank if for a planned vaccine.
        ...
        //Registries that do not collect administered amount should record the value as "999".
        ...
        if (vaccine.AdministeredAmt > 0 && vaccine.DrugUnitNum != 0)
        {
            ...
            writeCE(7,ucum.UcumCode,ucum.Description,"UCUM");
        }
         
        //UCUM is not in table HL70396, but it there was a note stating that it was required in the guide and UCUM was required in the test cases.
        ...
        Provider provAdministering = Providers.getProv(vaccine.ProvNumAdminister);
        //Can be null when vaccine.ProvNumAdminister is zero.
        ...
        if (vaccine.CompletionStatus != VaccineCompletionStatus.NotAdministered && !StringSupport.equals(vaccine.LotNumber.Trim(), ""))
        {
        ...
        {
            writeCE(18,"02",vaccine.Note,"NIP002");
        }
        else //The reason is required instead of a generic description for this particular situation.
        if (vaccine.RefusalReason == VaccineRefusalReason.PatientDecision)
        {
    ...
    /**
    * Pharmacy/Treatment Route segment.  Required if known.  Guide page 158.
    */
    private void rXR(VaccinePat vaccine) throws Exception {
        if (vaccine.AdministrationRoute == VaccineAdministrationRoute.None)
        {
            return ;
        }
         
        //Unspecified.  Therefore unknown and the entire segment is not required.
    ...
    /**
    * Type CE (guide page 53).  Writes a coded element into the fieldIndex field of the current segment.
    */
    private void writeCE(int fieldIndex, String strIdentifier, String strText, String strNameCodeSystem) throws Exception {
        //CE.1 Identifier.  Required (length 1..50).
        //CE.2 Text.  Required if known (length 1..999). Human readable text that is not further used.
    ...
    /**
    * Type EI (guid page 62).  Writes an Entity Identifier (order number) into the fieldIndex field of the current segment.
    */
    private void writeEI(int fieldIndex, String identifier, String city, String state) throws Exception {
        //EI.1 Entity Identifier.  Required (length 1..199).
        _seg.setField(fieldIndex,identifier,getAssigningAuthority(city,state));
    }

    //EI.2 Namespace ID.  Required if EI.3 is blank (length 1..20).  Value set HL70363 (guide page 229, 3 letter abbreviation for US state, US city, or US territory).
    ...
    /**
    * Corresponds to table HL70363 (guide page 229).
    */
    private String getAssigningAuthority(String city, String state) throws Exception {
        String code = "";
        //A value from Value set HL70363 (guide page 229, 3 letter abbreviation for US state, US city, or US territory).
        String st = state.Trim().ToUpper();
        String c = city.Trim().ToUpper().Replace(" ", "");
        code = st + "A";
        //Most of the codes are just the state code followed by an 'A'.  This includes American territories and districts. http://www.itl.nist.gov/fipspubs/fip5-2.htm
        if (StringSupport.equals(st, "IL") && StringSupport.equals(c, "CHICAGO"))
        {
            //CHICAGO ILLINOIS
            code = "CHA";
        }
        else //CHICAGO has thier own code.
        if (StringSupport.equals(st, "NY") && StringSupport.equals(c, "NEWYORK"))
        {
            //NEW YORK NEW YORK
            code = "BAA";
        }
        else //NEW YORK CITY has their own code.
        if (StringSupport.equals(st, "PA") && StringSupport.equals(c, "PHILADELPHIA"))
        {
            //PHILADELPHIA PENNSYLVANIA
            code = "PHA";
        }
        else //Philadelphia has their own code.
        if (StringSupport.equals(st, "PW"))
        {
            //REPUBLIC PALAU (American territory)
            code = "RPA";
        }
        else //This code is one that does not match the pattern for the rest of the codes.
        if (StringSupport.equals(st, "TX") && StringSupport.equals(c, "SANANTONIO"))
        {
            //SAN ANTONIO TEXAS
            code = "TBA";
        }
        else //SAN ANTONIO has their own code.
        if (StringSupport.equals(st, "TX") && StringSupport.equals(c, "HOUSTON"))
        {
            //HOUSTON TEXAS
            code = "THA";
        }
    ...
    //HOUSTON has their own code.
    /**
    * Type IS (guide page 68).  Writes a string corresponding to table HL70001 (guide page 193) into the fieldIndex field for the current segment.
    */
    ...
    /**
    * Type LA2 (guide page 68).  Writes facility information into the fieldIndex field of the current segment.
    */
    private void writeLA2(int fieldIndex, String facilityName) throws Exception {
        //LA2.1 Point of Care.  Optional.
        //LA2.2 Room.  Optional.
        //LA2.3 Bed.  Optional.
        //LA2.4 Facility.  Required.  Type HD (guide page 66).
        _seg.setField(fieldIndex,"","","",facilityName);
    }

    //LA2.4.1 Namespace ID.  Required when LA2.4.2 is blank.  Value sets HL70300 (guide page 224), HL70361 (guide page 229), HL70362 (guide page 229), HL70363 (guide page 229).  Value set used depends on usage.
    ...
    /**
    * Type XAD (guide page 74).  Writes an extended address into the fieldIndex field for the current segment.
    */
    private void writeXAD(int fieldIndex, String address1, String address2, String city, String state, String zip) throws Exception {
        //XAD.1 Street Address.  Required if known (length unspecified).  Data type SAD (guide page 72).  The SAD type only requires the first sub-component.
        //XAD.2 Other Designation.  Required if known (length 1..120).
        //XAD.3 City.  Required if known (length 1..50).
        //XAD.4 State or Province.  Required if known (length 1..50).
        _seg.setField(fieldIndex,address1,address2,city,state,zip,"USA","L");
    }

    //XAD.5 Zip or Postal Code.  Required if known (length 1..12).
    //XAD.6 Country.  Required if known (length 3..3).  Value set HL70399.  Defaults to USA.
    //XAD.7 Address Type.  Required (length 1..3).  Value set HL70190 (guide page 202).  C=Current or temporary,P=Permanent,M=Mailing,B=Firm/Business,O=Office,H=Home,L=Legal address,etc...
    ...
    /**
    * Type XCN (guide page 77).  Writes user name and id into the fieldIndex field for the current segment.
    * Either the fName and lName must be specified, or id and city and state must be specified. All fields may be specified.
    * Allowed values for nameTypeCode: A=Alias name,L=Legal name,D=Display name,M=Maiden name,C=Adopted name,B=Name at birth,P=Name of partner/spouse,U=Unspecified.
    */
    private void writeXCN(int fieldIndex, String fName, String lName, String middleI, String id, String city, String state, String nameTypeCode) throws Exception {
        boolean hasName = false;
        if (!StringSupport.equals(fName, "") && !StringSupport.equals(lName, ""))
        {
            hasName = true;
        }
        ...
        if (!StringSupport.equals(id, "") && !StringSupport.equals(city, "") && !StringSupport.equals(state, ""))
        {
            //All 3 fields must be present, or none of them should be sent.
            hasId = true;
            idModified = id;
        ...
        //XCN.1 ID Number.  Required if XCN.2 and XCN.3 are blank.
        //XCN.2 Family Name.  Required if known.
        //XCN.3 Given Name.  Required if known (length 1..30).
        //XCN.4 Second and Further Given Names or Initials Thereof.  Required if known (length 1..30).
        //XCN.5 Suffix.  Optional.
        //XCN.6 Prefix.  Optional.
        //XCN.7 Degree.  No longer used.
        //XCN.8 Source Table.  Optional.
        //XCN.9 Assigning Authority.  Required if XCN.1 is not blank.  Value set HL70363 (guide page 229).
        _seg.setField(fieldIndex,idModified,lName,fName,middleI,"","","","",assigningAuthority,nameTypeCode);
    }

    //XCN.10 Name Type Code.  Required if known (length 1..1).  Value set HL70200 (guide page 203).  A=Alias name,L=Legal name,D=Display name,M=Maiden name,C=Adopted name,B=Name at birth,P=Name of partner/spouse,U=Unspecified.
    ...
    /**
    * Type XTN (guide page 84).  Writes a phone number or other contact information (such as email address) into the fieldIndex field for the current segment.
    ...
    * Can specify 0 or more contacts. The first valid phone number in the list will be written and the other phone numbers will be ignored.
    */
    private void writeXTN(int fieldIndex, String... arrayContactInfo) throws Exception {
        int contactCount = 0;
        for (int i = 0;i < arrayContactInfo.Length;i += 4)
        {
            ...
            if (StringSupport.equals(strContactInfo, "") && !StringSupport.equals(strForce, "F"))
            {
                continue;
            }
             
            //When the contact info is blank and the information is not forced, then do not output.
            contactCount++;
            if (StringSupport.equals(strTeleUseCode, "NET"))
            {
                //Email address.
                if (contactCount == 1)
                {
                    //XTN.1 Telephone Number.  No longer used.
                    //XTN.2 Telecommunication Use Code.  Required.  Value set HL70201 (guide page 203).
                    //XTN.3 Telecommunication Equipment Type.  Required if known.  Value set HL70202 (guide page 203).
                    _seg.setField(fieldIndex,"",strTeleUseCode,strTeleEquipType,strContactInfo);
                }
                else
                {
                    ...
                    //At least one contact has already been specified (even if blank).  Repeat the field.  In testing, we were required to make a blank phone number preceed an email address.  This block makes it happen.
                    //XTN.1 Telephone Number.  No longer used.
                    //XTN.2 Telecommunication Use Code.  Required.  Value set HL70201 (guide page 203).
                    //XTN.3 Telecommunication Equipment Type.  Required if known.  Value set HL70202 (guide page 203).
                    _seg.repeatField(fieldIndex,"",strTeleUseCode,strTeleEquipType,strContactInfo);
                } 
            }
            else
            {
                ...
                if (StringSupport.equals(strPhone, ""))
                {
                    continue;
                }
                 
                //Either forced and empty, or the phone number is invalid.
                if (contactCount == 1)
                {
                    //XTN.1 Telephone Number.  No longer used.
                    //XTN.2 Telecommunication Use Code.  Required.  Value set HL70201 (guide page 203).
                    ...
                    //XTN.5 Country Code.  Optional.
                    //XTN.6 Area/City Code.  Required when XTN.2 is NOT set to "NET" (length 5..5).
                    _seg.SetField(fieldIndex, "", strTeleUseCode, strTeleEquipType, "", "", strPhone.Substring(0, 3), strPhone.Substring(3));
                }
                else
                {
                    //XTN.7 Local Number.  Required when XTN.2 is NOT set to "NET" (length 7..7).
                    ...
                    //XTN.11 Speed Dial Code.  Optional.
                    //XTN.12 Unformatted Telephone Number.  Optional.
                    //At least one contact has already been specified (even if blank).  Repeat the field.  In testing, we were required to make a blank phone number preceed an email address.  This block allows a phone number after a blank phone number.
                    //XTN.1 Telephone Number.  No longer used.
                    //XTN.2 Telecommunication Use Code.  Required.  Value set HL70201 (guide page 203).
                    //XTN.3 Telecommunication Equipment Type.  Required if known.  Value set HL70202 (guide page 203).
                    //XTN.4 Email Address.  Required when XTN.2 is set to "NET" (length 1..199).
                    //XTN.5 Country Code.  Optional.
                    //XTN.6 Area/City Code.  Required when XTN.2 is NOT set to "NET" (length 5..5).
                    _seg.RepeatField(fieldIndex, "", strTeleUseCode, strTeleEquipType, "", "", strPhone.Substring(0, 3), strPhone.Substring(3));
                } 
            } 
        }
    }
    ...
    //XTN.12 Unformatted Telephone Number.  Optional.
    /**
    * Removes any characters from the phone number which are not digits.  Returns empty string if the phone number is invalid.
    */
    private String tidyPhone(String phoneRaw) throws Exception {
        String strDigits = "";
        for (int j = 0;j < phoneRaw.Length;j++)
        {
            ...
            if (StringSupport.equals(strDigits, "") && StringSupport.equals(phoneRaw.Substring(j, 1), "1"))
            {
                continue;
            }
    ...
    * nameTypeCode can be one of: A=Alias Name,L=Legal Name,D=Display Name,M=Maiden Name,C=Adopted Name,B=Name at birth,P=Name of partner/spouse,U=Unspecified.
    */
    private void writeXPN(int fieldIndex, String fName, String lName, String middleI, String nameTypeCode) throws Exception {
        ...
        //XPN.1 Family Name.  Required (length 1..50).  Type FN (guide page 64).  Cardinality [1..1].  The FN type only requires the last name field and it is the first field.
        //XPN.2 Given Name.  Required (length 1..30).  Cardinality [1..1].
        //XPN.3 Second and Further Given Names or Initials Thereof (middle name).  Required if known (length 1..30).
    ...
    //XPN.7 Name Type Code.  Required if known (length 1..1).  Value set HL70200 (guide page 203).  A=Alias Name,L=Legal Name,D=Display Name,M=Maiden Name,C=Adopted Name,B=Name at birth,P=Name of partner/spouse,U=Unspecified.
    ...
    public static String validate(Patient pat, List<VaccinePat> vaccines) throws Exception {
        ...
        for (int i = 0;i < vaccines.Count;i++)
        {
            ...
            if (vaccine.CompletionStatus != VaccineCompletionStatus.NotAdministered)
            {
                //Some fields are not used when the vaccine was not administered.
                vaccineDef = VaccineDefs.getOne(vaccine.VaccineDefNum);
                ...
                if (vaccine.AdministrationNoteCode == VaccineAdministrationNote.NewRecord && StringSupport.equals(vaccine.LotNumber.Trim(), ""))
                {
                    writeError(sb,"Missing lot number.  Required for new records.");
                }
                 
                if (StringSupport.equals(vaccine.FilledCity.Trim(), ""))
                {
            ...
            List<String> stateCodes = new List<String>(new String[]{ "AK", "AL", "AR", "AZ", "CA", "CO", "CT", "DE", "FL", "GA", "HI", "IA", "ID", "IL", "IN", "KS", "KY", "LA", "MA", "MD", "ME", "MI", "MN", "MO", "MS", "MT", "NC", "ND", "NE", "NH", "NJ", "NM", "NV", "NY", "OH", "OK", "OR", "PA", "RI", "SC", "SD", "TN", "TX", "UT", "VA", "VT", "WA", "WI", "WV", "WY", "DC", "AS", "FM", "GU", "MH", "MP", "PR", "VI" });
            //50 States.
            //US Districts
            //US territories. Reference http://www.itl.nist.gov/fipspubs/fip5-2.htm
            //UM and PW are excluded here, because they are not allowed in HL7 table 0363.
            ...
            if (!StringSupport.equals(vaccine.LotNumber.Trim(), "") && vaccine.DateExpire.Year < 1880)
            {
            ...
            for (int j = 0;j < listVaccineObservations.Count;j++)
            {
                ...
                if (StringSupport.equals(vaccineObs.ValReported.Trim(), ""))
                {
                ...
                if (vaccineObs.IdentifyingCode == VaccineObsIdentifier.FundPgmEligCat && StringSupport.equals(vaccineObs.MethodCode.Trim(), ""))
                {
