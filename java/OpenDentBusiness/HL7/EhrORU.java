...
/**
* An ORU message is an Unsolicited Observation Message.  It is a message sent out from Open Dental containing immunization status in order to satisfy ehr public health requirements.
*/
public class EhrORU   
{
    ...
    * 
    */
    public EhrORU() throws Exception {
    }

    /**
    * Creates the Message object and fills it with data.  All panels will be for the same patient.
    */
    public void initialize(List<LabPanel> panels) throws Exception {
        if (panels.Count == 0)
        {
        ...
        for (int p = 0;p < panels.Count;p++)
        {
            ...
            //Simply shows an empty lab panel to the user.
    ...
    */
    private void mSH() throws Exception {
        ...
        //MSH-4: Sending Facility. Dummy.
        //MSH-5: Receiving Application.  Dummy.
        //MSH-6: Receiving Facility. Dummy.
        //MSH-9: fixed
        seg = new SegmentHL7("MSH|^~\\&|Open Dental|" + "OD|" + "Y|" + "OU|" + DateTime.Now.ToString("yyyyMMddHHmmss") + "||" + "ORU^R01^ORU_R01|" + DateTime.Now.ToString("yyyyMMddHHmmss") + "|" + "P|" + "2.3.1");
        //MSH-10: Control Id, str20. Fixed is ok for testing. We'll send date
        //MSH-11: P=production
    ...
    */
    private void pID(Patient pat) throws Exception {
        ...
        //Converts entries from PatientRace table to deprecated PatientRaceOld for call.
        seg.setField(11,pat.Address,pat.Address2,pat.City,pat.State,pat.Zip,"","M");
        //M is for mailing.
        seg.setField(13,convertPhone(pat.HmPhone));
        seg.setField(22,convertEthnicGroup(PatientRaces.getPatientRaceOldFromPatientRaces(pat.PatNum)));
        //Converts entries from PatientRace table to deprecated PatientRaceOld for call.
        msg.Segments.Add(seg);
    }
    ...
    /*This is example #5 from the mu site.  Hepatitis C is a legitimate reportable syndrome which would be reported to public health
    		...
    		*/
    private void oBR(LabPanel panel, DateTime datetime) throws Exception {
        ...
        //eg 10676-5, Hepatitis C Virus RNA
    ...
    //OBX|1|CE30955-9&30956-7^Vaccine type^LN|1|08^HepB-Adolescent/pediatric^CVX|
    /**
    * idx passed in will be zero-based. Will be converted to 1-based.
    */
    private void oBX(LabResult labresult, int idx) throws Exception {
        seg = new SegmentHL7(SegmentNameHL7.OBX);
        seg.setField(0,"OBX");
        seg.SetField(1, (idx + 1).ToString());
        seg.setField(2,"NM");
        //ValueType. NM=numeric, referring to the value that will follow in OBX-5
        seg.setField(3,labresult.TestID,labresult.TestName,"LN");
        //TestPerformed  ID^text^codingSystem.  eg. 10676-5^Hepatitis C Virus RNA^LN
        seg.setField(4,"1");
        //?
        seg.setField(5,labresult.ObsValue);
        //Value. Type based on OBX-2.  eg. 850000.
        seg.setField(6,labresult.ObsUnits);
        //Units. Example iU/mL^international units per mililiter
        seg.setField(7,labresult.ObsRange);
        //Range. Example High Viral Load >= 850000iU/mL
        seg.setField(8,"N");
        //?
        seg.setField(11,"F");
        //OBX-11 is required.  F means final.
        seg.SetField(14, labresult.DateTimeTest.ToString("yyyyMMddhhmmss"));
        //OBX-14 datetime
        msg.Segments.Add(seg);
    }
    ...
    /**
    * Uses deprecated PatientRaceOld enum which should be converted from PatientRace entries before this function is called.
    */
    ...
    * Uses deprecated PatientRaceOld enum which should be converted from PatientRace entries before this function is called.
    */
    private String convertEthnicGroup(PatientRaceOld race) throws Exception {
        switch(race)
        {
            case HispanicLatino: 
    ...
    private String convertPhone(String phone) throws Exception {
        ...
        for (int i = 0;i < phone.Length;i++)
        {
            ...
            if (StringSupport.equals(digits, "") && StringSupport.equals(phone.Substring(i, 1), "1"))
            {
                continue;
            }
        ...
        //1:deprecated
        retVal += "PRN^";
        //2:table201. PRN=primary residence number.
        retVal += "^^^";
        //3-5:
        retVal += digits.Substring(0, 3) + "^";
        //6:area code
